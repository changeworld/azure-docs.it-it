...
Nella copia di Microsoft Azure IoT SDK per Node.js scaricata in precedenza aprire il file *azure-iot-sdk-node/device/samples/pnp/simple_thermostat.js* in un editor di testo.
...
* Crea un gestore di proprietà scrivibili.
...
La funzione `sendTelemetry` consente al dispositivo di inviare i dati di telemetria sulla temperatura a IoT Central. Il metodo `getCurrentTemperatureObject` restituisce un oggetto simile a `{ temperature: 45.6 }`:
...
Il metodo `main` usa i due metodi seguenti per inviare la proprietà `maxTempSinceLastReboot` a IoT Central. Il metodo `main` chiama `createReportPropPatch` con un oggetto simile a `{maxTempSinceLastReboot: 80.9}`:
...
Il metodo `main` usa i due metodi seguenti per gestire gli aggiornamenti della proprietà scrivibile per la _temperatura di destinazione_ da IoT Central. Notare come `propertyUpdateHandle` crea la risposta con la versione e il codice di stato:
...
Il metodo `main` usa i due metodi seguenti per gestire le chiamate al comando `getMaxMinReport`. Il metodo `getMaxMinReportObject` genera il report come oggetto JSON:
...
Per eseguire l'applicazione di esempio, aprire un ambiente di riga di comando e passare alla cartella *azure-iot-sdk-node/device/samples/pnp* che contiene il file di esempio *simple_thermostat.js*.
