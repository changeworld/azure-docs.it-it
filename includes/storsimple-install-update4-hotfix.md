...
8. Ripetere i passaggi da 3 a 5 per installare gli aggiornamenti di secondo livello. **Per gli aggiornamenti secondo ordine, è possibile installare più aggiornamenti eseguendo `Start-HcsHotfix cmdlet` e puntando alla cartella in cui si trovano gli aggiornamenti secondo ordine. Il cmdlet eseguirà tutti gli aggiornamenti disponibili nella cartella.** Se è già installato un aggiornamento, la logica di aggiornamento lo rileva e non applica l'aggiornamento. 
...
1. Attivare la modalità di manutenzione per il dispositivo. **Si noti che non è consigliabile usare la comunicazione remota di Windows PowerShell quando ci si connette a un dispositivo in modalità di manutenzione. Eseguire invece questo cmdlet nel controller del dispositivo quando si è connessi tramite la console seriale del dispositivo.** Digitare:
