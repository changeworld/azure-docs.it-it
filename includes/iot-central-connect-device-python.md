...
Nella copia di Microsoft Azure SDK per Python che è stato scaricato in precedenza, aprire il file *Azure-le cose-SDK-Python/Azure-Internet (Device/Samples/PNP/temp_controller_with_thermostats. py* in un editor di testo.
...
* Invia i valori della proprietà iniziale a IoT Central. USA `pnp_helper` per creare le patch.
* Crea listener per i `getMaxMinReport` comandi e `reboot` . Ogni componente del termostato ha un proprio `getMaxMinReport` comando.
* Crea un listener di proprietà, per restare in ascolto degli aggiornamenti delle proprietà scrivibili.
* Avvia un ciclo per inviare i dati di telemetria della temperatura dai due componenti del termostato e working set i dati di telemetria dal componente predefinito ogni 8 secondi.
...
La `execute_command_listener` funzione gestisce le richieste di comando, esegue la `max_min_handler` funzione quando il dispositivo riceve il `getMaxMinReport` comando per i componenti del termostato e la `reboot_handler` funzione quando il dispositivo riceve il `reboot` comando. Usa il `pnp_helper` modulo per compilare la risposta:
...
`async def execute_property_listener`Gestisce gli aggiornamenti delle proprietà scrivibili come `targetTemperature` per i componenti del termostato e genera la risposta JSON. Usa il `pnp_helper` modulo per compilare la risposta:
...
La `send_telemetry_from_temp_controller` funzione Invia i messaggi di telemetria dai componenti del termostato al IOT Central. Usa il `pnp_helper` modulo per compilare i messaggi:
...
Per eseguire l'applicazione di esempio, aprire un ambiente della riga di comando e passare alla cartella *Azure-l'-SDK-Python/Azure-Internet, ovvero Device/Samples/PNP* che contiene il file di esempio *temp_controller_with_thermostats. py* .
