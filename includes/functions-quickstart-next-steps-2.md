---
author: ggailey777
ms.service: azure-functions
ms.topic: include
ms.date: 01/23/2019
ms.author: glenga
ms.openlocfilehash: a02eadcacddc91ae9d17c50efd5e573718c41a21
ms.sourcegitcommit: 772eb9c6684dd4864e0ba507945a83e48b8c16f0
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 03/19/2021
ms.locfileid: "67180819"
---
* [Concetti relativi a trigger e associazioni in Funzioni di Azure](../articles/azure-functions/functions-triggers-bindings.md)  
  Informazioni su come Funzioni si integra con altri servizi.  
* [Guida di riferimento per gli sviluppatori a Funzioni di Azure](../articles/azure-functions/functions-reference.md)  
  Include informazioni più tecniche sul runtime di Funzioni, nonché informazioni di riferimento per la codifica di funzioni e la definizione di trigger e associazioni.
* [Scrivere codici per Funzioni di Azure e testarle in locale](../articles/azure-functions/functions-develop-local.md)  
  Descrive le opzioni per lo sviluppo di funzioni in locale.
