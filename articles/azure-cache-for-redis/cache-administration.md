...
![Screenshot che evidenzia l'opzione di menu Riavvia.](./media/cache-administration/redis-cache-administration-reboot.png)
...
* **Master:** quando il nodo primario viene riavviato, cache di Azure per Redis esegue il cache di Azure per Redis sul nodo di replica e lo alza di livello a primario. Durante il failover potrebbe verificarsi un breve intervallo in cui le connessioni alla cache potrebbero avere esito negativo.
* **Replica:** quando il nodo di replica viene riavviato, in genere non c'è alcun impatto sui client della cache.
* **Primario e replica:** quando entrambi i nodi della cache vengono riavviati, tutti i dati vengono persi nella cache e le connessioni alla cache hanno esito negativo fino a quando il nodo primario non torna online. Se è stato configurato il [salvataggio permanente dei dati](cache-how-to-premium-persistence.md), il backup più recente viene ripristinato quando la cache ritorna in linea, ma tutte le scritture nella cache che si sono verificate dopo l'ultimo backup andranno perse.
...
Il **pannello Pianifica aggiornamenti** consente di definire una finestra di manutenzione per l'istanza della cache. Una finestra di manutenzione consente di controllare i giorni e l'ora di una settimana durante i quali è possibile aggiornare le macchine virtuali che ospitano la cache. cache di Azure per Redis possibile avviare e completare l'aggiornamento del software del server Redis entro l'intervallo di tempo specificato definito.

> [!NOTE] 
> La finestra di manutenzione si applica agli aggiornamenti del server Redis e al sistema operativo delle macchine virtuali che ospitano la cache. La finestra di manutenzione non si applica agli aggiornamenti del sistema operativo host per gli host che ospitano le macchine virtuali della cache o altri Rete di Azure componenti. In rari casi, in cui le cache sono ospitate in modelli meno recenti (è possibile determinare se la cache si trova in un modello precedente se il nome DNS della cache viene risolto in un suffisso "cloudapp.net", "chinacloudapp.cn", "usgovcloudapi.net" o "cloudapi.de"), la finestra di manutenzione non verrà applicata neanche agli aggiornamenti del sistema operativo guest.
>
...
Altre informazioni sulle cache di Azure per Redis funzionalità.

* [cache di Azure per Redis livelli di servizio](cache-overview.md#service-tiers)
