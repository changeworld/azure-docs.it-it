...
description: La cache di Azure per la linea di base di sicurezza di redis fornisce le informazioni e le risorse procedurali per l'implementazione delle raccomandazioni di sicurezza specificate nel benchmark di sicurezza di Azure.
author: msmbaldwin
ms.service: cache
ms.topic: conceptual
ms.date: 02/17/2021
...
Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1,0](../security/benchmarks/overview-v1.md) alla cache di Azure per Redis. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure.
Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili alla cache di Azure per Redis. I **controlli** non applicabili alla cache di Azure per Redis sono stati esclusi.

 
Per informazioni sul mapping completo della cache di Azure per Redis al benchmark di sicurezza di Azure, vedere il [file di mapping di base della sicurezza di Azure per Redis](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
- [Come configurare il supporto di una rete virtuale per una cache Premium di Azure per Redis](cache-how-to-premium-vnet.md)

- [Come configurare la cache di Azure per le regole del firewall Redis](https://docs.microsoft.com/azure/azure-cache-for-redis/cache-configure#firewall)
...
- [Gruppi di sicurezza delle applicazioni](/azure/virtual-network/security-overview#application-security-groups)
...
**Linee guida**: l'accesso al piano di controllo per cache di Azure per Redis viene controllato tramite Azure Active Directory (Azure ad). Azure AD non è il concetto di password predefinite.
...
- [Informazioni sull'API REST di cache di Azure per Redis](/rest/api/redis/)
...
**Linee guida**: configurare i percorsi denominati nell'accesso condizionale Azure Active Directory (Azure ad) per consentire l'accesso solo da specifici raggruppamenti logici di intervalli di indirizzi IP o paesi/aree geografiche.
...
- [Come distribuire cache di Azure per Redis in una VNET](cache-how-to-premium-vnet.md)
...
- [Informazioni sulle porte necessarie usate negli scenari di cache VNET](https://docs.microsoft.com/azure/azure-cache-for-redis/cache-how-to-premium-vnet#outbound-port-requirements)
...
- [Informazioni sulla crittografia per gli account di archiviazione di Azure](../storage/common/storage-service-encryption.md)
...
- [Come creare e usare i tag delle risorse](/azure/azure-resource-manager/resource-group-using-tags)
...
- [Come eseguire l'autenticazione a Key Vault](/azure/key-vault/managed-identity)
...
- [Come abilitare la persistenza di redis](cache-how-to-premium-persistence.md)

- [Come usare cache di Azure per l'esportazione Redis](cache-how-to-import-export-data.md)
...
- [Come usare cache di Azure per l'importazione Redis](cache-how-to-import-export-data.md)
