...
[Asset credenziali in Automazione di Azure](../automation/shared-resources/credentials.md)   
[Asset di tipo certificato in Automazione di Azure](../automation/shared-resources/certificates.md)
...
#### <a name="using-azanalysisservices-module"></a><a name="azmodule"></a>Uso del modulo Az.AnalysisServices
...
Quando ci si connette ad applicazioni client e app Web, i pacchetti installabili delle [librerie client AMO e ADOMD](/analysis-services/client-libraries?view=azure-analysis-services-current&preserve-view=true) versione 15.0.2 e successive di NuGet supportano le entità servizio nelle stringhe di connessione con la sintassi seguente: `app:AppID` e password o `cert:thumbprint`. 
...
[Automatizzare attività dell'area di lavoro e del set di dati di Power BI Premium con entità servizio](/power-bi/admin/service-premium-service-principal)