...
description: In questa esercitazione viene illustrato come configurare i ruoli utente e amministratore di Azure Analysis Services con il portale di Azure oppure SQL Server Management Studio.
author: minewiskan
...
- [Installare la versione più recente di SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms) (SSMS).
...
2. Nella finestra di dialogo **Connetti a server** incollare il nome del server copiato dal portale in **Nome del server**. In **Autenticazione** scegliere **Active Directory - Universale con supporto MFA**, quindi immettere l'account utente e infine fare clic su **Connetti**.
    ...
    > [!TIP]
    > È consigliabile scegliere Active Directory - Universale con supporto MFA. Questo tipo di autenticazione supporta l'[autenticazione a più fattori e non interattiva](../../azure-sql/database/authentication-mfa-ssms-overview.md). 
...
3. In **Esplora oggetti** espandere per visualizzare gli oggetti del server. Fare clic con il pulsante destro del mouse per visualizzare le proprietà del server.
    ...
    ![Esplora oggetti in SSMS](./media/analysis-services-tutorial-roles/aas-connect-ssms-objexp.png)
...
In questa attività si aggiunge un account utente o gruppo da Azure AD al ruolo di amministratore del server. Se si specifica un gruppo di sicurezza, usare `obj:groupid@tenantid`.
...
In questa attività si usa il comando [Create](/analysis-services/tmsl/create-command-tmsl) in uno script TMSL per creare un nuovo ruolo Internet Sales Global, specificare le autorizzazioni di *lettura* per il ruolo e aggiungere un account utente o gruppo da Azure AD.
