...
Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1,0](../security/benchmarks/overview-v1.md) alla griglia di eventi Microsoft Azure. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure.
Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili a griglia di eventi di Azure. I **controlli** non applicabili a griglia di eventi di Azure sono stati esclusi.

 
Per informazioni sul mapping completo di griglia di eventi di Azure al benchmark di sicurezza di Azure, vedere il file di mapping di base per la [sicurezza della griglia di eventi di Azure](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
È anche possibile usare le definizioni di criteri predefinite correlate a griglia di eventi di Azure, ad esempio:
...
**Linee guida**: la gestione degli accessi alle risorse di griglia di eventi viene controllata tramite Azure Active Directory (Azure ad). Azure AD non è il concetto di password predefinite.
...
**Indicazioni**: Non applicabile. Griglia di eventi non usa Azure Active Directory (Azure AD) per l'autenticazione dei client di pubblicazione di eventi; supporta l'autenticazione tramite chiavi di firma di accesso condiviso.
...
**Linee guida**: griglia di eventi di Azure supporta l'uso di Azure Active Directory (Azure ad) per autorizzare le richieste alle risorse di griglia di eventi. Con Azure AD, è possibile usare il controllo degli accessi in base al ruolo di Azure per concedere le autorizzazioni a un'entità di sicurezza, che può essere un utente o un'entità servizio dell'applicazione.
