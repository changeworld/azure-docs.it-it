...
description: Pianificare un vFXT per il cluster di Azure più adatto alle proprie esigenze. Scopri le domande da porre prima di passare ad Azure Marketplace o creare macchine virtuali.
author: ekpgh
...
* Usare il modello di creazione del cluster per creare la maggior parte delle risorse di infrastruttura necessarie per il cluster, inclusi i gruppi di risorse, le reti virtuali, le subnet e gli account di archiviazione.
  ...
  | Risorsa | Usare existing? | Requisiti |
  |----------|-----------|----------|
...
* Jump host: assegnare un indirizzo IP pubblico a una macchina virtuale separata all'interno della rete privata e usarla per creare un tunnel TLS nei nodi del cluster.
