---
title: Creare set di impostazioni in Azure Data Factory
...
| structure |Schema del set di dati.<br/><br/>Per informazioni dettagliate, vedere [struttura del set di dati](#Structure). |No |N/D |
| typeProperties | Le proprietà del tipo sono diverse per ogni tipo, ad esempio BLOB di Azure Blob, tabella SQL di Azure. Per informazioni dettagliate sui tipi supportati e le relative proprietà, vedere la sezione [Tipo di set di dati](#Type). |Sì |N/D |
...
* il **tipo** è impostato su AzureSqlTable.
* la proprietà **TableName** Type (specifica del tipo AzureSqlTable) è impostata su MyTable.
...
## <a name="dataset-type"></a><a name="Type"></a> Tipo di set di dati
...
## <a name="dataset-structure"></a><a name="Structure"></a>Struttura del set di dati
...
| offset |Intervallo di tempo in base al quale l'inizio e la fine di tutte le sezioni dei set di dati vengono spostate. <br/><br/>Notare che se si specifica sia **anchorDateTime** che **offset**, il risultato sarà lo spostamento combinato. |No |N/D |
...
## <a name="dataset-policy"></a><a name="Policy"></a>Criteri di set di dati
