---
title: "Esercitazione: Creare una pipeline con l'attività di copia usando l'API .NET "
...
Questo articolo illustra l'uso dell'[API .NET](https://portal.azure.com) per creare una data factory con una pipeline che copia i dati da una risorsa di Archiviazione BLOB di Azure al database SQL di Azure. Se non si ha familiarità con Azure Data Factory, prima di eseguire questa esercitazione vedere l'articolo [Introduzione ad Azure Data Factory](data-factory-introduction.md).   
...
> [!NOTE] 
> Per la documentazione completa sull'API .NET per Data Factory, vedere le [informazioni di riferimento sull'API NET di Data Factory](/dotnet/api/overview/azure/data-factory).
> 
...
* Azure PowerShell. Seguire le istruzioni disponibili nell'articolo [Come installare e configurare Azure PowerShell](/powershell/azure/install-Az-ps) per installare la versione più recente di Azure PowerShell nel computer. Azure PowerShell verrà usato per creare un'applicazione Azure Active Directory.
...
9. Aggiungere al metodo **Main** il codice seguente che crea un **servizio collegato di Azure SQL**.
   ...
   > [!IMPORTANT]
   > Sostituire **servername**, **databasename**, **username** e **password** con i nomi del server, del database, dell'utente e della password.
...
10. Aggiungere al metodo **Main** il codice seguente che crea **set di dati di input e output**.
    ...
    In questo passaggio si crea un set di dati di output denominato **OutputDataset**. Questo set di dati punta a una tabella SQL nel database rappresentato da **AzureSqlLinkedService**.
...
20. Verificare che i due record dipendente vengano creati nella tabella **emp** del database specificato.
