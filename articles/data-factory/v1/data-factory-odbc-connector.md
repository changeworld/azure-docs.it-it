---
title: Spostare dati da archivi dati ODBC
...
> * [Versione 1](data-factory-odbc-connector.md)
> * [Versione 2 (corrente)](../connector-odbc.md)

> [!NOTE]
> Le informazioni di questo articolo sono valide per la versione 1 di Data Factory. Se si usa la versione corrente del servizio Data Factory, vedere le informazioni sul [connettore ODBC nella versione 2](../connector-odbc.md).
...
È possibile crittografare le credenziali usando il cmdlet [New-AzDataFactoryEncryptValue](/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) (versione 1,0 del Azure PowerShell) o [New-AzureDataFactoryEncryptValue](/previous-versions/azure/dn834940(v=azure.100)) (0,9 o una versione precedente della Azure PowerShell).
...
La sezione **typeProperties** è diversa per ogni tipo di set di dati e fornisce informazioni sul percorso dei dati nell'archivio dati. La sezione typeProperties per il set di dati di tipo **RelationalTable** , che include il set di dati ODBC, presenta le proprietà seguenti
...
Le proprietà disponibili nella sezione **typeProperties** dell'attività variano invece a seconda del tipo di attività. Per l'attività di copia variano in base ai tipi di origine e sink.
...
Questo esempio fornisce le definizioni JSON che è possibile usare per creare una pipeline usando [Visual Studio](data-factory-copy-activity-tutorial-using-visual-studio.md) o [Azure PowerShell](data-factory-copy-activity-tutorial-using-powershell.md). Illustra come copiare dati da un'origine ODBC in un archivio BLOB di Azure. Tuttavia, i dati possono essere copiati in qualsiasi sink dichiarato [qui](data-factory-data-movement-activities.md#supported-data-stores-and-formats) usando l'attività di copia in Azure Data Factory.
...
Nell'esempio si presuppone che sia stata creata una tabella "MyTable" in un database ODBC e che contenga una colonna denominata "timestampcolumn" per i dati delle serie temporali.
...
Quando si spostano dati da archivi dati ODBC, viene eseguito il mapping dei tipi di dati ODBC ai tipi .NET, come riportato nell'argomento [Mapping dei tipi di dati ODBC](/dotnet/framework/data/adonet/odbc-data-type-mappings) .
