...
> * [Versione 1](data-factory-azure-sql-connector.md)
> * [Versione 2 (corrente)](../connector-azure-sql-database.md)

> [!NOTE]
> Le informazioni di questo articolo sono valide per la versione 1 di Data Factory. Se si usa la versione corrente del servizio Data Factory, vedere [Connettore del database SQL di Azure nella versione 2](../connector-azure-sql-database.md).
...
Un servizio collegato SQL di Azure collega il database SQL di Azure al data factory. La tabella seguente fornisce la descrizione degli elementi JSON specifici del servizio collegato SQL di Azure.
...
L'esempio copia i dati di una serie temporale (oraria, giornaliera e così via) da una tabella del database SQL di Azure a un BLOB ogni ora. Le proprietà JSON usate in questi esempi sono descritte nelle sezioni riportate dopo gli esempi.
...
```
Vedere la sezione sul servizio collegato SQL di Azure per l'elenco delle proprietà supportate da questo servizio collegato.
...
Vedere la sezione sulle proprietà del tipo di set di dati SQL di Azure per l'elenco delle proprietà supportate da questo tipo di set di dati.
...
L'esempio copia i dati di una serie temporale (oraria, giornaliera e così via) dal BLOB di Azure a una tabella nel database SQL di Azure ogni ora. Le proprietà JSON usate in questi esempi sono descritte nelle sezioni riportate dopo gli esempi.
...
Quando si copiano dati in un database SQL Server, per impostazione predefinita l'attività di copia accoda i dati alla tabella di sink. Per eseguire un UPSERT, vedere l'articolo relativo [alla scrittura ripetibile in sqlsink](data-factory-repeatable-copy.md#repeatable-write-to-sqlsink) .
