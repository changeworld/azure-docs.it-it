...
Lo sconto di capacità riservata flusso di dati ADF viene applicato all'esecuzione di runtime di integrazione su base oraria. La prenotazione acquistata viene confrontata con il tipo di calcolo usato dai runtime di integrazione per i flussi di dati. Per i flussi di dati che non vengono eseguiti nell'ora completa, la prenotazione viene applicata automaticamente ad altri flussi di dati corrispondenti agli attributi di prenotazione. Lo sconto può essere applicato anche ai flussi di dati in esecuzione simultanea. Se non si dispone di flussi di dati che vengono eseguiti per l'ora intera che corrispondono agli attributi di prenotazione, non si ottiene il vantaggio completo dello sconto relativo alla prenotazione per quell'ora.
