...
3. Attivare l' [ottimizzazione automatica](/azure/databricks/delta/optimizations/auto-optimize) aggiungendo le proprietà seguenti alla configurazione di [Spark](/azure/databricks/clusters/configure#spark-config):
...
Per informazioni dettagliate sulla configurazione del cluster, vedere [configurare i cluster](/azure/databricks/clusters/configure).
...
| ***In `exportSettings` :** _ |  |  |
| tipo | Il tipo di comando Export, impostato su _ * AzureDatabricksDeltaLakeExportCommand * *. | Sì |
...
| tipo | Tipo di comando Import impostato su _ * AzureDatabricksDeltaLakeImportCommand * *. | Sì |
