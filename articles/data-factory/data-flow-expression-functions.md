...
Controlla se il primo valore è compreso tra due valori. È possibile confrontare i valori numerici, stringa e DateTime * ``between(10, 5, 24)``
* ``true``
...
Operatore and bit per bit tra tipi integrali. Uguale all'operatore & * ``bitwiseAnd(0xf4, 0xef)``
* ``0xe4``
* ``(0xf4 & 0xef)``
* ``0xe4``
___
...
Operatore OR bit per bit nei tipi integrali. Uguale a | operatore * ``bitwiseOr(0xf4, 0xef)``
* ``0xff``
* ``(0xf4 | 0xef)``
* ``0xff``
___
...
Operatore OR bit per bit nei tipi integrali. Uguale a | operatore * ``bitwiseXor(0xf4, 0xef)``
* ``0x1b``
* ``(0xf4 ^ 0xef)``
* ``0x1b``
* ``(true ^ false)``
...
Calcola il digest Blake2 del set di colonne di tipi di dati primitivi variabili con una lunghezza di bit che può essere costituita solo da più di 8 tra 8 & 512. Può essere usato per calcolare un'impronta digitale per una riga * ``blake2b(256, 'gunchus', 8.2, 'bojjus', true, toDate('2010-4-4'))``
* ``'c9521a5080d8da30dffb430c50ce253c345cc4c4effc315dab2162dac974711d'``
___
### <code>blake2bBinary</code>
<code><b>blake2bBinary(<i>&lt;value1&gt;</i> : integer, <i>&lt;value2&gt;</i> : any, ...) => binary</b></code><br/><br/>
Calcola il digest Blake2 del set di colonne di tipi di dati primitivi variabili con una lunghezza di bit che può essere costituita solo da più di 8 tra 8 & 512. Può essere usato per calcolare un'impronta digitale per una riga * ``blake2bBinary(256, 'gunchus', 8.2, 'bojjus', true, toDate('2010-4-4'))``
...
___
### <code>mapIf</code>
<code><b>mapIf (<value1> : array, <value2> : binaryfunction, <value3>: binaryFunction) => any</b></il codice><BR/><BR/> esegue il mapping condizionale di una matrice a un'altra matrice di lunghezza uguale o inferiore. I valori possono essere di qualsiasi tipo di dati, incluso structTypes. Accetta una funzione di mapping in cui è possibile indirizzare l'elemento nella matrice come #item e l'indice corrente come #index. Per le mappe profondamente annidate, è possibile fare riferimento alla parentesi t maps using the ``#item_[n](#item_1 , #index_1...)`` notation.
...
___
### <code>mapLoop</code>
<code><b>mapLoop(<value1> : integer, <value2> : unaryfunction) => any</b></il codice><BR/><BR/> passa da 1 a lunghezza per creare una matrice di tale lunghezza. Accetta una funzione di mapping in cui è possibile indirizzare l'indice nella matrice come #index. Per le mappe profondamente annidate, è possibile fare riferimento alle mappe padre usando il #index_n (#index_1, #index_2...) notation.
...
I controlli del valore stringa sono un valore integer dato un formato facoltativo secondo le regole di ``toInteger()``
* ``isInteger('123') -> true``
* ``isInteger('$123' -> '$###') -> true``
...
I controlli del valore stringa sono un valore Long dato un formato facoltativo secondo le regole di ``toLong()``
* ``isLong('123') -> true``
* ``isLong('$123' -> '$###') -> true``
...
I controlli del valore di stringa sono un valore float dato un formato facoltativo secondo le regole di ``toFloat()``
* ``isFloat('123') -> true``
...
I controlli del valore stringa sono un valore Double dato un formato facoltativo secondo le regole di ``toDouble()``
* ``isDouble('123') -> true``
...
I controlli del valore stringa sono un valore decimale dato un formato facoltativo secondo le regole di ``toDecimal()``
* ``isDecimal('123.45') -> true``
* ``isDecimal('12/12/2000') -> false``
___
### <code>isTimestamp</code>
<code><b>isTimestamp (<value1> : string, [<format>: string]) => boolean</b></code><br/><br/>
Controlla se la stringa della data di input è un timestamp usando un formato di timestamp di input facoltativo. Vedere SimpleDateFormat di Java per i formati disponibili. Se il timestamp viene omesso ``yyyy-[M]M-[d]d hh:mm:ss[.f...]`` , viene usato il criterio predefinito. È possibile passare un fuso orario facoltativo nel formato 'GMT', 'PST', 'UTC', 'America/Cayman'. Timestamp supporta la precisione fino a millisecondo con valore 999 per i formati disponibili in Java SimpleDateFormat.
...
Restituisce una rappresentazione di stringa esadecimale di un valore binario * ``hex(toBinary([toByte(0x1f), toByte(0xad), toByte(0xbe)])) -> '1fadbe'``
___
### <code>unhex</code>
<code><b>unhex(<value1>: string) => binary</b></code><br/><br/>
Unhexes un valore binario dalla relativa rappresentazione di stringa. Questa operazione può essere usata insieme a SHA2, MD5 per la conversione da stringa a rappresentazione binaria *   ``unhex('1fadbe') -> toBinary([toByte(0x1f), toByte(0xad), toByte(0xbe)])``
