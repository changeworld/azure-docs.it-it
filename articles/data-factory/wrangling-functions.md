---
title: Funzioni di data Litigating in Azure Data Factory
description: Panoramica delle funzioni di confronto dei dati disponibili in Azure Data Factory
author: kromerm
ms.author: makromer
...
La creazione di dati in Azure Data Factory consente di eseguire la preparazione dei dati agile senza codice e la possibilità di litigare a livello di cloud traducendo gli ```M``` script Power query in uno script del flusso di dati. ADF si integra con [Power query online](/powerquery-m/power-query-m-reference) e rende ```M``` disponibili Power Query funzioni per data wrangling tramite l'esecuzione di Spark tramite l'infrastruttura Spark del flusso di dati. 

> [!NOTE]
> Power Query in ADF è attualmente avilable in anteprima pubblica
...
Informazioni su come [creare una data wrangling Power query in ADF](wrangling-tutorial.md).
