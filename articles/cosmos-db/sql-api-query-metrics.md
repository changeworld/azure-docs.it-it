...
Azure Cosmos DB fornisce un'[API SQL per le query sui dati](./sql-query-getting-started.md), senza che siano necessari schemi o indici secondari. In questo articolo vengono fornite le seguenti informazioni per gli sviluppatori:
...
Per altre informazioni sul partizionamento e sulle chiavi di partizione, vedere [Partizionamento in Azure Cosmos DB](partitioning-overview.md).
