---
title: 'Esercitazione: Creare applicazioni per dispositivi mobili con Xamarin e Azure Cosmos DB'
description: "Esercitazione: Esercitazione in cui si crea un'applicazione Xamarin iOS, Android o Forms usando Azure Cosmos DB. Azure Cosmos DB è un database su cloud per dispositivi mobili veloce e dotato di copertura globale."
author: SnehaGunda
...
* Autorizzazioni avanzate integrate. Con Azure Cosmos DB è possibile implementare facilmente modelli diffusi come i [dati per utente](https://github.com/Azure/azure-cosmos-dotnet-v2/tree/master/samples/xamarin/UserItems) o i dati condivisi fra più utenti senza codice di autorizzazione personalizzato complesso.
...
L'esercitazione seguente illustra come creare un'app per dispositivi mobili usando Xamarin e Azure Cosmos DB. È possibile trovare il codice sorgente completo per l'esercitazione in [Xamarin e Azure Cosmos DB su Github](https://github.com/Azure/azure-cosmos-dotnet-v2/tree/master/samples/xamarin).
...
:::image type="content" source="media/mobile-apps-with-xamarin/cosmos-db-quickstart.png" alt-text="Avvio rapido di Azure Cosmos DB per le app per dispositivi mobili":::
...
Come molti esempi introduttivi, l'esempio di Azure Cosmos DB che è stato scaricato si autentica al servizio usando la chiave principale specificata nel codice dell'app. Questa operazione predefinita non è una buona pratica per un'app che si vuole eseguire da qualsiasi posizione che non sia l'emulatore locale. Se un utente non autorizzato ottiene la chiave principale, tutti i dati nell'intero account Azure Cosmos DB potrebbero essere compromessi. Si desidera invece che l'app abbia accesso solo ai record per l'utente connesso. Azure Cosmos DB consente agli sviluppatori di concedere all'applicazione l'autorizzazione in lettura o in lettura/scrittura a un insieme, un set di documenti raggruppati in base a una chiave di partizione o a un documento specifico. 
...
È possibile trovare un esempio di codice completo di questo modello in [Resource Token Broker su Github](https://github.com/Azure/azure-cosmos-dotnet-v2/tree/master/samples/xamarin/UserItems). Questo diagramma illustra la soluzione:

:::image type="content" source="media/mobile-apps-with-xamarin/documentdb-resource-token-broker.png" alt-text="Utenti di Azure Cosmos DB e broker delle autorizzazioni" border="false":::
...
:::image type="content" source="media/mobile-apps-with-xamarin/cosmos-db-xamarin-scale.png" alt-text="Scalabilità della velocità effettiva su richiesta di Azure Cosmos DB":::
...
:::image type="content" source="media/mobile-apps-with-xamarin/cosmos-db-xamarin-replicate.png" alt-text="Scalabilità fra aree geografiche di Azure Cosmos DB" border="false":::
...
* Visualizzare il codice sorgente per [Xamarin e Azure Cosmos DB su Github](https://github.com/Azure/azure-cosmos-dotnet-v2/tree/master/samples/xamarin).
