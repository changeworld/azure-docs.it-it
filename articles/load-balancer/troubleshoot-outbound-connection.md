---
title: Risolvere i problemi relativi alle connessioni in uscita Azure Load Balancer
description: Soluzioni per i problemi comuni relativi alla connettività in uscita tramite Azure Load Balancer.
...
Questo articolo ha lo scopo di fornire soluzioni per i problemi comuni che possono verificarsi con le connessioni in uscita da un Azure Load Balancer. La maggior parte dei problemi con la connettività in uscita che i clienti possono verificare sono dovuti all'esaurimento delle porte SNAT e ai timeout di connessione che causano l'esaurimento dei pacchetti. Questo articolo illustra i passaggi per attenuare ognuno di questi problemi.

## <a name="managing-snat-pat-port-exhaustion"></a><a name="snatexhaust"></a> Gestione dell'esaurimento delle porte SNAT (PAT)
[Le porte effimere](load-balancer-outbound-connections.md) usate per [PAT](load-balancer-outbound-connections.md) sono una risorsa esauribile, come descritto in Macchina virtuale autonoma senza un indirizzo [IP](load-balancer-outbound-connections.md) pubblico e Macchina virtuale con carico bilanciato senza un [indirizzo IP pubblico.](load-balancer-outbound-connections.md) È possibile monitorare l'utilizzo delle porte effimere e confrontarlo con l'allocazione corrente per determinare il rischio di o per confermare l'esaurimento SNAT usando [questa](./load-balancer-standard-diagnostics.md#how-do-i-check-my-snat-port-usage-and-allocation) guida.
...
L'assegnazione di un indirizzo IP pubblico modifica lo scenario da [IP pubblico a una macchina virtuale](load-balancer-outbound-connections.md). Tutte le porte temporanee dell'IP pubblico usate per ogni VM sono disponibili per la VM, A differenza degli scenari in cui le porte effimere di un indirizzo IP pubblico vengono condivise con tutte le macchine virtuali associate al rispettivo pool back-end. È necessario prendere in considerazione alcuni compromessi, ad esempio il costo aggiuntivo degli indirizzi IP pubblici e il potenziale impatto del filtro di un numero elevato di singoli indirizzi IP.
...
Le [porte preallocate](load-balancer-outbound-connections.md#preallocatedports) vengono assegnate in base alle dimensioni del pool back-end e raggruppate in livelli per ridurre al minimo le interruzioni quando alcune delle porte devono essere riallocate in base al successivo livello di dimensioni maggiore del pool back-end.  È possibile scegliere di aumentare l'utilizzo delle porte SNAT per un determinato front-end ridimensionando il pool back-end fino alle dimensioni massime per un determinato livello.  Tenere presente che l'allocazione predefinita delle porte è necessaria per l'applicazione per aumentare il numero di richieste in modo efficiente senza rischiare l'esaurimento SNAT.

Ad esempio, due macchine virtuali nel pool back-end devono avere 1024 porte SNAT disponibili per ogni configurazione IP, per un totale di 2048 porte SNAT per la distribuzione.  Se la distribuzione viene aumentata a 50 macchine virtuali, anche se il numero di porte preallocate resta costante per ogni macchina virtuale, potrà essere usato un totale di 51.200 (50 x 1024) porte SNAT dalla distribuzione.  Se si vuole aumentare il numero di dimensioni della distribuzione, controllare il numero di porte [preallocate](load-balancer-outbound-connections.md#preallocatedports) per ogni livello per assicurarsi di modellare il numero di porte al massimo per il rispettivo livello.  Se nell'esempio precedente si sceglie di aumentare il numero di istanze fino a 51 anziché a 50, si passa al livello successivo, finendo con meno porte SNAT per ogni macchina virtuale e in totale.
...
Quando si usano keep-alive TCP, è sufficiente abilitarli sul lato della connessione. Ad esempio, è sufficiente abilitarli sul lato server solo per reimpostare il timer di inattività del flusso, mentre questo non è necessario per entrambi i lati per i keep-alive TCP avviati.  Si applicano concetti simili anche per il livello dell'applicazione, tra cui le configurazioni client-server di database.  Controllare sul lato server quali opzioni esistono per i keepalives specifici dell'applicazione.

## <a name="next-steps"></a>Passaggi successivi
Microsoft è sempre alla ricerca di migliorare l'esperienza dei clienti. Se si verificano problemi di connettività in uscita non elencati o risolti da questo articolo, inviare commenti e suggerimenti tramite GitHub nella parte inferiore di questa pagina e il feedback verrà risolto non appena possibile.
