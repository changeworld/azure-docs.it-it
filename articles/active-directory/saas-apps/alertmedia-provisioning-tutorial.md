...
> * [Accesso Single Sign-On](./alertmedia-tutorial.md) a AlertMedia (scelta consigliata)
...
3. Determinare quali dati [mappare tra Azure AD e AlertMedia](../app-provisioning/customize-application-attributes.md). 
...
9. Esaminare gli attributi utente sincronizzati tra Azure AD e AlertMedia nella sezione **Mapping di attributi**. Gli attributi selezionati come proprietà **corrispondenti** vengono usati per trovare le corrispondenze con gli account utente in AlertMedia per le operazioni di aggiornamento. Se si sceglie di cambiare l'[attributo di destinazione corrispondente](../app-provisioning/customize-application-attributes.md), sarà necessario assicurarsi che l'API AlertMedia supporti il filtro degli utenti basato su tale attributo. Selezionare il pulsante **Salva** per eseguire il commit delle modifiche.
