---
title: 'Esercitazione: integrazione di Azure Active Directory Single Sign-On (SSO) con mindWireless | Microsoft Docs'
...
ms.locfileid: "92518943"
---
# <a name="tutorial-azure-active-directory-single-sign-on-sso-integration-with-mindwireless"></a>Esercitazione: integrazione di Azure Active Directory Single Sign-On (SSO) con mindWireless
...
1. Oltre quelli elencati in precedenza, l'applicazione mindWireless prevede il passaggio di altri attributi nella risposta SAML. Tali attributi sono indicati di seguito. Anche questi attributi vengono prepopolati, ma è possibile esaminarli in base ai requisiti.
    ...
    > [!NOTE]
    > Il nome dell'attestazione è sempre **ID dipendente** e il valore di cui è stato eseguito il mapping a **User. EmployeeID**, che contiene il valore EmployeeID dell'utente. In questo caso il mapping utente da Azure AD a mindWireless viene eseguito su EmployeeID, ma è possibile eseguirlo su un valore diverso anche in base alle impostazioni dell'applicazione. È possibile collaborare prima di tutto con il [team di supporto di mindWireless](mailto:sdulloor@mindwireless.com) per usare l'identificatore corretto di un utente ed eseguire il mapping di tale valore all'attestazione **ID dipendente**.
...
### <a name="create-mindwireless-test-user"></a>Creare un utente di test di mindWireless

In questa sezione viene creato un utente di nome B.Simon in mindWireless. Collaborare con il [team di supporto di mindWireless](mailto:sdulloor@mindwireless.com) per aggiungere gli utenti nella piattaforma mindWireless. Gli utenti devono essere creati e attivati prima di usare l'accesso Single Sign-On.

## <a name="test-sso"></a>Testare l'accesso SSO

In questa sezione viene testata la configurazione dell'accesso Single Sign-On di Azure AD usando il pannello di accesso.

Quando si fa clic sul riquadro di mindWireless nel pannello di accesso, si dovrebbe accedere automaticamente all'applicazione mindWireless per cui si è configurato l'accesso SSO. Per altre informazioni sul pannello di accesso, vedere [Introduzione al Pannello di accesso](../user-help/my-apps-portal-end-user-access.md).
