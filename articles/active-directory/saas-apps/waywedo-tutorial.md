...
1. Nella pagina **Configurazione SAML di base** immettere i valori per i campi seguenti:
    ...
    b. Nella casella di testo **Identificatore (ID entità)** digitare un URL nel formato seguente: `https://<SUBDOMAIN>.waywedo.com`
...
1. Nella pagina **Single sign-on setup** (Configurazione Single Sign-On) seguire questa procedura:
    ...
    1. Impostare **Turn on single sign-on** (Attiva Single Sign-On) su **Yes** (Sì) per abilitare l'accesso Single Sign-On.

    1. Nella casella di testo **Single sign-on name** (Nome Single Sign-On) immettere il proprio nome.

    1. Nella casella di testo **Entity ID** (ID entità) incollare il valore di **Identificatore Azure AD** copiato dal portale di Azure.

    1. Nella casella di testo **SAML SSO URL** (URL SSO SAML) incollare il valore di **URL di accesso** copiato dal portale di Azure.

    1. Caricare il certificato facendo clic sul pulsante **Select** (Seleziona) accanto a **Certificate** (Certificato).

    1. **Impostazioni facoltative** -
