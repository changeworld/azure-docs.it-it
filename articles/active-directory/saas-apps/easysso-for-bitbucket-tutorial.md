...
## <a name="configure-and-test-azure-ad-sso-for-easysso-for-bitbucket"></a>Configurare e testare l'accesso Single Sign-On di Azure AD per EasySSO for BitBucket
...
Per configurare e testare l'accesso SSO di Azure AD con EasySSO for BitBucket, seguire questa procedura:
...
1. Nella pagina di integrazione dell'applicazione **EasySSO for BitBucket** del portale di Azure individuare la sezione **Gestione**. e selezionare **Single Sign-On**.
...
1. Passare alla sezione **Administration** (Amministrazione).
...
* Fare clic su **Test this application** (Testa questa applicazione) nel portale di Azure. Verrà eseguito il reindirizzamento all'URL di accesso di EasySSO for BitBucket, in cui è possibile avviare il flusso di accesso.

* Passare direttamente all'URL di accesso di EasySSO for BitBucket e avviare il flusso di accesso da questa posizione.

#### <a name="idp-initiated"></a>Avviato da IDP:

* Dopo aver fatto clic su **Test this application** (Testa questa applicazione) nel portale di Azure, si dovrebbe accedere automaticamente all'istanza di EasySSO for BitBucket per cui si è configurato l'accesso SSO

È anche possibile usare App personali Microsoft per testare l'applicazione in qualsiasi modalità. Quando si fa clic sul riquadro di EasySSO for BitBucket in App personali, se è stato configurato in modalità SP, si dovrebbe essere reindirizzati alla pagina di accesso dell'applicazione per avviare il flusso di accesso; se invece è configurato in modalità IDP, si dovrebbe accedere automaticamente all'istanza di EasySSO for BitBucket per cui si è configurato l'accesso SSO. Per altre informazioni su App personali, vedere l'[introduzione ad App personali](../user-help/my-apps-portal-end-user-access.md).
