---
title: "Esercitazione: Integrazione dell'accesso Single Sign-On (SSO) di Azure Active Directory con EasySSO for Jira | Microsoft Docs"
description: Informazioni su come configurare l'accesso Single Sign-On tra Azure Active Directory e EasySSO for Jira.
...
Questa esercitazione descrive come integrare EasySSO for Jira con Azure Active Directory (Azure AD). Integrando EasySSO for Jira con Azure AD, è possibile:

* Controllare in Azure AD chi può accedere a Jira.
* Abilitare gli utenti per l'accesso automatico a Jira con gli account Azure AD personali.
...
* Sottoscrizione di EasySSO for Jira abilitata per l'accesso Single Sign-On (SSO).

## <a name="scenario-description"></a>Descrizione dello scenario

In questa esercitazione vengono eseguiti la configurazione e il test dell'accesso Single Sign-On di Azure AD in un ambiente di test.

* EasySSO for Jira supporta l'accesso SSO avviato da **SP e IDP**
...
## <a name="adding-easysso-for-jira-from-the-gallery"></a>Aggiunta di EasySSO for Jira dalla raccolta
...
1. **[Configurare l'accesso Single Sign-On di EasySSO for Jira](#configure-easysso-for-jira-sso)** : per configurare le impostazioni di Single Sign-On sul lato applicazione.
    1. **[Creare l'utente di test di EasySSO for Jira](#create-easysso-for-jira-test-user)** : per avere una controparte di B.Simon in EasySSO for Jira collegata alla rappresentazione dell'utente in Azure AD.
...
1. L'applicazione EasySSO for Jira prevede un formato specifico per le asserzioni SAML. È quindi necessario aggiungere mapping di attributi personalizzati alla configurazione degli attributi del token SAML. Lo screenshot seguente mostra l'elenco degli attributi predefiniti.

    ![image](common/default-attributes.png)

1. Oltre quelli elencati in precedenza, l'applicazione EasySSO for Jira prevede il passaggio di altri attributi nella risposta SAML. Tali attributi sono indicati di seguito. Anche questi attributi vengono prepopolati, ma è possibile esaminarli in base ai requisiti.
...
1. Nella sezione **Certificato di firma SAML** della pagina **Configura l'accesso Single Sign-On con SAML** fare clic sui collegamenti di **download** relativi alle opzioni **Certificato (Base64)** o **XML metadati federazione** e salvare una o entrambi nel computer. Tali dati saranno necessari in seguito per configurare EasySSO per Jira.
    ...
    ![Collegamento di download del certificato](media/easysso-for-jira-tutorial/azure-ad-SAML-certificate.png)
    
    Se si prevede di eseguire manualmente la configurazione di EasySSO per JIRA con il certificato, è anche necessario copiare il valore di **URL di accesso** e **Identificatore Azure AD** dalla sezione seguente e salvarli nel computer.
...
1. Accedere all'istanza di Atlassian Jira con privilegi di amministratore e passare alla sezione **Manage Apps** (Gestisci app). 
...
6. Fare clic sul pulsante **Save** (Salva) nella parte inferiore della pagina. Si noterà che il contenuto dei file dei metadati o di certificato viene analizzato nei campi di configurazione. La configurazione di EasySSO for Jira è stata completata.
...
### <a name="create-easysso-for-jira-test-user"></a>Creare l'utente di test di EasySSO for Jira

In questa sezione viene creato un utente di nome Britta Simon in Jira. EasySSO for Jira supporta il provisioning utenti JIT, che è **disabilitato** per impostazione predefinita. Per abilitare il provisioning utenti, è necessario selezionare in modo esplicito l'opzione **Create user on successful login** (Crea utente in caso di accesso riuscito) nella sezione General (Generale) della configurazione del plug-in EasySSO. Se non esiste già un utente in Jira, ne viene creato uno nuovo dopo l'autenticazione.

Se, però, non si vuole abilitare il provisioning utenti automatico al primo accesso dell'utente, gli utenti devono esistere già nelle directory utente back-end usate dall'istanza di Jira, ad esempio LDAP o Atlassian Crowd.

![Provisioning utenti](media/easysso-for-jira-tutorial/jira-admin-6.png)
...
Quando si fa clic sul riquadro di EasySSO for Jira nel pannello di accesso, si dovrebbe accedere automaticamente all'istanza di Jira per cui si è configurato l'accesso SSO. Per altre informazioni sul pannello di accesso, vedere [Introduzione al Pannello di accesso](../user-help/my-apps-portal-end-user-access.md).

### <a name="sp-initiated-workflow"></a>Flusso di lavoro avviato da SP

In questa sezione viene testata la configurazione dell'accesso Single Sign-On di Azure AD usando il pulsante **SAML Login** (Accesso SAML) di Jira.

![Accesso SAML utente](media/easysso-for-jira-tutorial/jira-admin-7.png)

Questo scenario presuppone che sia stata abilitata l'opzione **SAML Login Button** (Pulsante di accesso SAML) nella scheda **Look & Feel** (Aspetto) nella pagina di configurazione di EasySSO per Jira (vedere sopra). Aprire l'URL di accesso di Jira nella modalità in incognito del browser per evitare interferenze con le sessioni esistenti. Fare clic sul pulsante **SAML Login** (Accesso SAML). Si verrà reindirizzati al flusso di autenticazione utente di Azure AD. Al termine dell'operazione, si verrà reindirizzati di nuovo all'istanza di Jira come utente autenticato tramite SAML.
...
- [Provare EasySSO for Jira con Azure AD](https://aad.portal.azure.com/)

- [Informazioni sul controllo sessioni in Microsoft Cloud App Security](/cloud-app-security/proxy-intro-aad)

- [Come proteggere EasySSO for Jira con visibilità e controlli avanzati](/cloud-app-security/proxy-intro-aad)