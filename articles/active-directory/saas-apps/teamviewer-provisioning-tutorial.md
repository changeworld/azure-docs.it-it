...
Questa esercitazione descrive le procedure da eseguire sia in TeamViewer che in Azure Active Directory (Azure AD) per configurare il provisioning utenti automatico. Una volta configurato, Azure AD effettua automaticamente il provisioning e il deprovisioning di utenti e gruppi per [TeamViewer](https://www.teamviewer.com/buy-now/) usando il servizio di provisioning di Azure AD. Per informazioni dettagliate sul funzionamento di questo servizio e domande frequenti, vedere [Automatizzare il provisioning e il deprovisioning utenti in applicazioni SaaS con Azure Active Directory](../app-provisioning/user-provisioning.md). 


## <a name="capabilities-supported"></a>Funzionalità supportate
> [!div class="checklist"]
> * Creazione di utenti in TeamViewer
> * Rimozione di utenti da TeamViewer quando l'accesso non è più necessario
> * Mantenimento della sincronizzazione degli attributi utente tra Azure AD e TeamViewer
> * [Accesso Single Sign-On](./teamviewer-tutorial.md) a TeamViewer (scelta consigliata)
...
* Una [licenza Tensor](https://www.teamviewer.com/de/teamviewer-tensor/) valida per TeamViewer.
...
1. Accedere alla [console di gestione di TeamViewer](https://login.teamviewer.com). Passare a **Edit Profile** (Modifica profilo).

    ![Console di amministrazione di TeamViewer](./media/teamviewer-provisioning-tutorial/admin.png)

2.  Passare a **Apps** (App). Fare clic su **Create Script Token** (Crea token di script).

    ![Creazione del token in TeamViewer](./media/teamviewer-provisioning-tutorial/createtoken.png)

3.  Specificare un nome per il token di script. Fare clic sul pulsante **Save** (Salva).

    ![Nome del token di TeamViewer](./media/teamviewer-provisioning-tutorial/tokenname.png)

4. Copiare il valore di **Token** e fare clic su **OK**. Questo valore verrà immesso nel campo **Token segreto** dell'applicazione TeamViewer nel portale di Azure.

    ![Token di TeamViewer](./media/teamviewer-provisioning-tutorial/token.png)

## <a name="step-3-add-teamviewer-from-the-azure-ad-application-gallery"></a>Passaggio 3. Aggiungere TeamViewer dalla raccolta di applicazioni di Azure AD

Aggiungere TeamViewer dalla raccolta di applicazioni di Azure AD per iniziare a gestire il provisioning in TeamViewer. Se TeamViewer è stato configurato in precedenza per l'accesso Single Sign-On, è possibile usare la stessa applicazione. È tuttavia consigliabile creare un'app separata per il test iniziale dell'integrazione. Per altre informazioni su come aggiungere un'applicazione dalla raccolta, fare clic [qui](../manage-apps/add-application-portal.md). 
...
## <a name="step-5-configure-automatic-user-provisioning-to-teamviewer"></a>Passaggio 5. Configurare il provisioning utenti automatico per TeamViewer 
...
5. Nella sezione **Credenziali amministratore** immettere `ttps://webapi.teamviewer.com/scim/v2` nel campo **URL tenant** e immettere il token di script creato in precedenza nel campo **Token segreto**. Fare clic su **Test connessione** per verificare che Azure AD possa connettersi a TeamViewer. Se la connessione non riesce, verificare che l'account TeamViewer abbia autorizzazioni di amministratore e riprovare.

    ![Screenshot che mostra la finestra di dialogo Credenziali amministratore, in cui è possibile immettere l'URL del tenant e il token segreto.](./media/teamViewer-provisioning-tutorial/provisioning.png)
...
8. Nella sezione **Mapping** selezionare **Synchronize Azure Active Directory Users to TeamViewer** (Sincronizza utenti di Azure Active Directory in TeamViewer).

9. Esaminare gli attributi utente sincronizzati da Azure AD a TeamViewer nella sezione **Mapping attributi**. Gli attributi selezionati come proprietà **corrispondenti** vengono usati per l'abbinamento con gli account utente in TeamViewer per le operazioni di aggiornamento. Se si sceglie di modificare l'[attributo di destinazione corrispondente](../app-provisioning/customize-application-attributes.md), sarà necessario assicurarsi che l'API TeamViewer supporti il filtro degli utenti basato su tale attributo. Selezionare il pulsante **Salva** per eseguire il commit delle modifiche.
...
11. Per abilitare il servizio di provisioning di Azure AD per TeamViewer, impostare **Stato del provisioning** su **Sì** nella sezione **Impostazioni**.

    ![Stato del provisioning attivato](common/provisioning-toggle-on.png)

12. Definire gli utenti e/o i gruppi di cui si vuole effettuare il provisioning in TeamViewer selezionando i valori desiderati in **Ambito** nella sezione **Impostazioni**.
