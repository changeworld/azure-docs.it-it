...
> - [Single Sign-on](./travelperk-tutorial.md) per TravelPerk (scelta consigliata)
...
3. Determinare quali dati eseguire il [mapping tra Azure ad e TravelPerk](../app-provisioning/customize-application-attributes.md).
...
Aggiungere TravelPerk dalla raccolta di applicazioni Azure AD per iniziare a gestire il provisioning in TravelPerk. Se in precedenza è stato configurato TravelPerk per SSO, è possibile usare la stessa applicazione. È tuttavia consigliabile creare un'app separata per il test iniziale dell'integrazione. Per altre informazioni su come aggiungere un'applicazione dalla raccolta, fare clic [qui](../manage-apps/add-application-portal.md).
...
9. Esaminare gli attributi utente che vengono sincronizzati da Azure AD a TravelPerk nella sezione **mapping degli attributi** . Gli attributi selezionati come proprietà **corrispondenti** vengono usati per trovare le corrispondenze con gli account utente in TravelPerk per le operazioni di aggiornamento. Se si sceglie di modificare l' [attributo di destinazione corrispondente](../app-provisioning/customize-application-attributes.md), sarà necessario assicurarsi che l'API TravelPerk supporti il filtraggio degli utenti in base a tale attributo. Selezionare il pulsante **Salva** per eseguire il commit delle modifiche.
