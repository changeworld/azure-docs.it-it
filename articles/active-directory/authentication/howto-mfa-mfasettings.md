...
La funzionalità _indirizzi IP attendibili_ di Azure ad multi-factor authentication ignora i prompt di autenticazione a più fattori per gli utenti che effettuano l'accesso da un intervallo di indirizzi IP definito. È possibile impostare intervalli di indirizzi IP attendibili per gli ambienti locali, in modo che quando gli utenti si trovano in uno di questi percorsi, non viene Azure AD Multi-Factor Authentication prompt. La funzionalità _indirizzi IP attendibili_ di Azure ad Multi-Factor Authentication richiede Azure ad Premium edizione P1. 
...
>
> Gli intervalli IPv6 sono supportati solo nell'interfaccia [location denominata (anteprima)](../conditional-access/location-condition.md) .
