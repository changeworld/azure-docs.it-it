...
description: Panoramica dei metodi di autenticazione registrati e usati dall'organizzazione per accedere ed eseguire la reimpostazione della password.
...
Fare clic su **utenti in grado di multi-factor authentication di Azure**, **utenti in grado di eseguire l'autenticazione con password** o **utenti in grado di reimpostare la password self-service** o informazioni dettagliate per pre-filtrare un elenco di dettagli di registrazione dell'utente.

- **Gli utenti in grado di multi-factor authentication di Azure** mostrano la suddivisione degli utenti che supportano l'autenticazione a più fattori nel Azure ad. Gli utenti sono considerati idonei se sono entrambi registrati per un metodo di autenticazione avanzata e abilitati dal criterio per l'uso di tale metodo per eseguire l'autenticazione a più fattori. Questo numero non riflette gli utenti registrati per l'autenticazione a più fattori al di fuori della Azure AD. 
- **Gli utenti che supportano l'autenticazione senza password** mostrano la suddivisione degli utenti in grado di accedere senza una password. Sono inclusi gli utenti registrati per FIDO2, Windows Hello for business e l'accesso tramite telefono senza password con l'app Microsoft Authenticator. 
- **Gli utenti che supportano la reimpostazione della password self-service** mostrano la suddivisione degli utenti in grado di reimpostare la password self-service. Gli utenti sono considerati idonei per SSPR se sono entrambi registrati per un numero sufficiente di metodi per soddisfare i criteri SSPR di un'organizzazione e abilitati per l'esecuzione di SSPR. 

  ![Screenshot degli utenti in grado di registrare](media/how-to-authentication-methods-usage-insights/users-capable.png)

**Gli utenti registrati tramite il metodo di autenticazione** mostrano il numero di utenti registrati per ogni metodo di autenticazione. Fare clic su un metodo di autenticazione per vedere quali utenti sono registrati per il metodo. 

![Screenshot degli utenti registrati](media/how-to-authentication-methods-usage-insights/users-registered.png)

**Registrazione recente per metodo di autenticazione** Mostra il numero di registrazioni del metodo di autenticazione riuscite e non riuscite in base al metodo di autenticazione. Fare clic su un metodo di autenticazione per visualizzare gli eventi di registrazione recenti per il metodo.
...
Il report **sull'utilizzo** Mostra i metodi di autenticazione utilizzati dagli utenti per l'accesso e la reimpostazione delle password.

![Screenshot della pagina Usage](media/how-to-authentication-methods-usage-insights/usage-page.png)

**Accessi in base al requisito di autenticazione** indica il numero di accessi interattivi utente riusciti necessari per eseguire l'autenticazione a fattore singolo e a più fattori in Azure ad. Questo non riflette gli accessi in cui l'autenticazione a più fattori è stata applicata da un provider di autenticazione a più fattori di terze parti.
...
- Motivo dell'errore (spiegazione)
  ...
  ![Screenshot della pagina Usage](media/how-to-authentication-methods-usage-insights/registration-and-reset-logs.png)

## <a name="limitations"></a>Limitazioni

Le registrazioni del passaggio di accesso temporaneo (TAP) non vengono riflesse nella scheda registrazione del report perché sono valide solo per un breve periodo di tempo.
