---
title: Esempi di PowerShell e Graph per licenze di gruppo-Azure AD | Microsoft Docs
...
La funzionalità completa per le licenze basate sui gruppi è disponibile tramite il [portale di Azure](https://portal.azure.com)e attualmente sono disponibili alcune attività utili che possono essere eseguite usando i cmdlet e i Microsoft Graph di [PowerShell MSOnline](/powershell/module/msonline) esistenti. In questo documento sono riportati esempi di operazioni che si possono eseguire.
...
## <a name="assign-licenses-to-a-group"></a>Assegnare licenze a un gruppo

Usare l'esempio seguente per assegnare le licenze a un gruppo usando Microsoft Graph:
...
Il cmdlet [Get-MsolGroup](/powershell/module/msonline/get-msolgroup) può essere usato per recuperare l'oggetto gruppo e verificare la proprietà *Licenses*: indica tutte le licenze attualmente assegnate al gruppo.
...
## <a name="get-all-users-with-license-errors-in-the-entire-organization"></a>Ottenere tutti gli utenti con errori di licenza nell'intera organizzazione
...
> [!NOTE]
> Questo script enumera tutti gli utenti dell'organizzazione, che potrebbero non essere ottimali per le organizzazioni di grandi dimensioni.
...
Questo script esegue queste funzioni su ogni utente dell'organizzazione, usando l'ID SKU come input. in questo esempio si è interessati alla licenza per *Enterprise Mobility + Security*, che nell'organizzazione è rappresentata con ID *Contoso: EMS*:
...
Lo scopo di questo script è rimuovere le licenze dirette non necessarie dagli utenti che già ereditano la stessa licenza da un gruppo. ad esempio, come parte di una [transizione alle licenze basate sui gruppi](licensing-groups-migrate-users.md).
> [!NOTE]
