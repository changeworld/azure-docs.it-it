...
Proteggendo queste chiavi con il TPM, si migliora la sicurezza per PRT da attori malintenzionati che tentano di rubare le chiavi o riprodurre il PRT.  Pertanto, l'utilizzo di un TPM migliora significativamente la sicurezza dei Azure AD Uniti, Azure AD ibrido Uniti e Azure AD dispositivi registrati contro il furto di credenziali. Per garantire prestazioni e affidabilità, è consigliata la versione TPM 2.0 per tutti gli scenari di registrazione di dispositivi Azure AD in Windows 10. A partire dall'aggiornamento di Windows 10, 1903, Azure AD non usa TPM 1,2 per una delle chiavi precedenti a causa di problemi di affidabilità. 
...
| D | Il plug-in CloudAP crea il cookie del token di aggiornamento primario, effettua l'accesso con la chiave della sessione associata al TPM e invia di nuovo il cookie all'host client nativo. |
