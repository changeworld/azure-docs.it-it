---
title: Creare un'app daemon che chiama API Web | Azure
...
Un altro caso comune in cui le applicazioni non daemon usano le credenziali client: anche quando agiscono per conto degli utenti, devono accedere a un'API Web o a una risorsa con la propria identità per motivi tecnici. Un esempio è l'accesso ai segreti in Azure Key Vault o nel database SQL di Azure per una cache.
...
- Durante la [registrazione dell'applicazione](./scenario-daemon-app-registration.md), l'URI di risposta non è necessario. Condividere i segreti o i certificati o le asserzioni firmate con Azure AD. È anche necessario richiedere le autorizzazioni dell'applicazione e concedere il consenso dell'amministratore per usare tali autorizzazioni dell'app.
...
Passare all'articolo successivo in questo scenario, registrazione dell' [app](./scenario-daemon-app-registration.md).
