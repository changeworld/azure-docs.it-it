...
Nel nodo MSAL acquisire i token tramite il flusso del codice di autorizzazione con la chiave di prova per lo scambio di codice (PKCE). Il nodo MSAL usa una cache dei token in memoria per verificare se sono presenti account utente nella cache. Se è presente, l'oggetto account può essere passato al `acquireTokenSilent()` metodo per recuperare un token di accesso memorizzato nella cache.
...
Da [`SystemWebViewOptions`](/dotnet/api/microsoft.identity.client.systemwebviewoptions) in MSAL.NET 4.1 è possibile specificare:
...
Per altre informazioni su tutti gli altri parametri facoltativi per `AcquireTokenInteractive`, vedere [AcquireTokenInteractiveParameterBuilder](/dotnet/api/microsoft.identity.client.acquiretokeninteractiveparameterbuilder#methods).
...
Nel nodo MSAL acquisire i token tramite il flusso del codice di autorizzazione con la chiave di prova per lo scambio di codice (PKCE). Il processo prevede due passaggi: prima di tutto, l'applicazione ottiene un URL che può essere usato per generare un codice di autorizzazione. Questo URL può essere aperto in un browser preferito, in cui l'utente può immettere le proprie credenziali e verrà reindirizzato di nuovo al `redirectUri` (registrato durante la registrazione dell'app) con un codice di autorizzazione. In secondo luogo, l'applicazione passa il codice di autorizzazione ricevuto al `acquireTokenByCode()` metodo che lo scambia per un token di accesso.
...
Per l'elenco dei modificatori possibili in AcquireTokenByIntegratedWindowsAuthentication, vedere [AcquireTokenByIntegratedWindowsAuthParameterBuilder](/dotnet/api/microsoft.identity.client.acquiretokenbyintegratedwindowsauthparameterbuilder#methods).
...
Questo flusso non è ancora supportato nel nodo MSAL.
...
Per altre informazioni su tutti i modificatori che possono essere applicati a `AcquireTokenByUsernamePassword`, vedere [AcquireTokenByUsernamePasswordParameterBuilder](/dotnet/api/microsoft.identity.client.acquiretokenbyusernamepasswordparameterbuilder#methods).
...
Il codice di esempio seguente illustra la sinossia dei casi più recenti, con spiegazioni del tipo di eccezioni che è possibile ottenere e della relativa mitigazione. Per un esempio di codice completamente funzionante, vedere [Active-Directory-dotnetcore-devicecodeflow-V2](https://github.com/azure-samples/active-directory-dotnetcore-devicecodeflow-v2) su GitHub.
...
Passare all'articolo successivo in questo scenario, [chiamare un'API Web dall'app desktop](scenario-desktop-call-api.md).
