...
description: Scenari e linee guida per gli sviluppatori per l'accesso condizionale di Azure AD e la piattaforma di identità di Microsoft.
...
* [Autenticazione a più fattori](../authentication/concept-mfa-howitworks.md)
...
Per altre informazioni sulle funzionalità di accesso condizionale complete, vedere l'articolo [Cos'è l'accesso condizionale](../conditional-access/overview.md).
...
In particolare, tutti gli ambiti di Microsoft Graph rappresentano alcuni set di dati a cui possono essere applicati i criteri singolarmente. Poiché ai criteri di accesso condizionale vengono assegnati set di dati specifici, Azure AD imporrà i criteri di accesso condizionale in base ai dati sottostanti a Graph, anziché a Graph stesso.
...
Quando l'app tenta di accedere a un servizio con criteri di accesso condizionale, potrebbe riscontrare una richiesta di accesso condizionale. La richiesta è codificata nel parametro `claims` fornito in una risposta da Azure AD. Ecco un esempio del parametro della richiesta:
...
## <a name="scenario-single-page-app-spa-using-msaljs"></a>Scenario: App a pagina singola (SPA) che usa MSAL.js

In questo scenario viene illustrato il caso di un'app a pagina singola (SPA) che usa MSAL.js per chiamare un'API Web protetta con accesso condizionale. Si tratta di un'architettura semplice, ma ci sono alcuni aspetti da prendere in considerazione in caso di sviluppo incentrato sull'accesso condizionale.

In MSAL.js ci sono poche funzioni che ottengono token: `loginPopup()`, `acquireTokenSilent(...)`, `acquireTokenPopup(…)` e `acquireTokenRedirect(…)`.

* `loginPopup()` ottiene un token ID tramite una richiesta di accesso interattiva, ma non ottiene token di accesso per alcun servizio (inclusa un'API Web protetta con accesso condizionale).
...
![Diagramma di flusso per le app a pagina singola che usano MSAL](./media/v2-conditional-access-dev-guide/spa-using-msal-scenario.png)

Verrà ora esaminato un esempio con lo scenario di accesso condizionale. L'utente finale è appena arrivato nel sito e non ha una sessione. Si esegue una chiamata a `loginPopup()` e si ottiene un token ID senza autenticazione a più fattori. L'utente preme quindi un pulsante che comporta una richiesta di dati dall'app a un'API Web. L'app prova a eseguire una chiamata a `acquireTokenSilent()`, che ha però esito negativo perché l'utente non ha eseguito ancora l'autenticazione a più fattori e deve rispettare i criteri di accesso condizionale.
