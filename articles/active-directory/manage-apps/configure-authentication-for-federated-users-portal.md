...
description: Informazioni su come configurare i criteri di individuazione dell'area di autenticazione principale per Azure Active Directory per gli utenti federati, inclusi l'accelerazione automatica e i suggerimenti per il dominio.
...
Esistono tre modi per controllare l'accelerazione automatica per un Provider di identità federato:

- Usare un hint [di dominio nelle](#domain-hints) richieste di autenticazione per un'applicazione.
...
- Configurare un criterio di individuazione dell'area di autenticazione principale per ignorare [i suggerimenti](prevent-domain-hints-with-home-realm-discovery.md) di dominio da applicazioni specifiche o per determinati domini.
...
Per impostazione predefinita, Azure AD tenta di reindirizzare l'accesso al provider  di identità configurato per un dominio se si verificano entrambe le condizioni seguenti:
...
>[!NOTE]
>Se un hint di dominio è [](#home-realm-discovery-policy-to-prevent-auto-acceleration)incluso in una richiesta di autenticazione e deve essere rispettato, la relativa presenza sostituisce l'accelerazione automatica impostata per l'applicazione nei criteri HRD.
...
Per le applicazioni, è consigliabile usare le librerie di Azure AD e l'accesso interattivo per autenticare gli utenti. Le librerie si occupano dei flussi dell'utente federato.  A volte le applicazioni legacy, in particolare quelle che usano le concessioni ROPC, inviano nome utente e password direttamente Azure AD e non vengono scritte per comprendere la federazione. Queste non eseguono l'individuazione dell'area di autenticazione e non interagiscono con il corretto endpoint federato per autenticare un utente. Se si sceglie di , è possibile usare Criteri HRD per abilitare applicazioni legacy specifiche che inviano credenziali nome utente/password usando la concessione ROPC per l'autenticazione direttamente con Azure Active Directory. La sincronizzazione degli hash delle password deve essere abilitata.
...
Il tipo di criterio è "[HomeRealmDiscoveryPolicy](/graph/api/resources/homeRealmDiscoveryPolicy)".
...
**PreferredDomain** è facoltativo. **PreferredDomain** deve indicare un dominio al quale accelerare. Può essere omesso se il tenant dispone di un solo dominio federato.  Se è omesso ed esiste più di un dominio federato verificato, i criteri non hanno effetto.
 ...
 Se **PreferredDomain** è specificato, deve corrispondere a un dominio federato verificato per il tenant. Tutti gli utenti dell'applicazione devono essere in grado di accedere a tale dominio. Gli utenti che non possono accedere al dominio federato saranno bloccati e non potranno completare l'accesso.
...
- **DomainHintPolicy è** un oggetto complesso facoltativo che impedisce agli hint di dominio di [  accelerare automaticamente gli utenti nei domini federati.](prevent-domain-hints-with-home-realm-discovery.md) Questa impostazione a livello di tenant viene usata per garantire che le applicazioni che inviano hint di dominio non impediti agli utenti di accedere con credenziali gestite dal cloud.

### <a name="priority-and-evaluation-of-hrd-policies"></a>Priorità e valutazione dei criteri HRD

È possibile creare criteri HRD e assegnarli a specifiche organizzazioni e entità servizio. Ciò significa che è possibile che più criteri si applichino a un'applicazione specifica, quindi Azure AD deve decidere quale ha la precedenza. Un set di regole decide quali criteri HRD (di molte applicazioni) vengono applicati:

- Se nella richiesta di autenticazione è presente un hint di dominio, viene controllato il criterio HRD per il tenant (il criterio impostato come predefinito del tenant) per verificare se gli hint di dominio devono [essere ignorati.](prevent-domain-hints-with-home-realm-discovery.md) Se sono consentiti hint di dominio, viene usato il comportamento specificato dall'hint di dominio.
...
- Per altre informazioni sull'accesso Single Sign-On dell'utente, vedere [Single Sign-On alle](what-is-single-sign-on.md)applicazioni in Azure Active Directory .
