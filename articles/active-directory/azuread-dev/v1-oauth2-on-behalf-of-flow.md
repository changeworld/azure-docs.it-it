---
title: Autenticazione da servizio a servizio con il flusso On-Behalf-Of di OAuth 2.0 | Microsoft Docs
...
Questi passaggi costituiscono il flusso On-Behalf-Of: ![Passaggi del flusso On-Behalf-Of di OAuth 2.0](./media/v1-oauth2-on-behalf-of-flow/active-directory-protocols-oauth-on-behalf-of-flow.png)
...
La richiesta HTTP POST seguente richiede un token di accesso per l'API Web https://graph.microsoft.com. `client_id` identifica il servizio che richiede il token di accesso.
...
La richiesta HTTP POST seguente richiede un token di accesso per l'API Web https://graph.microsoft.com con un certificato. `client_id` identifica il servizio che richiede il token di accesso.
