...
Quando il diritto keychain è abilitato in ogni applicazione e si è pronti per usare SSO, indicare a Identity SDK il keychain usando l'impostazione seguente in `ADAuthenticationSettings` con l'impostazione seguente:
...
Questo è tutto. L'SDK condividerà le credenziali tra tutte le applicazioni. Anche l'elenco degli utenti verrà condiviso tra le istanze dell'applicazione.
...
ADAL usa –canOpenURL: per controllare se il broker è installato nel dispositivo. In iOS 9 Apple ha bloccato gli schemi di cui un'applicazione può effettuare una query. Sarà necessario aggiungere "msauth" alla sezione LSApplicationQueriesSchemes del `info.plist file` .
