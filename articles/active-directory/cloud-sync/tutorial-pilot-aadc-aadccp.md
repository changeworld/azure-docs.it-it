---
title: Esercitazione-pilota Azure AD Connect Cloud Sync per una foresta di Active Directory sincronizzata esistente
description: Informazioni su come pilotare la sincronizzazione cloud per una foresta di Active Directory di test già sincronizzata con la sincronizzazione della connessione Azure Active Directory (Azure AD).
...
3. Assicurarsi che gli oggetti nell'ambito pilota dispongano di MS-DS-consistencyGUID popolato in modo che la sincronizzazione cloud corrisponda a quelle degli oggetti. 
...
## <a name="create-custom-user-inbound-rule"></a>Creare una regola utente in ingresso personalizzata
 ...
 ![Menu dell'editor delle regole di sincronizzazione](media/tutorial-migrate-aadc-aadccp/user-8.png)</br>
 
 2. Selezionare **In ingresso** nell'elenco a discesa Direzione e fare clic su **Aggiungi nuova regola**.
 ![Screenshot che mostra la finestra per la visualizzazione e la gestione delle regole di sincronizzazione, con l'opzione "In ingresso" e il pulsante per l'aggiunta di una nuova regola selezionati.](media/tutorial-migrate-aadc-aadccp/user-1.png)</br>
 ...
 3. Nella pagina **Descrizione** immettere quanto segue e fare clic su **Avanti**:
    ...
    **Tag**: lasciare vuoto questo campo<br>
    ![Screenshot che mostra la pagina relativa alla descrizione nella finestra per la creazione di una regola di sincronizzazione in ingresso, con i valori specificati.](media/tutorial-migrate-aadc-aadccp/user-2.png)</br>
 ...
 4. Nella pagina **Scoping filter** (Filtro di ambito) immettere l'unità organizzativa o il gruppo di sicurezza in base a cui creare il progetto pilota.  Per filtrare in base a unità organizzativa, aggiungere la parte OU del nome distinto. Questa regola verrà applicata a tutti gli utenti inclusi in tale unità organizzativa.  Quindi, se il nome distinto termina con "OU=CPUsers,DC=contoso,DC=com, aggiungere questo filtro.  Quindi fare clic su **Next**. 
    ...
    ![Screenshot che mostra la pagina relativa al filtro dell'ambito nella finestra per la creazione di una regola di sincronizzazione in ingresso, con un valore di filtro dell'ambito specificato.](media/tutorial-migrate-aadc-aadccp/user-3.png)</br>
 ...
 ![Screenshot che mostra la pagina relativa alle trasformazioni nella finestra per la creazione di una regola di sincronizzazione in ingresso, con un flusso di trasformazione costante aggiunto.](media/tutorial-migrate-aadc-aadccp/user-4.png)</br>
...
## <a name="create-custom-user-outbound-rule"></a>Creare una regola utente in uscita personalizzata
 ...
 ![Screenshot che mostra la direzione "In uscita" selezionata e il pulsante per l'aggiunta di una nuova regola evidenziato.](media/tutorial-migrate-aadc-aadccp/user-5.png)</br>
 ...
 2. Nella pagina **Descrizione** immettere quanto segue e fare clic su **Avanti**:
    ...
    **Tag**: lasciare vuoto questo campo<br>
    ...
    ![Screenshot che mostra la pagina relativa alla descrizione con le proprietà specificate.](media/tutorial-migrate-aadc-aadccp/user-6.png)</br>
 
 3. Nella pagina **Scoping filter** (Filtro di ambito) scegliere **cloudNoFlow** uguale **True**. Quindi fare clic su **Next**.
 ![Regola personalizzata](media/tutorial-migrate-aadc-aadccp/user-7.png)</br>
...
3. Eseguire la sincronizzazione Cloud Azure AD Connect (AADConnectProvisioningAgent. Installer)
...
Per configurare il provisioning, seguire questa procedura:
 ...
 4. Per la schermata di **gestione della sincronizzazione cloud** 
  ![ , vedere il collegamento "Gestisci sincronizzazione cloud".](media/how-to-configure/manage-1.png)</br>
 5.  Fare clic sullo screenshot **nuova configurazione** 
  ![ della schermata Azure ad Connect sincronizzazione cloud con il collegamento "nuova configurazione" evidenziato.](media/tutorial-single-forest/configure-1.png)</br>
 ...
 ![Screenshot della schermata di configurazione con il messaggio di posta elettronica di notifica compilato e l'opzione Abilita selezionata.](media/tutorial-single-forest/configure-2.png)</br>
 7. In **Configura** selezionare **Tutti gli utenti** per cambiare l'ambito della regola di configurazione.
 ![Screenshot della schermata di configurazione con l'opzione "Tutti gli utenti" evidenziata accanto a "Definisci l'ambito per gli utenti".](media/how-to-configure/scope-2.png)</br>
 8. A destra cambiare l'ambito per includere la specifica unità organizzativa creata, "OU=CPUsers,DC=contoso,DC=com".
 ![Screenshot della schermata Definisci l'ambito per gli utenti in cui è evidenziato l'ambito modificato e impostato sull'unità organizzativa creata.](media/tutorial-existing-forest/scope-2.png)</br>
 9.  Fare clic su **Fine** e su **Salva**.
 10. L'ambito dovrebbe ora essere impostato su una unità organizzativa. 
 ![Screenshot della schermata di configurazione con l'opzione "1 unità organizzativa" evidenziata accanto a "Definisci l'ambito per gli utenti".](media/tutorial-existing-forest/scope-3.png)</br>
 

## <a name="verify-users-are-provisioned-by-cloud-sync"></a>Verificare che gli utenti siano sottoposti a provisioning da Cloud Sync
A questo punto verificare che gli utenti presenti nella directory locale siano stati sincronizzati e che ora siano inclusi nel tenant di Azure AD.  Per essere completata, questa operazione può richiedere alcune ore.  Per verificare che gli utenti eseguono il provisioning tramite sincronizzazione cloud, attenersi alla procedura seguente:
...
3. Fare clic su **Azure AD Connect**
4. Fare clic su **Gestisci sincronizzazione cloud**
5. Fare clic sul pulsante **Log**
6. Cercare un nome utente per confermare che l'utente è sottoposta a provisioning da Cloud Sync
