...
description: La linea di base di sicurezza delle istanze di contenitore fornisce linee guida procedurali e risorse per l'implementazione delle raccomandazioni di sicurezza specificate nel benchmark di sicurezza di Azure.
author: msmbaldwin
...
ms.locfileid: "106078606"
---
# <a name="azure-security-baseline-for-container-instances"></a>Baseline della sicurezza di Azure per le istanze di contenitore

Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1.0](../security/benchmarks/overview-v1.md) alle istanze di contenitore. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure. Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili alle istanze di contenitore. I **controlli** non applicabili alle istanze di contenitore o per i quali la responsabilità è Microsoft sono stati esclusi.

Per informazioni sul modo in cui le istanze del contenitore sono completamente mappate al benchmark di sicurezza di Azure, vedere il [file di mapping della linea di base di sicurezza](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)per le istanze
...
**Linee guida**: integrare i gruppi di contenitori in istanze di contenitore di Azure con una rete virtuale di Azure.  Le reti virtuali di Azure consentono di inserire molte delle risorse di Azure, ad esempio i gruppi di contenitori, in una rete instradabile non Internet.
...
**Linee guida**: quando si usa container Registry di Azure con istanze di contenitore di Azure, si consiglia di definire e implementare configurazioni di sicurezza standard per le risorse di rete associate al registro contenitori di Azure.

Usare gli alias di criteri di Azure negli spazi dei nomi **Microsoft. ContainerRegistry** e **Microsoft. Network** per creare criteri personalizzati per controllare o applicare la configurazione di rete dei registri di contenitori.

È possibile usare i progetti di Azure per semplificare le distribuzioni di Azure su larga scala mediante la creazione di pacchetti di elementi chiave dell'ambiente, ad esempio modelli di Azure Resource Manager, controlli RBAC di Azure e definizioni di criteri in una singola definizione di progetto. Applica facilmente il progetto a nuove sottoscrizioni e ottimizza il controllo e la gestione tramite il controllo delle versioni.
...
**Linee guida**: monitoraggio di Azure raccoglie i log delle risorse (in precedenza denominati log di diagnostica) per gli eventi guidati dall'utente. Raccogliere e usare questi dati per controllare gli eventi di autenticazione del contenitore e fornire una traccia completa delle attività sugli elementi come gli eventi pull e push, in modo da poter diagnosticare i problemi di sicurezza con il gruppo di contenitori.
...
**Linee guida**: se si usa un registro privato basato su cloud come registro contenitori di Azure con istanze di contenitore di Azure, usare l'area di lavoro di Azure log Analytics per il monitoraggio e l'invio di avvisi sulle attività anomale nei registri di sicurezza e negli eventi correlati al registro contenitori di Azure.
...
**Indicazioni**: Non applicabile. Se si usa un registro privato basato su cloud come il registro contenitori di Azure con istanze di contenitore di Azure, il registro contenitori di Azure non elabora o produce log correlati anti-malware.
...
**Indicazioni**: Non applicabile. Se si usa un registro privato basato su cloud come registro contenitori di Azure con istanze di contenitore di Azure, il registro contenitori di Azure è un endpoint e non avvia la comunicazione e il servizio non esegue query sul DNS.
...
Se si usa un registro privato basato su cloud come registro contenitori di Azure con istanze di contenitore di Azure, creare procedure per abilitare l'account amministratore predefinito di un registro contenitori. Disabilitare l'account quando non è in uso.
...
Se si usa un registro privato basato su cloud come il registro contenitori di Azure con istanze di contenitore di Azure, per l'accesso singolo al registro contenitori, usare il singolo segno non integrato con Azure AD.
...
**Linee guida**: è possibile accedere alle origini dei registri eventi per l'attività di accesso, il controllo e il rischio di Azure Active Directory (Azure ad), che consentono di eseguire l'integrazione con qualsiasi strumento/Monitoring di gestione di informazioni ed eventi di sicurezza (Siem).
...
- [Bloccare un'immagine del contenitore in un registro contenitori di Azure](../container-registry/container-registry-image-lock.md)
...
- [Limitare l'accesso a un registro contenitori di Azure usando una rete virtuale di Azure o regole del firewall](../container-registry/container-registry-vnet.md)
...
**Linee guida**: se si usa un registro privato basato su cloud come il registro contenitori di Azure con istanze di contenitore di Azure, l'identificazione dei dati, la classificazione e le funzionalità di prevenzione della perdita non sono ancora disponibili per Azure container Registry. Implementare una soluzione di terze parti, se necessaria ai fini della conformità.
...
**Linee guida**: se si usa un registro privato basato su cloud come Azure container Registry con istanze di contenitore di Azure, usare il controllo degli accessi in base al ruolo di Azure (RBAC di Azure) per gestire l'accesso ai dati e alle risorse in un registro contenitori di Azure.
...
**Linee guida**: usare la crittografia inattiva in tutte le risorse di Azure. Se si usa un registro privato basato su cloud come registro contenitori di Azure con istanze di contenitore di Azure, per impostazione predefinita tutti i dati in un registro contenitori di Azure vengono crittografati a riposo usando chiavi gestite da Microsoft.
...
**Linee guida**: è possibile usare una soluzione di terze parti per applicare patch alle immagini dell'applicazione. Inoltre, se si usa un registro privato basato su cloud come registro contenitori di Azure con istanze di contenitore di Azure, è possibile eseguire le attività di Azure Container Registry per automatizzare gli aggiornamenti alle immagini dell'applicazione in un registro contenitori in base alle patch di sicurezza o ad altri aggiornamenti nelle immagini di base.
...
**Linee guida**: se si usa un registro privato basato su cloud come registro contenitori di Azure con istanze di contenitore di Azure, integrare Azure container Registry (ACR) con il Centro sicurezza di Azure per consentire l'analisi periodica delle immagini dei contenitori per le vulnerabilità e per classificare i rischi. Facoltativamente, Distribuisci soluzioni di terze parti da Azure Marketplace per eseguire analisi periodiche delle vulnerabilità delle immagini e classificazione dei rischi.
...
**Linee guida**: se si usa un registro privato basato su cloud come Azure container Registry (ACR) con istanze di contenitore di Azure, ACR gestisce i metadati, inclusi i tag e i manifesti per le immagini in un registro. Seguire le procedure consigliate per l'assegnazione di tag agli artefatti.

- [Informazioni sui registri, i repository e le immagini](../container-registry/container-registry-concepts.md)
...
**Linee guida**: se si usa un registro privato basato su cloud come Azure container Registry (ACR) con istanze di contenitore di Azure, analizzare e monitorare i log di Azure container Registry per un comportamento anomalo ed esaminare regolarmente i risultati. Usare l'area di lavoro Log Analytics di monitoraggio di Azure per esaminare i log ed eseguire query sui dati di log.
...
- [Ad esempio, come controllare l'esecuzione di script di PowerShell negli ambienti Windows](/powershell/module/microsoft.powershell.security/set-executionpolicy)
...
Se si usa un registro privato basato su cloud come Azure Container Registry (ACR) con istanze di contenitore di Azure, controllare la conformità dei registri contenitori di Azure usando criteri di Azure:.
...
**Linee guida**: non applicabile; Questo controllo è valido solo per le risorse di calcolo.
...
Applicare criteri di Azure per limitare il tipo di risorse che è possibile creare nelle sottoscrizioni.
...
- [Come eseguire l'autenticazione a Key Vault](container-instances-managed-identity.md)
...
- [Come crittografare i dati con istanze di contenitore](container-instances-encrypt-data.md)
...
**Linee guida**: Microsoft antimalware è abilitato nell'host sottostante che supporta i servizi di Azure, ad esempio istanze di contenitore di Azure, ma non viene eseguito sui dati dei clienti.
...
**Linee guida**: se si usa un registro privato basato su cloud come Azure container Registry (ACR) con istanze di contenitore di Azure, i dati nel registro di Microsoft Azure contenitore vengono sempre replicati automaticamente per assicurare durabilità e disponibilità elevata. Azure Container Registry copia i dati in modo che siano protetti da eventi pianificati e non pianificati.
...
- [Come ripristinare chiavi di Azure Key Vault in Azure](/powershell/module/az.keyvault/restore-azkeyvaultkey)
