...
1. Nel processo di Analisi di flusso selezionare **Input** sotto **Topologia processo**. Fare clic su **Aggiungi input di riferimento** e scegliere **Database SQL**.
   ...
   ![Gli input sono selezionati nel riquadro di spostamento a sinistra. In input, + Aggiungi input di riferimento è selezionato, mostrando un elenco a discesa che mostra i valori archiviazione BLOB e database SQL.](./media/sql-reference-data/stream-analytics-inputs.png)
...
2. Compilare le configurazioni di input di Analisi di flusso. Scegliere il nome del database, il nome del server, il nome utente e la password. Se si vuole che l'input dei dati di riferimento venga aggiornato periodicamente, scegliere "Attivato" per specificare la frequenza di aggiornamento in GG:HH:MM. Se si hanno grandi set di dati con una frequenza di aggiornamento veloce, è possibile usare una [query delta](sql-reference-data.md#delta-query).
   ...
   ![Quando si seleziona database SQL, viene visualizzata la pagina nuovo input del database SQL. Nel riquadro sinistro è presente un modulo di configurazione e una query snapshot nel riquadro di destra.](./media/sql-reference-data/sql-input-config.png)
...
Passare a **Impostazioni account di archiviazione** sotto **Configura** e selezionare **Aggiungi account di archiviazione**.
   ...
   ![Le impostazioni dell'account di archiviazione sono selezionate nel riquadro sinistro. Nel riquadro di destra è disponibile un pulsante Aggiungi account di archiviazione.](./media/sql-reference-data/storage-account-settings.png)
...
3. Inserire i valori appropriati per il progetto in **Nome**, **Percorso** e **Nome soluzione** e scegliere **OK**.
   ...
   ![Il modello di analisi di flusso è selezionato, l'applicazione analisi di flusso di Azure è selezionata e le caselle nome, percorso e nome soluzione sono evidenziate.](./media/sql-reference-data/stream-analytics-vs-new-project.png)

### <a name="define-sql-database-reference-data-input"></a>Definire l'input dei dati di riferimento del database SQL

1. Creare un nuovo input.

   ![In Aggiungi nuovo elemento è selezionato input.](./media/sql-reference-data/stream-analytics-vs-input.png)
...
3. Compilare **Stream Analytics Input Configuration** (Configurazione input Analisi di flusso). Scegliere nome del database, nome del server, tipo di aggiornamento e frequenza di aggiornamento. Specificare la frequenza di aggiornamento nel formato `DD:HH:MM`.
   ...
   ![Nella configurazione di input di analisi di flusso, i valori vengono immessi o selezionati dagli elenchi a discesa.](./media/sql-reference-data/stream-analytics-vs-input-config.png)
   ...
   ![Viene evidenziato il codebehind SQL file Chemicals. snapshot. SQL.](./media/sql-reference-data/once-or-periodically-codebehind.png)
   ...
   ![Vengono evidenziati i file codebehind SQL Chemicals. Delta. SQL e Chemicals. snapshot. SQL.](./media/sql-reference-data/periodically-delta-codebehind.png)
...
Aprire **JobConfig.json** per specificare l'account di archiviazione in cui archiviare gli snapshot di riferimento SQL.
   ...
   ![La configurazione del processo di analisi di flusso viene visualizzata con i valori predefiniti. Le impostazioni di archiviazione globali sono evidenziate.](./media/sql-reference-data/stream-analytics-job-config.png)
...
1. Configurare l'input dei dati di riferimento SQL.
   ...
   ![Un editor di Visual Studio Code (scheda) Mostra ReferenceSQLDatabase.jssu.](./media/sql-reference-data/configure-sql-reference-data-input.png)

2. Selezionare l'icona SQL Server e fare clic su **Aggiungi connessione**.
   
   ![+ Aggiungi connessione viene visualizzato nel riquadro sinistro ed è evidenziato.](./media/sql-reference-data/add-sql-connection.png)

3. Immettere le informazioni di connessione.
   
   ![Vengono evidenziate le due caselle per le informazioni su database e server.](./media/sql-reference-data/fill-connection-information.png)

4. Fare clic con il pulsante destro del mouse in SQL di riferimento e scegliere **Esegui query**.
   
   ![L'esecuzione della query viene evidenziata nel menu di scelta rapida.](./media/sql-reference-data/execute-query.png)

5. Scegliere la connessione.
   
   ![Nella finestra di dialogo viene visualizzato "crea un profilo di connessione dall'elenco sottostante" e l'elenco include una voce, ovvero noterete.](./media/sql-reference-data/choose-connection.png)

6. Esaminare e verificare il risultato della query.
   
   ![I risultati della ricerca di query si trovano in una scheda Editor VS Code.](./media/sql-reference-data/verify-result.png)
