...
1. Il processo è ottimizzato per avere una [topologia parallela](./stream-analytics-parallelization.md). Se è possibile modificare la scala del processo mentre è in esecuzione, il processo ha una topologia parallela e può essere configurato per la scalabilità automatica.
2. [Creare un account di automazione di Azure](../automation/automation-create-standalone-account.md) con l'opzione "RunAsAccount" abilitata. Questo account deve avere le autorizzazioni per gestire i processi di analisi di flusso.
