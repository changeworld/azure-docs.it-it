---
title: Flusso Widevine Android offline
description: Questo argomento illustra come configurare l'account di servizi multimediali di Azure V3 per lo streaming offline di contenuto protetto da Widevine.
...
Per abilitare la modalità **offline** per le licenze Widevine, è necessario configurare il [modello di licenza Widevine](drm-widevine-license-template-concept.md). Nell'oggetto **policy_overrides** impostare la proprietà **can_persist** su **true** come illustrato in [ConfigureWidevineLicenseTempate](https://github.com/Azure-Samples/media-services-v3-dotnet-tutorials/blob/master/AMSV3Tutorials/EncryptWithDRM/Program.cs#L563). Il valore predefinito è false. 
...
Per alcuni dispositivi Android meno recenti, è necessario impostare i valori per le proprietà **policy_overrides** seguenti, definite nel [modello di licenza Widevine](drm-widevine-license-template-concept.md): **rental_duration_seconds**, **playback_duration_seconds** e **license_duration_seconds**. In alternativa, è possibile impostare queste proprietà su zero, ovvero su una durata infinita o illimitata.  
...
Per ulteriori informazioni, vedere [Widevine nella raccolta questions](questions-collection.md#widevine-streaming-for-android).
