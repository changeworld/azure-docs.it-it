...
La rete CDN memorizza nella cache il contenuto trasmesso da un endpoint di streaming di servizi multimediali [(Origin)](stream-streaming-endpoint-concept.md) per codec, per protocollo di streaming, per velocità in bit, per ogni formato di contenitore e per crittografia/DRM. Per ogni combinazione di codec-Streaming Protocol-Container Format-bitrate-Encryption, sarà presente una cache della rete CDN separata.
...
- L' [endpoint di streaming](stream-streaming-endpoint-concept.md) `hostname` e l'URL di streaming rimangono invariati, indipendentemente dal fatto che la rete CDN sia abilitata.
...
* Assicurarsi di esaminare il documento dell' [endpoint di streaming (Origin)](stream-streaming-endpoint-concept.md) .
