...
![Flusso di lavoro per la codifica su richiesta con creazione dinamica dei pacchetti](./media/encode-dynamic-packaging-concept/media-services-dynamic-packaging.svg)
...
> [!TIP]
> Se si vuole eseguire lo streaming di un sublip del video senza ricodificare il video, è consigliabile usare i [manifesti di pre-filtro con Dynamic Packager](filters-dynamic-manifest-concept.md).
...
- **EncoderNamedPreset. AdaptiveStreaming** (scelta consigliata): supporta la codifica a bitrate adattivo H. 264. Per altre informazioni, vedere [Codificare con una tabella di coppie velocità in bit-risoluzione generata automaticamente](encode-autogen-bitrate-ladder.md).
