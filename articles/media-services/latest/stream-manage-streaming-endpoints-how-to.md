...
Quando viene creato l'account di servizi multimediali, all'account viene aggiunto un [endpoint di streaming](stream-streaming-endpoint-concept.md) **predefinito** con stato **interrotto** . Per avviare lo streaming del contenuto e sfruttare i vantaggi della creazione [dinamica dei pacchetti](encode-dynamic-packaging-concept.md) e della [crittografia dinamica](drm-content-protection-concept.md), l'endpoint di streaming da cui si vuole trasmettere il contenuto deve essere nello stato in **esecuzione** .
...
Vedere l' [esempio di codice .NET](https://github.com/Azure-Samples/media-services-v3-dotnet/blob/main/Streaming/StreamHLSAndDASH/Program.cs#L112)completo.
