...
* Le intestazioni con nomi contenenti un valore diverso da lettere, cifre, trattini e caratteri di sottolineatura non vengono passate all'applicazione. Questo vale solo per i nomi di intestazione e non per i valori di intestazione. Si tratta di una modifica sostanziale rispetto alla versione V1.
...
1. Eseguire `Get-Help AzureAppGWMigration.ps1` per esaminare i parametri obbligatori:
   ...
   * **trustedRootCertificates: [PSApplicationGatewayTrustedRootCertificate]: facoltativo**. Elenco delimitato da virgole di oggetti PSApplicationGatewayTrustedRootCertificate creati per rappresentare i [certificati radice attendibili](ssl-overview.md) per l'autenticazione delle istanze di back-end dal gateway V2.
      ...
      Per creare un elenco di oggetti PSApplicationGatewayTrustedRootCertificate, vedere [New-AzApplicationGatewayTrustedRootCertificate](/powershell/module/Az.Network/New-AzApplicationGatewayTrustedRootCertificate).
