...
In questo esempio, il set di riscrittura verrebbe associato immediatamente a una regola di routing di base. Nel caso di una regola di routing basata sul percorso, l'associazione non sarà abilitata per impostazione predefinita. Il set di riscrittura può essere abilitato tramite il controllo dei percorsi in cui deve essere applicato tramite il portale o fornendo una configurazione di mapping del percorso URL che specifica il RewriteRuleSet rispetto a ogni opzione del percorso.  
