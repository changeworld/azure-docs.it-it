...
Creare una risorsa ExportPipeline per il registro contenitori di origine usando la Azure Resource Manager modello.
...
Usare lo strumento AzCopy o altri metodi per [trasferire](../storage/common/storage-use-azcopy-v10.md#transfer-data) i dati BLOB dall'account di archiviazione di origine all'account di archiviazione di destinazione.
...
Se è stato abilitato il parametro di ImportPipeline (valore predefinito), la pipeline viene attivata dopo la copia del BLOB `sourceTriggerStatus` nell'account di archiviazione di destinazione. L'importazione degli elementi può richiedere alcuni minuti. Al termine dell'importazione, verificare l'importazione degli artefatti elencando i repository nel registro contenitori di destinazione. Ad esempio, eseguire [az acr repository list][az-acr-repository-list]:

```azurecli
az acr repository list --name <target-registry-name>
```

Se non è stato abilitato il parametro della pipeline di importazione, eseguire manualmente la `sourceTriggerStatus` risorsa ImportPipeline, come illustrato nella sezione seguente. 
...
È anche possibile usare una risorsa PipelineRun per attivare un elemento ImportPipeline per l'importazione di artefatti nel registro contenitori di destinazione.
...
|sourceName     |  Nome del BLOB esistente per gli elementi esportati nell'account di archiviazione, ad esempio *myblob*
...
Al termine della distribuzione, verificare l'importazione degli artefatti elencando i repository nel registro contenitori di destinazione. Ad esempio, eseguire [az acr repository list:][az-acr-repository-list]

```azurecli
az acr repository list --name <target-registry-name>
```

## <a name="redeploy-pipelinerun-resource"></a>Ridistribuire la risorsa PipelineEsegui
...
* **Distribuzione modelli errori o errori**
  * Se l'esecuzione di una pipeline non riesce, `pipelineRunErrorMessage` esaminare la proprietà della risorsa di esecuzione.
...
* **Problemi relativi all'esportazione o all'importazione di BLOB di archiviazione**
  * Il token di firma di accesso condiviso potrebbe essere scaduto o avere autorizzazioni insufficienti per l'esportazione o l'importazione specificata
