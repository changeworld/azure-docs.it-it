---
title: 'Esercitazione: integrare il gateway NAT con un servizio di bilanciamento del carico interno-portale di Azure'
titleSuffix: Virtual Network NAT
description: In questa esercitazione si apprenderà come integrare un gateway NAT di rete virtuale con un servizio di bilanciamento del carico interno usando il portale di Azure.
author: asudbring
ms.author: allensu
...
ms.locfileid: "104670846"
---
# <a name="tutorial-integrate-a-nat-gateway-with-an-internal-load-balancer-using-the-azure-portal"></a>Esercitazione: integrare un gateway NAT con un servizio di bilanciamento del carico interno usando il portale di Azure

In questa esercitazione si apprenderà come integrare un gateway NAT con un servizio di bilanciamento del carico interno.

Per impostazione predefinita, un Load Balancer Standard di Azure è protetto. La connettività in uscita viene definita in modo esplicito abilitando SNAT in uscita (Source Network Address Translation). 

SNAT è abilitato per un pool back-end interno tramite un altro servizio di bilanciamento del carico pubblico, un routing di rete o un indirizzo IP pubblico definito in una macchina virtuale.

L'integrazione del gateway NAT sostituisce la necessità di distribuire un servizio di bilanciamento del carico pubblico, un routing di rete o un indirizzo IP pubblico definito in una macchina virtuale nel pool back-end.
...
3. In **Crea rete virtuale** immettere o selezionare queste informazioni nella scheda **Generale**:
    ...
    | Gruppo di risorse   | Selezionare **Crea nuovo**. Immettere **TutorIntLBNAT-RG**. </br> Selezionare **OK**. |
    | **Dettagli istanza** |                                                                 |
...
4. Nella pagina **Crea** servizio di bilanciamento del carico immettere o selezionare le informazioni seguenti: 
    ...
    | Resource group         | Selezionare **TutorIntLBNAT-RG**.|
    | **Dettagli istanza** |    |
    ...
    | Zona di disponibilità | Selezionare **Con ridondanza della zona** per creare un servizio di bilanciamento del carico resiliente. </br> Per creare un servizio di bilanciamento del carico di zona, selezionare una zona specifica tra 1, 2 o 3 |
...
3. Nella pagina **Aggiungi un pool back-end** digitare **myBackendPool** in nome per il pool back-end, quindi selezionare **Aggiungi**.
...
2. In **Crea macchina virtuale** digitare o selezionare i valori nella scheda **Nozioni di base**:
    ...
    | **Dettagli del progetto** |  |
    | Subscription | Selezionare la sottoscrizione ad Azure |
    | Gruppo di risorse | Selezionare **TutorIntLBNAT-RG** |
    | **Dettagli istanza** |  |
...
2. Prendere nota dell'indirizzo IP pubblico:
    ...
    :::image type="content" source="./media/tutorial-nat-gateway-load-balancer-internal-portal/find-public-ip.png" alt-text="Screenshot dell'individuazione dell'indirizzo IP pubblico del gateway NAT." border="true":::

3. Selezionare **tutti i servizi** nel menu a sinistra, selezionare **tutte le risorse** e quindi nell'elenco delle risorse selezionare **myVM1** che si trova nel gruppo di risorse **TutorIntLBNAT-RG** .
...
9. Verificare che l'indirizzo IP visualizzato corrisponda all'indirizzo del gateway NAT annotato nel passaggio precedente:
    ...
    :::image type="content" source="./media/tutorial-nat-gateway-load-balancer-internal-portal/my-ip.png" alt-text="Screenshot di Internet Explorer che mostra l'indirizzo IP in uscita esterno." border="true":::
...
2. Selezionare il gruppo di risorse **TutorIntLBNAT-RG** .

3. Selezionare **Elimina gruppo di risorse**.

4. Immettere **TutorIntLBNAT-RG** e selezionare **Elimina**.
