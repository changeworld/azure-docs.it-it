...
5. <a name="add-peering"></a>Immettere o selezionare i valori per le impostazioni seguenti:
    ...
    - **Consenti accesso alla rete virtuale:** Selezionare **abilitato** (impostazione predefinita) se si vuole abilitare la comunicazione tra le due reti virtuali tramite il `VirtualNetwork` flusso predefinito. L'abilitazione della comunicazione tra reti virtuali consente alle risorse connesse a una o all'altra delle reti virtuali di comunicare tra loro con la stessa larghezza di banda e latenza che userebbero se fossero connesse alla stessa rete virtuale. Tutte le comunicazioni tra le risorse nelle due reti virtuali avvengono tramite la rete privata di Azure. Il tag del servizio **virtualnetwork** per i gruppi di sicurezza di rete comprende la rete virtuale e la rete virtuale con peering quando questa impostazione è **abilitata**. Per altre informazioni sui tag di servizio del gruppo di sicurezza di rete, vedere [Cenni preliminari sui gruppi di sicurezza di rete](./network-security-groups-overview.md#service-tags). Selezionare **Disabled (disabilitato** ) se non si vuole che il traffico scorra alla rete virtuale con peering per impostazione predefinita. È possibile selezionare **disabilitato** se è stato effettuato il peering di una rete virtuale con un'altra rete virtuale, ma occasionalmente si vuole disabilitare il flusso di traffico predefinito tra le due reti virtuali. L'abilitazione/disabilitazione può risultare più pratica che eliminare e ricreare i peering. Quando questa impostazione è disabilitata, il traffico non viene propagato tra le reti virtuali con peering per impostazione predefinita. Tuttavia, il traffico può ancora fluire se consentito in modo esplicito tramite una regola del [gruppo di sicurezza di rete](./network-security-groups-overview.md) che include gli indirizzi IP appropriati o i gruppi di sicurezza delle applicazioni.
      > [!WARNING]
      > La disabilitazione dell'impostazione **Consenti accesso alla rete virtuale** cambia solo la definizione del tag del servizio **virtualnetwork** . *Non* impedisce completamente il flusso del traffico attraverso la connessione peer, come illustrato in questa descrizione dell'impostazione.    
