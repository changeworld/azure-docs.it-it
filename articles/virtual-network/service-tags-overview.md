...
![Isolamento della rete dei servizi di Azure tramite tag di servizio](./media/service-tags-overview/service_tags.png)
...
| **AzureCognitiveSearch** | Ricerca cognitiva di Azure <br/><br/>Questo tag o gli indirizzi IP associati possono essere usati per concedere agli indicizzatori l'accesso sicuro alle origini dati. Per altri [dettagli, vedere](../search/search-indexer-troubleshooting.md#connection-errors) la documentazione sulla connessione dell'indicizzatore. <br/><br/> *Nota*: l'indirizzo IP del servizio di ricerca non è incluso nell'elenco di intervalli IP per questo tag del servizio ed **è necessario aggiungerlo anche** al firewall IP delle origini dati. | In ingresso | No | No |
...
| **AzureMonitor** | Log Analytics, Application Insights, AzMon e metriche personalizzate (GiG endpoints).<br/><br/>*Nota:* Per Log Analytics è necessario **anche il** tag Storage. Se vengono usati agenti Linux, è necessario anche il tag **GuestAndHybridManagement.** | In uscita | No | Sì |
...
> [!NOTE]
> I dati dell'API rappresenteranno i tag che possono essere usati con le regole del gruppo di sicurezza di rete, un subset dei tag attualmente presenti nel file JSON scaricabile. Nell'anteprima pubblica non è garantito che i dati rimangano invariati da un aggiornamento a quello successivo. 
