...
3. In **Impostazioni** a sinistra selezionare **Rete** e passare alla risorsa dell'interfaccia di rete selezionandone il nome.
     ![Visualizzare le interfacce di rete](./media/diagnose-network-routing-problem/view-nics.png)
4. A sinistra selezionare **Route effettive.** Le route effettive per un'interfaccia di rete **denominata myVMNic1** sono illustrate nell'immagine ![ seguente: Visualizzare le route effettive](./media/diagnose-network-routing-problem/view-effective-routes.png)
...
Ottenere le route valide per un'interfaccia di rete con [az network nic show-effective-route-table](/cli/azure/network/nic#az_network_nic_show_effective_route_table). L'esempio seguente ottiene le route effettive per un'interfaccia di rete denominata *myVMNic1* che si trova in un gruppo di risorse *denominato myResourceGroup*:
