...
description: Informazioni sui vantaggi del data plan Development Kit (DPDK) e sulla configurazione di DPDK in una macchina virtuale Linux.
...
Per qualsiasi versione del kernel Linux non elencata, vedere [Patches for building an Azure-tuned Linux kernel](https://github.com/microsoft/azure-linux-kernel) (Patch per la compilazione di un kernel Linux ottimizzato per Azure). Per altre informazioni, è anche possibile contattare [aznetdpdk@microsoft.com](mailto:aznetdpdk@microsoft.com). 
...
La rete accelerata deve essere abilitata su una macchina virtuale Linux. La macchina virtuale deve avere almeno due interfacce di rete, con un'interfaccia per la gestione. L'abilitazione della rete accelerata sull'interfaccia di gestione non è consigliata. Informazioni sulle modalità per [Creare una macchina virtuale Linux con rete accelerata abilitata](create-vm-accelerated-networking-cli.md).
