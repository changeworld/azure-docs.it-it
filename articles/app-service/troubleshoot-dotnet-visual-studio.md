...
## <a name="app-configuration-and-management"></a><a name="sitemanagement"></a>Configurazione e gestione di app
...
1. Se si è già effettuato l'accesso ad Azure in Visual Studio, fare clic on il pulsante destro del mouse su **Azure** e selezionare Connetti a **Sottoscrizione di Microsoft Azure** in **Esplora Server**.
   ...
   >
    ...
    Per altre informazioni sulla connessione alle risorse di Azure da Visual Studio, vedere [assegnare i ruoli di Azure usando il portale di Azure](../role-based-access-control/role-assignments-portal.md).
...
## <a name="access-app-files-in-server-explorer"></a><a name="remoteview"></a>Accedere ai file dell'app in Esplora server
...
:::image type="content" source="./media/web-sites-dotnet-troubleshoot-visual-studio/genericerror.png" alt-text="Screenshot che mostra un errore del server nell'applicazione '/' in un Web browser.":::
...
## <a name="remote-debugging-apps"></a><a name="remotedebug"></a>Debug remoto di app
...
1. Fare clic su **Pubblica**. Al termine della distribuzione, quando il browser si apre all'URL di Azure dell'app, chiudere il browser.
...
1. Passare il puntatore sulla variabile `currentTime` per visualizzare il valore relativo all'ora.
...
1. Immettere un nuovo valore per la variabile `currentTime` , ad esempio "In esecuzione in Azure".
...
7. In **Esplora server** espandere **Azure > Servizio app > gruppo di risorse dell'utente > App dell'utente > Processi Web > Continuo**, quindi fare clic con il pulsante destro del mouse su **ContosoAdsWebJob**.
...
## <a name="diagnostic-logs-overview"></a><a name="logsoverview"></a>Panoramica dei log di diagnostica
...
* **Log di traccia dell'applicazione**<br/>
  L'applicazione crea questi log chiamando i metodi della classe [System.Diagnostics.Trace](/dotnet/api/system.diagnostics.trace) .
...
## <a name="view-web-server-logs"></a><a name="webserverlogs"></a>Visualizzazione dei log del server Web
...
## <a name="view-detailed-error-message-logs"></a><a name="detailederrorlogs"></a>Visualizzare i log dei messaggi di errore dettagliati
...
## <a name="download-file-system-logs"></a><a name="downloadlogs"></a>Download dei log del file system
...
14. To look at the details for a single row, double-click one of the rows.
 ...
 -->
## <a name="view-failed-request-tracing-logs"></a><a name="failedrequestlogs"></a>Visualizzare i log di traccia delle richieste non riuscite
...
* [Analisi delle perdite di memoria nel Servizio app di Azure con Visual Studio 2013](https://devblogs.microsoft.com/devops/investigating-memory-leaks-in-azure-web-sites-with-visual-studio-2013/). Post del blog di Microsoft ALM sulle funzionalità di Visual Studio per l'analisi dei problemi relativi alla memoria gestita.
...
Per altre informazioni su come usare la modalità debug in Visual Studio, vedere gli argomenti [Debug in Visual Studio](/visualstudio/debugger/debugging-in-visual-studio) e [Suggerimenti sul debug con Visual Studio 2010](https://weblogs.asp.net/scottgu/archive/2010/08/18/debugging-tips-with-visual-studio-2010.aspx).
...
* [Introduzione al debug remoto del Servizio app di Azure](https://azure.microsoft.com/blog/2014/05/06/introduction-to-remote-debugging-on-azure-web-sites/).
* [Introduzione al debug remoto del Servizio app di Azure parte 2 - Informazioni di approfondimento](https://azure.microsoft.com/blog/2014/05/07/introduction-to-remote-debugging-azure-web-sites-part-2-inside-remote-debugging/)
...
Per la registrazione degli errori, in alternativa alla scrittura di codice di traccia personalizzato, è possibile utilizzare un framework di registrazione open source come [ELMAH](https://nuget.org/packages/elmah/). Per ulteriori informazioni, vedere i [post di blog di Scott Hanselman su ELMAH](https://www.hanselman.com/blog/NuGetPackageOfTheWeek7ELMAHErrorLoggingModulesAndHandlersWithSQLServerCompact.aspx).
