...
Questa esercitazione illustra il processo di creazione, configurazione, distribuzione e ridimensionamento di app Web Java in Azure. Al termine, si avrà un'applicazione [Spring Boot](https://projects.spring.io/spring-boot/) che memorizza i dati in [Azure Cosmos DB](../cosmos-db/index.yml) in esecuzione nel [servizio app di Azure in Linux](overview.md).
...
* [Interfaccia della riga di comando di Azure](/cli/azure/overview) installata nel computer. 
...
Aprire il file `pom.xml` nella directory `initial/spring-boot-todo` e aggiungere la configurazione seguente del [plug-in dell'app Web di Azure per Maven](https://github.com/Microsoft/azure-maven-plugins/blob/develop/azure-webapp-maven-plugin/README.md).
...
Se queste risorse non sono necessarie per [](#next)un'altra esercitazione (vedere Passaggi successivi), è possibile eliminarle eseguendo il comando seguente nel Cloud Shell:â€ ̄ Â 
```azurecli
...
[Spring Boot](https://spring.io/projects/spring-boot), [Spring Data per Cosmos DB](/azure/developer/java/spring-framework/configure-spring-boot-starter-java-app-with-cosmos-db), [Azure Cosmos DB](../cosmos-db/introduction.md) e [Servizio app in Linux](overview.md).
...
> [Guida dello sviluppatore Java per il servizio app in Linux](configure-language-java.md?pivots=platform-linux)
