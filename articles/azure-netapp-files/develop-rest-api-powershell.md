---
title: Sviluppare per Azure NetApp Files con l'API REST usando PowerShell | Microsoft Docs
description: Viene descritto come iniziare a usare l'API REST di Azure NetApp Files tramite PowerShell.
...
ms.locfileid: "102180378"
---
# <a name="develop-for-azure-netapp-files-with-rest-api-using-powershell"></a>Sviluppare per Azure NetApp Files con l'API REST con PowerShell

L'API REST per il servizio Azure NetApp Files definisce le operazioni HTTP sulle risorse, ad esempio l'account di NetApp, il pool di capacità, i volumi e gli snapshot. Questo articolo consente di iniziare a usare l'API REST di Azure NetApp Files usando PowerShell.
...
1. [Installare l'interfaccia della riga di comando di Azure](/cli/azure/install-azure-cli) se non lo si è già fatto.
2. Creare un'entità servizio in Azure Active Directory (Azure AD):
   ...
   2. Immettere il comando seguente nell'interfaccia della riga di comando di Azure:  
      ...
      Per visualizzare le informazioni sull'entità servizio, digitare `$RBAC_SP` e premere INVIO.
      ...
      L'output viene salvato nell'oggetto variabile `$RBAC_SP` . Si useranno i `$RBAC_SP.appId` `$RBAC_SP.password` valori, e `$RBAC_SP.tenant` .

3. Richiedere un token di accesso OAuth:

    Gli esempi in questo articolo usano PowerShell. È anche possibile usare vari strumenti API, ad esempio [Postman](https://www.getpostman.com/), [Insomnia](https://insomnia.rest/) e [Paw](https://paw.cloud/).  

    Con la `$RBAC_SP` variabile verrà ora ottenuto il token di porta. 
    ...
    ```
    L'output fornisce un oggetto token di porta. È possibile visualizzare il token di accesso digitando `$BearerToken.access_token` . Questo ID è simile al seguente:

    ```output
    eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6Im5iQ3dXMTF3M1hrQi14VWFYd0tSU0xqTUhHUSIsImtpZCI6Im5iQ3dXMTF3M1hrQi14VWFYd0tSU0xqTUhHUSJ9
    ```

    Il token visualizzato è valido per 3600 secondi. Successivamente, è necessario richiedere un nuovo token. Il token viene salvato nella variabile e verrà usato nel passaggio successivo.

4. Creare l'oggetto `headers`:
...
5. Inviare un test call e includere il token per convalidare l'accesso all'API REST:
...
Prima di eseguire gli esempi seguenti con valori personalizzati, è necessario assegnare i valori delle variabili. È possibile accedere alle variabili di PowerShell digitando `$variablename` .
Le variabili di PowerShell vengono assegnate tramite `$variablename = “value”` .
...
Utilizzare una richiesta PUT per creare nuovi oggetti Azure NetApp Files, come mostrato negli esempi seguenti. Il corpo della richiesta PUT include i dati in formato JSON per le modifiche. Deve essere incluso nel comando di PowerShell come testo o come riferimento come file. Per fare riferimento al corpo come file, salvare l'esempio JSON in un file e aggiungerlo `-body (Get-Content @<filename>)` al comando di PowerShell.
...
Se la risorsa non esiste, si verifica un errore. Usare una richiesta GET per eseguire query sugli oggetti Azure NetApp Files in una sottoscrizione, come negli esempi seguenti:
...
### <a name="complete-powershell-scripts"></a>Completare gli script di PowerShell
Questa sezione illustra gli script di esempio per PowerShell.
