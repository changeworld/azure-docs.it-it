...
### <a name="does-the-data-path-for-nfs-or-smb-go-over-the-internet"></a>Il percorso dati per NFS o SMB passa attraverso Internet?  

No. Il percorso dei dati per NFS o SMB non passa attraverso Internet. Azure NetApp Files è un servizio nativo di Azure distribuito nella rete virtuale di Azure (VNet) in cui è disponibile il servizio. Azure NetApp Files usa una subnet delegata ed effettua il provisioning di un'interfaccia di rete direttamente nel VNet. 
...
### <a name="can-i-manage-smb-shares-sessions-and-open-files-through-computer-management-console-mmc"></a>È possibile gestire `SMB Shares` , `Sessions` e `Open Files` tramite la console di gestione computer (MMC)?

La gestione di `SMB Shares` , `Sessions` e `Open Files` tramite la console di gestione computer (MMC) non è attualmente supportata.
...
Usare il collegamento **visualizzazione JSON** nel riquadro Panoramica volume e cercare l'identificatore **IPIniziale** in **proprietà**  ->  **mountTargets**.
