...
> [!WARNING]
>  Questa guida di avvio rapido usa i pacchetti azure-servicebus precedenti. Per una guida di avvio rapido che usa il nuovo pacchetto azure-messaging-servicebus, vedere [Inviare e ricevere messaggi con azure-messaging-servicebus](service-bus-java-how-to-use-topics-subscriptions.md).
...
![Aggiungere le librerie di Microsoft Azure per Java al progetto Eclipse](media/service-bus-java-how-to-use-topics-subscriptions-legacy/eclipse-azure-libraries-java.png)
...
Aggiungere una classe con un metodo **principale** e quindi aggiungere le istruzioni `import` seguenti all'inizio del file Java:
...
Aggiorna il metodo **principale** per creare un oggetto **TopicClient** e richiamare un metodo helper che invia in modo asincrono i messaggi di esempio all'argomento del bus di servizio.

> [!NOTE] 
> - Sostituire `<NameOfServiceBusNamespace>` con il nome dello spazio dei nomi del bus di servizio. 
> - Sostituire `<AccessKey>` con la chiave di accesso per lo spazio dei nomi.
...
Aggiornare il metodo **principale** per creare tre oggetti **SubscriptionClient** per tre sottoscrizioni e richiamare un metodo helper che riceve in modo asincrono i messaggi dall'argomento del bus di servizio. Il codice di esempio presuppone che sia stato creato un argomento denominato **BasicTopic** e tre sottoscrizioni denominate **Subscription1**, **Subscription2** e **Subscription3**. Se sono stati usati nomi diversi, aggiornare il codice prima di eseguire il test. 
