...
I prodotti middleware orientati ai messaggi hanno tradizionalmente usato protocolli proprietari per le comunicazioni tra applicazioni client e broker. Ciò significa che dopo aver selezionato il broker di messaggistica di un determinato fornitore, è necessario usare le librerie di tale fornitore per connettere le applicazioni client a tale broker. Il risultato è un livello di dipendenza da tale fornitore, perché il trasferimento di un'applicazione a un prodotto diverso richiede modifiche al codice in tutte le applicazioni connesse. Nella community Java, gli standard API specifici del linguaggio come Java Message Service (JMS) e le astrazioni di Spring Framework hanno attenuato il dolore, ma hanno un ambito di funzionalità limitato ed escludono gli sviluppatori che usano altri linguaggi.

La connessione di broker di messaggistica da fornitori diversi è inoltre complicata Richiede in genere il bridging a livello di applicazione per spostare i messaggi da un sistema a un altro e per tradurli tra i formati dei messaggi proprietari. Si tratta di un requisito comune. ad esempio, quando è necessario fornire una nuova interfaccia unificata a sistemi diversi meno recenti o integrare i sistemi IT in seguito a una fusione. AMQP consente di collegare direttamente i broker di connessione, ad esempio usando router come [Apache Qpid Dispatch router](https://qpid.apache.org/components/dispatch-router/index.html) o "pale" native di broker come quello di [RabbitMQ](service-bus-integrate-with-rabbitmq.md).

Il settore software è caratterizzato da rapido sviluppo. Nuovi linguaggi di programmazione e framework applicazione vengono introdotti a ritmo sorprendente. Analogamente, i requisiti dei sistemi IT si evolvono nel tempo e gli sviluppatori desiderano avvalersi delle possibilità offerte dai linguaggi e dalle funzionalità più recenti della piattaforma. Tuttavia, a volte il fornitore della messaggistica selezionato non supporta queste piattaforme. Se i protocolli di messaggistica sono proprietari, non è possibile che altri forniscano librerie per queste nuove piattaforme. Pertanto, è necessario usare approcci come la creazione di gateway o bridge che consentano di continuare a usare il prodotto di messaggistica.
...
* **Indipendente dal modello di broker**: la specifica AMQP 1,0 non prevede alcun requisito per il modello di messaggistica utilizzato da un broker. È pertanto possibile aggiungere con facilità il supporto per il protocollo AMQP 1.0 ai broker di messaggistica esistenti.
