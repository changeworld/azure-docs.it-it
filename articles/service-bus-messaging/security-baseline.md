...
ms.locfileid: "105967813"
---
# <a name="azure-security-baseline-for-service-bus"></a>Baseline della sicurezza di Azure per il bus di servizio

Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1,0](../security/benchmarks/overview-v1.md) al bus di servizio. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure. Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili al bus di servizio. I **controlli** non applicabili al bus di servizio o per i quali la responsabilità è Microsoft, sono stati esclusi.
...
**Definizioni predefinite di criteri di Azure-Microsoft. ServiceBus**:
...
**Linee guida**: usare i tag del servizio rete virtuale per definire i controlli di accesso alla rete nei gruppi di sicurezza di rete o nei firewall di Azure che filtrano il traffico verso e dalle risorse del bus di servizio. È possibile usare tag di servizio invece di indirizzi IP specifici nella creazione di regole di sicurezza. Specificando il nome del tag di servizio (ad esempio, ServiceBus) nel campo di origine o di destinazione appropriato di una regola, è possibile consentire o negare il traffico per il servizio corrispondente. I prefissi di indirizzo inclusi nel tag di servizio sono gestiti da Microsoft, che lo aggiorna automaticamente in caso di modifica degli indirizzi. 
...
- [Criteri predefiniti di Azure per lo spazio dei nomi del bus di servizio](https://docs.microsoft.com/azure/service-bus-messaging/policy-reference#azure-service-bus-messaging)
...
- [Come impostare i parametri di conservazione dei log per aree di lavoro Log Analytics](https://docs.microsoft.com/azure/azure-monitor/logs/manage-cost-storage#change-the-data-retention-period)
...
- [Ruoli predefiniti per il bus di servizio di Azure](https://docs.microsoft.com/azure/service-bus-messaging/authenticate-application#azure-built-in-roles-for-azure-service-bus)
...
**Linee guida**: abilitare l'autenticazione a più fattori di Azure Active Directory (Azure ad) e seguire le indicazioni sulla gestione delle identità e dell'accesso del Centro sicurezza di Azure per proteggere le risorse abilitate per il bus di servizio.
...
**Indicazioni**: usare workstation con accesso con privilegi (Paw) con l'autenticazione a più fattori configurata per accedere e configurare le risorse abilitate per il bus di servizio.
...
**Linee guida**: usare Azure Active Directory (Azure ad) come sistema di autenticazione e autorizzazione centrale per le risorse di Azure, ad esempio il bus di servizio. Questo consente il controllo degli accessi in base al ruolo di Azure (RBAC) per le risorse amministrative riservate.

- [Come creare e configurare un'istanza di Azure AD](../active-directory/fundamentals/active-directory-access-create-new-tenant.md)

- [Autorizzare l'accesso alle risorse del bus di servizio usando Azure AD](authenticate-application.md)
...
**Linee guida**: usare le funzionalità di protezione delle identità e rilevamento dei rischi di Azure Active Directory (Azure ad) per configurare risposte automatiche per le azioni sospette rilevate correlate alle risorse abilitate per il bus di servizio. È necessario abilitare le risposte automatiche tramite Sentinel di Azure per implementare le risposte di sicurezza dell'organizzazione.
...
- [Autenticare un'identità gestita con Azure Active Directory (Azure AD) per accedere alle risorse del bus di servizio](service-bus-managed-service-identity.md)
...
- [Autenticare un'identità gestita con Azure AD per accedere alle risorse del bus di servizio](service-bus-managed-service-identity.md)
