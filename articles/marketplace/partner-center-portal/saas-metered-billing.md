---
title: Fatturazione a consumo per le offerte SaaS tramite il servizio di misurazione Microsoft Commercial Marketplace
description: Informazioni sui modelli di fatturazione flessibili per le offerte SaaS tramite il servizio di misurazione del Marketplace commerciale.
...
Con il servizio di misurazione del Marketplace commerciale è possibile creare offerte SaaS (Software as a Service) addebitate in base alle unità non standard. Prima di pubblicare un'offerta SaaS nel Marketplace commerciale, è possibile definire le dimensioni di fatturazione, ad esempio la larghezza di banda, i ticket o i messaggi di posta elettronica elaborati.  I clienti pagano quindi in base al consumo di queste dimensioni, con il sistema che informa Microsoft attraverso l'API del servizio di misurazione del Marketplace commerciale degli eventi fatturabili non appena si verificano.  
...
- Soddisfa tutti i requisiti dell'offerta per un' [offerta di vendita tramite Microsoft](../plan-saas-offer.md#listing-options) , come descritto in [creare un'offerta SaaS nel Marketplace commerciale](../create-new-saas-offer.md).
...
L'offerta SaaS può quindi integrarsi con le [API del servizio di misurazione del Marketplace commerciale](./marketplace-metering-service-apis.md) per informare Microsoft degli eventi fatturabili.
...
- Oltre alle tariffe ricorrenti, un piano tariffario flat può includere anche dimensioni personalizzate facoltative usate per addebitare i clienti per l'utilizzo in eccedenza non incluso nella tariffa fissa.  Ogni dimensione rappresenta un'unità fatturabile che il servizio comunicherà a Microsoft tramite l' [API del servizio di misurazione del Marketplace commerciale](./marketplace-metering-service-apis.md).
...
Analogamente ai prezzi forfettari, è possibile impostare prezzi per le dimensioni di fatturazione per ogni paese o area geografica supportata. È necessario usare la funzionalità di importazione ed esportazione dei dati dei prezzi nel centro per i partner, come indicato di seguito.

1. Definire le dimensioni desiderate e contrassegnare i mercati supportati. 
1. Esportare i dati in un file.
...
[![dimensioni del servizio di misurazione del Marketplace commerciale](media/metering-service-dimensions.png "Fare clic per la visualizzazione ingrandita")](media/metering-service-dimensions.png)
...
La fatturazione a consumo con il servizio di misurazione del Marketplace commerciale non è compatibile con l'offerta di una versione di valutazione gratuita.  Non è possibile configurare un piano per usare la fatturazione a consumo e una versione di valutazione gratuita.

### <a name="locking-behavior"></a>Comportamento di blocco

Poiché una dimensione utilizzata con il servizio di misurazione del Marketplace commerciale rappresenta una conoscenza del modo in cui un cliente pagherà il servizio, tutti i dettagli di una dimensione non saranno più modificabili dopo la pubblicazione.  È importante che le dimensioni siano definite completamente per un piano prima della pubblicazione.
...
Il numero massimo di dimensioni che possono essere configurate per una singola offerta è 30 dimensioni univoche.
