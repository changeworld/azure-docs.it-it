...
## <a name="the-defender-for-iot-platform"></a>Il Defender per la piattaforma Internet
...
Le organizzazioni usano i controller di logica programmabili (PLC) per gestire i processi fisici, ad esempio le armi robotizzate nelle fabbriche, le turbine rotanti nelle farm eoliche e le centrifughe nelle centrali elettriche nucleari.
...
Per inviare informazioni sugli avvisi a FortiSIEM, usare Defender per le regole di inoltramento di Internet.
...
È possibile impostare automaticamente i criteri di blocco nel firewall di FortiGate in base a Defender per gli avvisi.
...
1. Salvare la chiave API.

    :::image type="content" source="media/integration-fortinet/api-key.png" alt-text="La descrizione del telefono cellulare genera automaticamente la nuova chiave API":::

## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>Blocca il traffico sospetto usando il firewall FortiGate

1. Nel riquadro sinistro selezionare **inoltri**.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="Opzione della finestra di inoltri in un sensore.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Selezionare **create inoltr Rules** e define Rule Parameters.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Screenshot della finestra Crea regola di inoltri":::
    ...
    | **Protocolli** | Per impostazione predefinita, vengono selezionati tutti i protocolli.<br><br>Per selezionare un protocollo specifico, selezionare **specifico** e selezionare il protocollo per cui viene applicata la regola. |
    | **Motori** | Per impostazione predefinita, sono interessati tutti i motori di sicurezza.<br><br>Per selezionare un motore di sicurezza specifico a cui applicare la regola, selezionare **specifico** e selezionare il motore. |
    | **Notifiche di sistema** | Stato di avanzamento del sensore *online e offline* . Questa opzione è disponibile solo se è stato effettuato l'accesso alla console di gestione locale. |

1. Per indicare a Defender le informazioni per l'invio di regole del firewall a FortiGate, selezionare **azione** e quindi selezionare **Invia a FortiGate**.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="Crea finestra regola di inoltramento e seleziona Invia a FortiGate":::

1. Per configurare la regola di invio FortiGate:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Configurare la finestra Crea regola di inoltri":::
...
1. Nel riquadro **azioni** impostare i parametri seguenti:
    ...
    | Username | Tipo di nome utente del server FortiGate. |
    | Chiave API | Immettere la chiave API creata in FortiGate. |
    | Interfaccia in ingresso| Definire la modalità di esecuzione del blocco:<br /><br />**In base all'indirizzo IP**: crea sempre i criteri di blocco su panorama in base all'indirizzo IP.<br /><br />**Per nome di dominio completo o indirizzo IP**: crea i criteri di blocco su Panorama basato su FQDN se esistente, in caso contrario indirizzo IP.| 
    | Interfaccia in uscita |Impostare l'indirizzo di posta elettronica per il messaggio di notifica del criterio. <br /><br /> **Nota**: assicurarsi di aver configurato un server di posta elettronica in XSense. Se non viene immesso alcun indirizzo di posta elettronica, XSense non invia un messaggio di posta elettronica di notifica.| 
    |Configurare| Configurare le opzioni seguenti per consentire il blocco delle origini sospette da parte del firewall FortiGate: <br /><br />**Blocca i codici di funzione non validi**: violazioni del protocollo-valore di campo non valido che viola la specifica del protocollo ICS (potenziale exploit)<br /><br />**Blocca la programmazione o gli aggiornamenti del firmware del PLC non autorizzati**: modifiche al PLC non autorizzate<br /><br />**Blocca arresto del PLC non autorizzato**: arresto del PLC (tempo di inattività)<br /><br />**Blocca gli avvisi correlati al malware**: blocco dei tentativi di malware industriali (Triton, NotPetya e così via). È possibile selezionare l'opzione di **blocco automatico**. In tal caso, il blocco viene eseguito automaticamente e immediatamente.<br /><br />*Blocca l'analisi non autorizzata*: analisi non autorizzata (potenziale esplorazione)
