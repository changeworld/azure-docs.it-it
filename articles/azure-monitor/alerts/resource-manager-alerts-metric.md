...
## <a name="single-criteria-static-threshold"></a>Singolo criterio, soglia statica
L'esempio seguente crea una regola di avviso delle metriche usando un singolo criterio e una soglia statica.
...
## <a name="single-criteria-dynamic-threshold"></a>Singolo criterio, soglia dinamica
L'esempio seguente crea una regola di avviso delle metriche usando un singolo criterio e una soglia dinamica.
...
## <a name="multiple-criteria-static-threshold"></a>Più criteri, soglia statica
Gli avvisi delle metriche supportano l'invio di avvisi per metriche multidimensionali e fino a 5 criteri per ogni regola di avviso. L'esempio seguente crea una regola di avviso per metriche dimensionali e specifica più criteri.

I vincoli seguenti si applicano quando si usano le dimensioni in una regola di avviso che contiene più criteri:
...
## <a name="multiple-dimensions-dynamic-thresholds"></a>Più dimensioni, soglie dinamiche
...
## <a name="custom-metric-static-threshold"></a>Metrica personalizzata, soglia statica
...
Monitoraggio di Azure supporta il monitoraggio di più risorse dello stesso tipo con una singola regola di avviso delle metriche, per le risorse esistenti nella stessa area di Azure. Questa funzionalità è attualmente supportata solo nel cloud pubblico di Azure e solo per macchine virtuali, database SQL Server, pool elastici di SQL Server e dispositivi Data Box Edge. Inoltre, questa funzionalità è disponibile solo per le metriche della piattaforma e non è supportata per le metriche personalizzate.
...
Questo modello crea una regola di avviso delle metriche con soglie dinamiche che monitora il valore di Percentuale CPU per tutte le macchine virtuali (in un'area di Azure) in uno o più gruppi di risorse.
...
Questo esempio crea una regola di avviso delle metriche con soglia statica che monitora il valore di Percentuale CPU per tutte le macchine virtuali (in un'area di Azure) di una sottoscrizione.
...
Questo esempio crea una regola di avviso delle metriche con soglie dinamiche che monitora il valore di Percentuale CPU per tutte le macchine virtuali (in un'area di Azure) di una sottoscrizione.
...
Questo esempio crea una regola di avviso delle metriche con soglia statica che monitora il valore di Percentuale CPU per un elenco di macchine virtuali (in un'area di Azure) di una sottoscrizione.
...
Questo esempio crea una regola di avviso delle metriche con soglie dinamiche che monitora il valore di Percentuale CPU per un elenco di macchine virtuali (in un'area di Azure) di una sottoscrizione.
