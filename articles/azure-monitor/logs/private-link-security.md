...
* `in.ai` - (Application Insights'endpoint di inserimento, verranno visualizzati una voce globale e una voce a livello di regione
...
* Per assicurarsi che l'area di lavoro o il componente non riceva richieste da reti pubbliche (non connesse tramite AMPLS), impostare i flag di inserimento pubblico e di query della risorsa su *No,* come illustrato [in](#manage-access-from-outside-of-private-links-scopes)Gestire l'accesso dall'esterno degli ambiti dei collegamenti privati .
...
Per consentire all'agente di Log Analytics di scaricare i Pacchetti di soluzioni, aggiungere i nomi di dominio completi appropriati all'elenco di elementi consentiti del firewall. 
...
* Usa zone DNS personalizzate per app specifiche: questa soluzione consente di accedere a componenti Application Insights selezionati tramite un collegamento privato, mantenendo tutto il traffico sulle route pubbliche.
    ...
    - Creare un'istanza di AMPLS e un endpoint privato e scegliere di **non** eseguire l'integrazione automatica con DNS privato
...
Per creare e gestire ambiti di collegamento privato, usare [l'API REST](/rest/api/monitor/private%20link%20scopes%20(preview)) o l'interfaccia della riga di comando di [Azure (az monitor private-link-scope).](/cli/azure/monitor/private-link-scope)
