...
Consultare [Domande frequenti per la transizione dal portale di OMS al portale di Azure per gli utenti di Log Analytics](../overview.md) per informazioni su come effettuare la transizione al portale di Azure. 

## <a name="user-access-and-role-migration"></a>Migrazione del ruolo e dell'accesso utente
La gestione di accesso al portale Azure è più completa e più potente rispetto alla gestione di accesso nel portale di OMS. Vedere [progettazione dell'area di lavoro log di monitoraggio di Azure](../logs/design-logs-deployment.md) per informazioni dettagliate sulla gestione dell'accesso in log Analytics.
...
Se il [gruppo di gestione di Operations Manager è stato connesso a Log Analytics](../agents/om-agents.md), continuerà a funzionare senza alcuna variazione. Per le nuove connessioni, tuttavia, è necessario seguire le indicazioni fornite in [Microsoft System Center Operations Manager Management Pack to configure Operations Management Suite](https://techcommunity.microsoft.com/t5/system-center-blog/bg-p/SystemCenterBlog) (Pacchetto di gestione di Microsoft System Center Operations Manager per configurare Operations Management Suite).

## <a name="next-steps"></a>Passaggi successivi
- Consultare [Domande frequenti per la transizione dal portale di OMS al portale di Azure per gli utenti di Log Analytics](../overview.md) per il materiale sussidiario sullo spostamento dal portale di OMS al portale di Azure.