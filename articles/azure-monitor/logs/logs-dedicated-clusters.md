...
- **[Chiavi gestite dal cliente](../logs/customer-managed-keys.md)** : crittografare i dati del cluster usando chiavi fornite e controllate dal cliente.
- **[Archivio](../logs/customer-managed-keys.md#customer-lockbox-preview)** dati: i clienti possono controllare le richieste di accesso ai tecnici del supporto tecnico Microsoft.
...
> [!WARNING]
> La creazione del cluster attiva l'allocazione delle risorse e il provisioning. Il completamento di questa operazione può richiedere alcune ore. È consigliabile eseguirlo in modo asincrono.
...
- **keyVaultProperties** : aggiorna la chiave in Azure Key Vault. Vedere [Aggiorna cluster con i dettagli dell'identificatore di chiave](../logs/customer-managed-keys.md#update-cluster-with-key-identifier-details). Contiene i parametri seguenti: *KeyVaultUri*, *nome di codice*, *versione della versione*. 
...
- messaggi di errore
  ...
  -  400--KeyVaultProperties non è vuoto ma presenta un formato non valido. Vedere [aggiornamento dell'identificatore di chiave](../logs/customer-managed-keys.md#update-cluster-with-key-identifier-details).
  -  400--non è stato possibile convalidare la chiave in Key Vault. La causa potrebbe essere la mancanza di autorizzazioni o la chiave non esiste. Verificare di aver [impostato i criteri di accesso e chiave](../logs/customer-managed-keys.md#grant-key-vault-permissions) in Key Vault.
...
- Informazioni sulla [progettazione corretta delle aree di lavoro log Analytics](../logs/design-logs-deployment.md)