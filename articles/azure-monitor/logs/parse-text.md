...
Quando si analizzano i dati in fase di raccolta, configurare [campi personalizzati](../logs/custom-fields.md) che creano nuove proprietà nella tabella. Le query non devono includere una logica di analisi ma semplicemente usare queste proprietà come qualsiasi altro campo nella tabella.
...
Per informazioni sull'analisi dei dati durante la raccolta, vedere [Creare campi personalizzati in Monitoraggio di Azure](../logs/custom-fields.md). Crea proprietà personalizzate nella tabella che è possibile usare nelle query proprio come qualsiasi altra proprietà.
...
Potrebbero essere disponibili più query che eseguono la stessa analisi di una tabella specifica. In questo caso [creare una funzione](../logs/functions.md) che restituisca i dati analizzati invece di replicare la logica di analisi in ogni query. È quindi possibile usare l'alias di funzione al posto della tabella originale in altre query.
