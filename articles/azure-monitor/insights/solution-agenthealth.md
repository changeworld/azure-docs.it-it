...
| `IsGatewayInstalled` | Se Log Analytics gateway è installato, il valore è *true*. in caso contrario, il valore è *false*.|
| `ComputerIP` | Indirizzo IP pubblico del computer. Nelle VM di Azure verrà visualizzato l'indirizzo IP pubblico, se disponibile. Per le macchine virtuali che usano indirizzi IP privati, verrà visualizzato l'indirizzo Azure SNAT, non l'indirizzo IP privato. |
...
* Informazioni sugli [avvisi in monitoraggio di Azure](../alerts/alerts-overview.md) per informazioni dettagliate sulla generazione di avvisi dalle query di log. 
