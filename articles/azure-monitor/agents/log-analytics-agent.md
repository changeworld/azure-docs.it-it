...
- L'estensione Diagnostica di Azure invia i dati ad Archiviazione di Azure, alle [metriche di Monitoraggio di Azure](../essentials/data-platform-metrics.md) (solo Windows) e a Hub eventi. L'agente Log Analytics invia dati ai [log di monitoraggio di Azure](../logs/data-platform-logs.md).
- L'agente di Log Analytics è necessario per le [soluzioni](../monitor-reference.md#insights-and-core-solutions), le [informazioni dettagliate sulla macchina virtuale](../vm/vminsights-overview.md)e altri servizi, ad esempio il [Centro sicurezza di Azure](../../security-center/index.yml).
...
## <a name="supported-operating-systems"></a>Sistemi operativi supportati
 ...
 Per un elenco delle versioni del sistema operativo Windows e Linux supportate dall'agente di Log Analytics, vedere [sistemi operativi supportati](../agents/agents-overview.md#supported-operating-systems) . 
...
| --- | --- |
| [Log eventi di Windows](../agents/data-sources-windows-events.md) | Informazioni inviate al sistema di registrazione eventi di Windows. |
| [Syslog](../agents/data-sources-syslog.md)                     | Informazioni inviate al sistema di registrazione eventi di Linux. |
| [Prestazioni](../agents/data-sources-performance-counters.md)  | Valori numerici che misurano le prestazioni di diversi aspetti del sistema operativo e dei carichi di lavoro. |
| [Log IIS](../agents/data-sources-iis-logs.md)                 | Informazioni sull'utilizzo per siti Web IIS in esecuzione nel sistema operativo guest. |
| [Log personalizzati](../agents/data-sources-custom-logs.md)           | Eventi raccolti da file di testo in computer Windows e Linux. |
...
Per informazioni dettagliate sulla connessione di un agente a un gruppo di gestione Operations Manager, vedere [configurare l'agente per la segnalazione a un gruppo di gestione Operations Manager](../agents/agent-manage.md#configure-agent-to-report-to-an-operations-manager-management-group) .
...
* Gli agenti Windows e Linux supportano lo [standard FIPS 140](/windows/security/threat-protection/fips-140-validation), ma [potrebbero non essere supportati altri tipi di protezione avanzata](../agents/agent-linux.md#supported-linux-hardening).
...
- [VM Insights](../vm/vminsights-enable-overview.md) offre più metodi che abilitano gli agenti su larga scala. Ciò include l'installazione dell'agente di Log Analytics e dell'agente di dipendenza. 
...
- Installare manualmente per le singole macchine virtuali [di Azure dalla portale di Azure](../vm/quick-collect-azurevm.md?toc=%2fazure%2fazure-monitor%2ftoc.json).

### <a name="windows-virtual-machine-on-premises-or-in-another-cloud"></a>Macchina virtuale Windows locale o in un altro cloud

- Usare i [server abilitati per Azure Arc](../../azure-arc/servers/overview.md) per distribuire e gestire l'estensione della macchina virtuale log Analytics.
- [Installare manualmente](../agents/agent-windows.md) l'agente dalla riga di comando.
- Automatizzare l'installazione con [Automation DSC di Azure](../agents/agent-windows.md#install-agent-using-dsc-in-azure-automation).
...
- Usare i [server abilitati per Azure Arc](../../azure-arc/servers/overview.md) per distribuire e gestire l'estensione della macchina virtuale log Analytics.
- [Installare manualmente](../vm/quick-collect-linux-computer.md) l'agente che chiama uno script wrapper ospitato in GitHub.
- Integrare [System Center Operations Manager](./om-agents.md) con monitoraggio di Azure per l'invio di dati raccolti da computer Windows che inviano report a un gruppo di gestione.
...
Se si prevede di usare il ruolo di lavoro ibrido per Runbook di automazione di Azure per connettersi e registrarsi al servizio di automazione per l'uso di manuali operativi o delle funzionalità di gestione nell'ambiente, deve avere accesso al numero di porta e agli URL descritti in [configurare la rete per il ruolo di lavoro ibrido per Runbook](../../automation/automation-hybrid-runbook-worker.md#network-planning).

### <a name="proxy-configuration"></a>Configurazione proxy

L'agente Windows e Linux supporta la comunicazione con Monitoraggio di Azure tramite un server proxy o un gateway di Log Analytics usando il protocollo HTTPS.  Sono supportate sia l'autenticazione anonima che quella di base (nome utente/password).  Per l'agente di Windows connesso direttamente al servizio, la configurazione del proxy viene specificata durante l'installazione o [dopo la distribuzione](../agents/agent-manage.md#update-proxy-settings) dal Pannello di controllo o con PowerShell.  

Per l'agente di Linux, è possibile specificare il server proxy durante l'installazione o [dopo l'installazione](../agents/agent-manage.md#update-proxy-settings) modificando il file di configurazione proxy.conf. Il valore di configurazione del proxy dell'agente Linux ha la sintassi seguente:
...
* Esaminare le [origini dati](../agents/agent-data-sources.md) per informazioni sulle origini dati disponibili per raccogliere i dati dal sistema Windows o Linux. 
