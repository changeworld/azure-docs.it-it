...
È anche possibile passare le informazioni di traccia da un componente a un altro per una corretta correlazione. Si consideri, ad esempio, uno scenario in cui sono presenti due componenti `module1` e `module2` . Module1 chiama funzioni in Module2 e per ottenere log da `module1` e `module2` in una singola traccia è possibile usare l'approccio seguente:
