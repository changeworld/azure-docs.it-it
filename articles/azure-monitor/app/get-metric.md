...
La limitazione delle richieste è particolarmente importante in quanto, ad esempio, il campionamento, la limitazione può causare la mancata presenza di avvisi perché la condizione per attivare un avviso potrebbe verificarsi localmente e quindi essere eliminata in corrispondenza dell'endpoint di inserimento a causa di una quantità eccessiva di dati inviati. Per questo motivo, per .NET e .NET Core non è consigliabile usare, a meno che non sia `TrackMetric()` stata implementata la logica di aggregazione locale. Se si sta provando a tenere traccia di ogni istanza di un evento in un determinato periodo di tempo, è possibile trovare [`TrackEvent()`](./api-custom-events-metrics.md#trackevent) una soluzione migliore. Si tenga tuttavia presente che, a differenza delle metriche personalizzate, gli eventi personalizzati sono soggetti al campionamento. Ovviamente, è ancora possibile usare `TrackMetric()` anche senza scrivere la pre-aggregazione locale, ma se si è consapevoli dei problemi.
...
> [!NOTE]
> Solo gli SDK .NET e .NET Core hanno un metodo getmetric (). Se si usa Java è possibile usare le [metriche del micrometro](./micrometer-java.md) o `TrackMetric()` . Per JavaScript e Node.js si usa ancora `TrackMetric()` , ma si tenga presente le avvertenze descritte nella sezione precedente. Per Python è possibile usare [OpenCensus. stats](./opencensus-python.md#metrics) per inviare metriche personalizzate, ma l'implementazione della metrica è diversa.

## <a name="getting-started-with-getmetric"></a>Introduzione a getmetric

Per gli esempi, verrà usata un'applicazione di servizio di base di .NET Core 3,1 Worker. Se si vuole replicare esattamente l'ambiente di test usato con questi esempi, seguire i passaggi 1-6 dell' [articolo monitoraggio del servizio Worker](./worker-service.md#net-core-30-worker-service-application) per aggiungere Application Insights a un modello di progetto di servizio di lavoro di base. Questi concetti si applicano a qualsiasi applicazione generale in cui è possibile usare l'SDK, tra cui app Web e app console.
...
> [!NOTE]
> Getmetric non supporta il rilevamento dell'ultimo valore (ovvero "misuratore") o l'istogramma o le distribuzioni di rilevamento.
...
Oltre a memorizzare nella cache l'handle della metrica, l'esempio precedente ha ridotto anche `Task.Delay` a 50 millisecondi, in modo che il ciclo verrebbe eseguito con maggiore frequenza, ottenendo 772 `TrackValue()` chiamate.
...
Attualmente sono supportate 10 dimensioni, tuttavia, maggiori di tre dimensioni richiedono l'uso di `MetricIdentifier` :
...
Le metriche non usano il contesto di telemetria del `TelemetryClient` utilizzato per accedervi. i nomi di dimensione speciali disponibili come costanti nella `MetricDimensionNames` classe sono la soluzione migliore per questa limitazione.

Le aggregazioni di metrica inviate dalla "dimensione speciale della richiesta dell'operazione"-la metrica **non** avranno la relativa `Context.Operation.Name` impostazione su "operazione speciale". Mentre `TrackMetric()` o qualsiasi altro TrackXXX () sarà `OperationName` impostato correttamente su "operazione speciale".
...
Ad esempio, quando l'aggregazione metrica risultante dall'istruzione successiva viene inviata all'endpoint Cloud Application Insights, il `Context.Operation.Name` campo dati verrà impostato su "operazione speciale":
...
Il `MetricConfiguration` costruttore accetta alcune opzioni per la gestione di serie diverse all'interno della rispettiva metrica e un oggetto di una classe `IMetricSeriesConfiguration` che implementa che specifica il comportamento di aggregazione per ogni singola serie della metrica:
