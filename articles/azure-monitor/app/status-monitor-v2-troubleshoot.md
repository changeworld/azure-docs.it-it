...
Questo prodotto è stato scritto e testato usando PowerShell v 5.1.
Questo modulo non è compatibile con le versioni 6 o 7 di PowerShell.
È consigliabile usare PowerShell v 5.1 insieme a versioni più recenti. Per altre informazioni, vedere [uso di PowerShell 7 affiancato a powershell 5,1](/powershell/scripting/install/migrating-from-windows-powershell-51-to-powershell-7#using-powershell-7-side-by-side-with-windows-powershell-51).
...
1. Passare alla directory di configurazione condivisa e trovare il file di applicationHost.config.
...
Se un modulo non è stato caricato in una sessione di PowerShell, è possibile caricarlo manualmente tramite il `Import-Module <path to psd1>` comando.
...
Per una descrizione dettagliata di come usare questo cmdlet, vedere le informazioni di [riferimento sulle API](status-monitor-v2-api-reference.md) .
...
### <a name="how-to-capture-full-sql-command-text"></a>Come acquisire il testo completo del comando SQL

Per acquisire il testo completo del comando SQL, è necessario modificare il file di applicationinsights.config con il codice seguente:
