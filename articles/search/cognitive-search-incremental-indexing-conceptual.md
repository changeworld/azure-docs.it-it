...
1. [Creare un indicizzatore](/rest/api/searchservice/create-indexer) , più un insieme di [competenze](/rest/api/searchservice/create-skillset) , per richiamare la pipeline. Durante l'elaborazione, le fasi di arricchimento vengono salvate per ogni documento nell'archiviazione BLOB per un uso futuro.
1. Testare il codice e, dopo avere apportato le modifiche, usare [Update Skills](/rest/api/searchservice/update-skillset) per modificare una definizione.
1. [Eseguire l'indicizzatore](/rest/api/searchservice/run-indexer) per richiamare la pipeline, recuperando l'output memorizzato nella cache per un'elaborazione più veloce ed economicamente conveniente.
...
+ [Crea indicizzatore (API-Version = 2020-06 -30-Preview)](/rest/api/searchservice/create-indexer) 

+ [Aggiorna indicizzatore (API-Version = 2020-06 -30-Preview)](/rest/api/searchservice/update-indexer) 

+ [Update Skills (API-Version = 2020-06-30)](/rest/api/searchservice/update-skillset) (nuovo parametro URI nella richiesta)

+ [Reimpostazione delle competenze (API-Version = 2020-06-30)](/rest/api/searchservice/preview-api/reset-skills)

+ Indicizzatori di database (SQL di Azure, Cosmos DB). Alcuni indicizzatori recuperano i dati tramite query. Per le query che recuperano dati, l' [aggiornamento dell'origine dati](/rest/api/searchservice/update-data-source) supporta un nuovo parametro in una richiesta **ignoreResetRequirement**, che deve essere impostata su `true` quando l'azione di aggiornamento non deve invalidare la cache. 
