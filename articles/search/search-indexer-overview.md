...
Gli indicizzatori sono solo Azure, con singoli indicizzatori per SQL di Azure, Azure Cosmos DB, archiviazione tabelle di Azure e archiviazione BLOB. Quando si configura un indicizzatore, è necessario specificare un'origine dati (Origin), oltre a un indice (destinazione). Diverse origini dati, ad esempio gli indicizzatori di archiviazione BLOB, dispongono di proprietà aggiuntive specifiche del tipo di contenuto.
