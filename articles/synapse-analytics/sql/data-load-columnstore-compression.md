...
ms.locfileid: "107567962"
---
# <a name="maximize-rowgroup-quality-for-columnstore-index-performance"></a>Ottimizzare la qualità dei rowgroup per le prestazioni degli indici columnstore
...
Per altre informazioni sui gruppi di righe, vedere [Descrizione degli indici columnstore](/sql/relational-databases/indexes/columnstore-indexes-overview?view=azure-sqldw-latest&preserve-view=true).
...
Per altre informazioni sul caricamento bulk, vedere [Caricamento bulk in un indice columnstore cluster](/sql/relational-databases/indexes/columnstore-indexes-data-loading-guidance?view=azure-sqldw-latest#bulk&preserve-view=true).

## <a name="how-to-monitor-rowgroup-quality"></a>Come monitorare la qualità di un gruppo di righe

L'sys.dm_pdw_nodes_db_column_store_row_group_physical_stats DMV ([sys.dm_db_column_store_row_group_physical_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-column-store-row-group-physical-stats-transact-sql?view=azure-sqldw-latest&preserve-view=true) contiene la definizione della vista corrispondente al database SQL) che espone informazioni utili, ad esempio il numero di righe nei rowgroup e il motivo per cui è stato tagliato se è stato tagliato. Per effettuare una query su questa DMV allo scopo di ottenere informazioni sul trimming di un gruppo di righe, è possibile creare la vista seguente.
...
La memoria massima necessaria per comprimere un rowgroup è approssimativamente la seguente:
...
> [!NOTE]
> Dove short-string-columns usa tipi di dati stringa di <= 32 byte e long-string-columns usano tipi di dati stringa di > 32 byte.
...
Per una descrizione approfondita dei requisiti di memoria columnstore, vedere il video Synapse SQL [scalabilità: configurazione e indicazioni.](https://channel9.msdn.com/Events/Ignite/2016/BRK3291)
...
Ogni distribuzione comprime i rowgroup nel columnstore in parallelo quando sono disponibili più core CPU per ogni distribuzione. Il parallelismo richiede risorse di memoria aggiuntive che possono portare a richieste di memoria pesanti e al taglio del gruppo di righe.
...
Per trovare altri modi per migliorare le prestazioni in Synapse SQL, vedere Panoramica [delle prestazioni.](../overview-terminology.md)
