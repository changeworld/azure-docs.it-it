...
json_path = [espressione di percorso JSON](/sql/relational-databases/json/json-path-expressions-sql-server?view=sql-server-ver15) per una colonna o una proprietà annidata. La [modalità percorso](/sql/relational-databases/json/json-path-expressions-sql-server?view=sql-server-ver15#PATHMODE) predefinita è lax.
...
L'esempio seguente mostra come usare le [espressioni di percorso JSON](/sql/relational-databases/json/json-path-expressions-sql-server?view=sql-server-ver15) in una clausola WITH e illustra la differenza tra le modalità di percorso strict e lax: 
