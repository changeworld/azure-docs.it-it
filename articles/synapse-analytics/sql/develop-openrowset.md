...
json_path = [espressione di percorso JSON](/sql/relational-databases/json/json-path-expressions-sql-server?view=azure-sqldw-latest&preserve-view=true) per una colonna o una proprietà annidata. La [modalità percorso](/sql/relational-databases/json/json-path-expressions-sql-server?view=azure-sqldw-latest&preserve-view=true#PATHMODE) predefinita è lax.
...
- Formato supportato per il tipo di dati DATETIME2: YYYY-MM-DD HH:MM:SS[.secondi frazionari]
...
Specifica la tabella codici dei dati contenuti nel file di dati. Il valore predefinito è 65001 (codifica UTF-8). Per altre informazioni su questa opzione, vedere [qui](/sql/t-sql/functions/openrowset-transact-sql?view=sql-server-ver15#codepage).
...
L'esempio seguente mostra come usare le [espressioni di percorso JSON](/sql/relational-databases/json/json-path-expressions-sql-server?view=azure-sqldw-latest&preserve-view=true) in una clausola WITH e illustra la differenza tra le modalità di percorso strict e lax: 
