---
title: 'Guida introduttiva: usare Java e JDBC con il server flessibile database di Azure per PostgreSQL'
description: Questa Guida introduttiva illustra come usare Java e JDBC con un database di Azure per PostgreSQL flexible server.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "98605954"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>Guida introduttiva: usare Java e JDBC con il server flessibile database di Azure per PostgreSQL

Questo argomento illustra la creazione di un'applicazione di esempio che usa Java e [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) per archiviare e recuperare le informazioni nel [server flessibile database di Azure per PostgreSQL](./index.yml).
...
> [!NOTE]
> Per informazioni più dettagliate sulla creazione di server PostgreSQL, vedere [Creare un server di Database di Azure per PostgreSQL nel portale di Azure](./quickstart-create-server-portal.md).
...
Il server PostgreSQL creato in precedenza ha un database **Postgres** vuoto. Per questa Guida introduttiva si creerà un nuovo database denominato `demo` attenendosi alla procedura seguente:

1. Installare un'istanza locale di [PSQL](https://www.postgresql.org/docs/current/static/app-psql.html) per connettersi a un server PostgreSQL di Azure. 
2. Eseguire il comando psql seguente per connettersi al database **Postgres** nel server PostgreSQL
...
3. Creare un nuovo database denominato **demo** al prompt digitando il comando seguente:
