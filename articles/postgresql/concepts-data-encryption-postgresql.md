...
Per evitare problemi durante la configurazione della crittografia dei dati gestita dal cliente durante il ripristino o la creazione della replica di lettura, è importante seguire questa procedura nei server primario e ripristinato/di replica:

* Avviare il processo di ripristino o lettura della replica dal server singolo primario di Database di Azure per PostgreSQL.
...
Per Database di Azure per PostgreSQL, il supporto per la crittografia dei dati in stato di inquieto tramite la chiave gestita dei clienti presenta alcune limitazioni:
