...
I log di flusso sono l'origine di tutte le attività di rete nell'ambiente cloud. Che si sia un'avvio imminente che tenta di ottimizzare le risorse o che un'azienda di grandi dimensioni stia tentando di rilevare le intrusioni, i log di Flow sono la soluzione migliore. È possibile usarlo per ottimizzare i flussi di rete, monitorare la velocità effettiva, verificare la conformità, rilevare intrusioni e altro ancora.

## <a name="common-use-cases"></a>Casi d'uso comuni

**Monitoraggio di rete:** identificare il traffico sconosciuto o indesiderato. Monitorare i livelli di traffico e il consumo della larghezza di banda. Filtrare i log dei flussi in base all'INDIRIZZO IP e alla porta per comprendere il comportamento dell'applicazione. Esportare i log di flusso in strumenti di analisi e visualizzazione di propria scelta per configurare i dashboard di monitoraggio.

**Monitoraggio e ottimizzazione dell'utilizzo:** Identificare i principali talker nella rete. Combinare con i dati GeoIP per identificare il traffico tra aree. Informazioni sulla crescita del traffico per la previsione della capacità. Usare i dati per rimuovere regole di traffico estremamente restrittive.

**Conformità:** usare i dati di flusso per verificare l'isolamento della rete e la conformità con le regole di accesso aziendale

**Analisi forense & sicurezza:** analizzare i flussi di rete da IP e interfacce di rete compromessi. Esportare i log di flusso in qualsiasi strumento SIEM o IDS di propria scelta.

## <a name="how-logging-works"></a>Funzionamento della registrazione

**Proprietà chiave**

- I log dei flussi operano [al livello 4](https://en.wikipedia.org/wiki/OSI_model#Layer_4:_Transport_Layer) e registrano tutti i flussi IP in ingresso e in uscita da un gruppo di sicurezza di rete
...
- I log vengono scritti in formato JSON e mostrano i flussi in uscita e in ingresso in base alla regola del gruppo di protezione di rete.
- Ogni record di log contiene l'interfaccia di rete (NIC) a cui si applica il flusso, informazioni a 5 tuple, informazioni sulla velocità effettiva del & (solo versione 2). Per _informazioni dettagliate,_ vedere formato di log più avanti.
- I log dei flussi hanno una funzionalità di conservazione che consente di eliminare automaticamente i log fino a un anno dopo la creazione. 
...
- Le reti definite dal software sono organizzate in base alle reti virtuali (VNET) e alle subnet. La sicurezza di queste reti virtuali e subnet può essere gestita tramite un gruppo di sicurezza di rete.
...
- Il risultato di queste valutazioni sono i log dei flussi dei gruppi di sicurezza di rete. I log dei flussi vengono raccolti tramite la piattaforma Azure e non richiedono alcuna modifica alle risorse del cliente.
- Nota: le regole sono di due tipi, & terminazione, ognuna con comportamenti di registrazione diversi.
- - Le regole di negazione del gruppo di sicurezza di rete vengono terminate. Il gruppo di sicurezza di rete che nega il traffico lo registra nei log dei flussi e l'elaborazione in questo caso si arresta dopo che un gruppo di sicurezza di rete nega il traffico. 
- - Le regole consentite del gruppo di sicurezza di rete non sono di terminazione, il che significa che anche se un gruppo di sicurezza di rete lo consente, l'elaborazione continuerà al gruppo di sicurezza di rete successivo. L'ultimo gruppo di sicurezza di rete che consente il traffico registra il traffico ai log di Flow.
- I log dei flussi dei gruppi di rete vengono scritti in account di archiviazione da cui è possibile accedervi.
        ...
        * **rule**: regola per cui vengono elencati i flussi.
            * **flows**: raccolta di flussi.
                ...
                * **flowTuples**: stringa che contiene più proprietà per la tupla del flusso nel formato con valori separati da virgole.
                    ...
                    * **Pacchetti - Da origine a destinazione - Solo versione 2** Numero totale di pacchetti TCP inviati dall'origine alla destinazione dall'ultimo aggiornamento.
                    * **Byte inviati - Origine a destinazione - Solo versione 2** Numero totale di byte di pacchetti TCP inviati dall'origine alla destinazione dall'ultimo aggiornamento. I byte dei pacchetti includono l'intestazione del pacchetto e il payload.
                    * **Pacchetti - Da destinazione a origine - Solo versione 2** Numero totale di pacchetti TCP inviati dalla destinazione all'origine dall'ultimo aggiornamento.
                    * **Byte inviati - Destinazione all'origine - Solo versione 2** Numero totale di byte di pacchetti TCP inviati dalla destinazione all'origine dall'ultimo aggiornamento. I byte dei pacchetti includono payload e intestazione del pacchetto.
...
Per aggiornare i parametri tramite gli strumenti da riga di comando, usare lo stesso comando usato per abilitare i log dei flussi (da sopra) ma con i parametri aggiornati che si vuole modificare.
...
- [Esportare i log dei flussi del gruppo di protezione di rete in Splunk](https://www.splunk.com/en_us/blog/tips-and-tricks/splunking-microsoft-azure-network-watcher-data.html)
...
- [Analisi del traffico di Azure](./traffic-analytics.md) è un servizio nativo di Azure per elaborare i log dei flussi, estrarre informazioni dettagliate e visualizzare i log dei flussi. 
- [[Esercitazione] Visualizzare i log dei flussi del gruppo di Power BI](./network-watcher-visualize-nsg-flow-logs-power-bi.md)
- [[Esercitazione] Visualizzare i log dei flussi dei NSG con Elastic Stack](./network-watcher-visualize-nsg-flow-logs-open-source-tools.md)
- [[Esercitazione] Gestire e analizzare i log dei flussi dei gruppi di protezione di rete con Grafana](./network-watcher-nsg-grafana.md)
- [[Esercitazione] Gestire e analizzare i log dei flussi dei gruppi di protezione di rete con Graylog](./network-watcher-analyze-nsg-flow-logs-graylog.md)

*Disabilitare i log di flusso*

Quando il log di flusso è disabilitato, la registrazione del flusso per il gruppo di protezione di rete associato viene arrestata. Tuttavia, il log di flusso come risorsa continua a esistere con tutte le relative impostazioni e associazioni. Può essere abilitato in qualsiasi momento per iniziare la registrazione dei flussi nel gruppo di protezione di rete configurato. I passaggi per disabilitare/abilitare i log dei flussi sono disponibili in [questa guida.](./network-watcher-nsg-flow-logging-powershell.md)  

*Eliminare i log dei flussi*

Quando il log di flusso viene eliminato, non solo la registrazione del flusso per il gruppo di sicurezza di rete associato viene arrestata, ma anche la risorsa del log di flusso viene eliminata con le relative impostazioni e associazioni. Per iniziare di nuovo la registrazione del flusso, è necessario creare una nuova risorsa log di flusso per tale gruppo di protezione dei criteri di rete. Un log di flusso può essere eliminato usando [PowerShell, l'interfaccia](https://docs.microsoft.com/powershell/module/az.network/remove-aznetworkwatcherflowlog) [della riga di comando](https://docs.microsoft.com/cli/azure/network/watcher/flow-log#az_network_watcher_flow_log_delete) o [l'API REST.](https://docs.microsoft.com/rest/api/network-watcher/flowlogs/delete) Il supporto per l'eliminazione dei log di flusso portale di Azure è in pipeline.    
...
1. **Più schede di interfaccia di** rete in una macchina virtuale: nel caso in cui più schede di interfaccia di rete siano collegate a una macchina virtuale, la registrazione del flusso deve essere abilitata in tutte le macchine virtuali
1. **Avere un gruppo di** sicurezza di rete sia a livello di scheda di interfaccia di rete che a livello di subnet: nel caso in cui il gruppo di sicurezza di rete sia configurato sia a livello di scheda di interfaccia di rete che a livello di subnet, la registrazione del flusso deve essere abilitata in entrambi i NSG. 

**Provisioning dell'archiviazione:** il provisioning dell'archiviazione deve essere eseguito in modo ottimizzato con il volume del log di flusso previsto.

**Denominazione:** il nome del gruppo di criteri di rete deve essere fino a 80 caratteri e i nomi delle regole NSG devono essere fino a 65 caratteri. Se i nomi superano il limite di caratteri, potrebbe essere troncato durante la registrazione.
...
- In **Consenti accesso da** selezionare Reti  **selezionate**. Quindi,  **in Eccezioni** selezionare la casella accanto a ****Consenti alle servizi Microsoft attendibili di accedere a questo account di archiviazione****
- Se l'opzione è già selezionata, non occorre modificare niente.
- Individuare il gruppo di protezione di rete di destinazione nella pagina di panoramica [dei](https://ms.portal.azure.com/#blade/Microsoft_Azure_Network/NetworkWatcherMenuBlade/flowLogs) log dei flussi del gruppo di protezione di rete e abilitare i log dei flussi del gruppo di protezione di rete con l'account di archiviazione precedente selezionato.
...
**Ricerca per categorie usare i log dei flussi dei gruppi di protezione di rete con un account di archiviazione dietro un endpoint di servizio?**

I log dei flussi dei NSG sono compatibili con gli endpoint di servizio senza richiedere alcuna configurazione aggiuntiva. Vedere [l'esercitazione sull'abilitazione degli endpoint di](../virtual-network/tutorial-restrict-network-access-to-resources.md#enable-a-service-endpoint) servizio nella rete virtuale.

**Qual è la differenza tra le versioni dei log dei flussi 1 & 2?**

La versione 2 dei log dei flussi introduce il concetto _di_ stato del & archivia le informazioni sui byte e i pacchetti trasmessi. [Altre informazioni](#log-format)

## <a name="pricing"></a>Prezzi

I log dei flussi dei NSG vengono addebitati per GB di log raccolti e sono disponibili con un livello gratuito di 5 GB al mese per sottoscrizione. Per i prezzi correnti nella propria area, vedere la [pagina Network Watcher prezzi.](https://azure.microsoft.com/pricing/details/network-watcher/)

L'archiviazione dei log viene addebitata separatamente. [Vedere Archiviazione di Azure prezzi dei BLOB](https://azure.microsoft.com/pricing/details/storage/blobs/) in blocchi per i prezzi pertinenti.