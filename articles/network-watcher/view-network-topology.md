...
2. Creare un'istanza di Network Watcher con il comando [az network watcher configure](/cli/azure/network/watcher#az_network_watcher_configure). L'esempio seguente crea un'istanza di Network Watcher nell'area *eastus*:
...
3. Visualizzare la topologia con il comando [az network watcher show-topology](/cli/azure/network/watcher#az_network_watcher_show_topology). L'esempio seguente visualizza la topologia per un gruppo di risorse denominato *myResourceGroup*:
...
I comandi descritti nei passaggi seguenti possono essere eseguiti in due modi:
- In Azure Cloud Shell, selezionando **Prova** nell'angolo in alto a destra di ogni comando. Azure Cloud Shell è una shell interattiva gratuita in cui sono disponibili gli strumenti comuni di Azure preinstallati e configurati per l'uso con un account.
- Tramite PowerShell dal computer. Se si esegue PowerShell dal computer, questo articolo richiede il modulo `Az` Azure PowerShell. Eseguire `Get-Module -ListAvailable Az` per trovare la versione installata. Se è necessario eseguire l'aggiornamento, vedere [Installare e configurare Azure PowerShell](/powershell/azure/install-Az-ps). Se si esegue PowerShell in locale, è anche necessario eseguire `Connect-AzAccount` per creare una connessione con Azure.
...
3. Recuperare un'Network Watcher con [Get-AzNetworkWatcher](/powershell/module/az.network/get-aznetworkwatcher). L'esempio seguente recupera un'istanza di Network Watcher nell Stati Uniti orientali:
