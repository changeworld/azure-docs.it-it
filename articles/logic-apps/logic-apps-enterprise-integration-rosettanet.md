...
1. Per configurare il contratto per l'invio di messaggi al Partner Guest, selezionare **Invia impostazioni**.
   ...
   1. In **endpoint** specificare gli URL necessari da usare per l'invio di messaggi di azione e riconoscimenti.
      ...
      | Proprietà | Obbligatoria | Descrizione |
      |----------|----------|-------------|
      | **Action URL** |  Sì | URL da utilizzare per l'invio dei messaggi di azione. L'URL è un campo obbligatorio per i messaggi sincroni e asincroni. |
...
1. Fornire le informazioni per le proprietà dell'azione:
   ...
   ![Screenshot che mostra la posizione in cui vengono fornite le informazioni per le proprietà dell'azione.](media/logic-apps-enterprise-integration-rosettanet/decode-action-details.png)
