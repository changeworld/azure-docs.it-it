...
Per informazioni sulle restrizioni dei criteri di ripetizione, vedere [Limiti e configurazione per App per la logica](../logic-apps/logic-apps-limits-and-config.md#http-limits).
...
Per impostazione predefinita, quando tutte le azioni dell'ambito hanno esito positivo, lo stato dell'ambito è contrassegnato `Succeeded` . Se l'azione finale in un ambito risulta come `Failed` o `Aborted` , lo stato dell'ambito è contrassegnato come `Failed` .

Per intercettare le eccezioni in un `Failed` ambito ed eseguire azioni che gestiscono tali errori, è possibile usare la `runAfter` proprietà per tale `Failed` ambito. In questo modo, se *le azioni nell'* ambito hanno esito negativo e si usa la `runAfter` proprietà per tale ambito, è possibile creare una singola azione per rilevare gli errori.
...
1. Eseguire un' `For_each` azione di ciclo sugli output della *matrice filtrata* . Questo passaggio esegue un'azione per ogni risultato di azione non riuscita precedentemente filtrato.
   ...
   Se una singola azione nell'ambito ha esito negativo, le azioni nel `For_each` ciclo vengono eseguite una sola volta. Molte azioni non riuscite determinano un'azione per errore.

1. Inviare un HTTP POST sul `For_each` corpo della risposta dell'elemento, che è l' `@item()['outputs']['body']` espressione.
...
Come riferimento, di seguito è riportato un esempio di un singolo elemento `@result()`, che mostra le proprietà `name`, `body`, e `clientTrackingId` analizzate nell'esempio precedente. Al di fuori di un' `For_each` azione, `@result()` restituisce una matrice di questi oggetti.
...
Le espressioni descritte in precedenza in questo articolo possono essere usate per eseguire diversi modelli di gestione delle eccezioni. È possibile scegliere di eseguire una singola azione di gestione delle eccezioni al di fuori dell'ambito che accetta l'intera matrice filtrata di errori e di rimuovere l' `For_each` azione. È anche possibile includere altre proprietà utili della `\@result()` risposta, come descritto in precedenza.
