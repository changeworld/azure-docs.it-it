...
description: Informazioni sulla migrazione dei dati nella serie StorSimple 5000-7000 al dispositivo serie 8000 e sui prerequisiti per il processo di migrazione.
services: storsimple
...
| Vantaggi                                                                                                                                     |Svantaggi                                                                                                                                                              |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Il processo di migrazione mantiene la cronologia dei backup che sono stati eseguiti sul dispositivo serie 5000/7000.                                               | Quando gli utenti provano ad accedere ai dati, il processo di migrazione scarica i dati da Azure con il conseguente addebito dei costi di download.                                     |
| Non viene eseguita la migrazione di dati sul lato host.                                                                                                     | Questo processo comporta un tempo di inattività tra l'avvio del backup e il backup più recente visibile sul dispositivo serie 8000. Questo tempo può essere stimato tramite lo strumento di migrazione. |
| Questo processo consente di mantenere tutti i criteri, i modelli di larghezza di banda, la crittografia e le altre impostazioni nei dispositivi serie 8000.                      | Con l'accesso utente vengono riattivati solo i dati accessibili agli utenti e non l'intero set di dati.                                                  |
| Questo processo richiede più tempo per convertire tutti i backup precedenti in Azure. L'operazione viene eseguita in modo asincrono senza conseguenze sulla produzione. | La migrazione può essere eseguita solo al livello della configurazione cloud  e quindi non separatamente per i singoli volumi della configurazione.                       |
...
| Vantaggi                                                                                                                      |Svantaggi                                                                                                                                                                                                      |
|---------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| La migrazione può essere eseguita in maniera graduale, su un volume alla volta.                                               | I backup precedenti, creati sul dispositivo serie 5000/7000, non sono disponibili sul dispositivo serie 8000.                                                                                                       |
| Consente di consolidamento dei dati in un unico account di archiviazione in Azure.                                                       | Il primo backup nel cloud per il dispositivo serie 8000 richiede più tempo perché è necessario eseguire il backup di tutti i dati del dispositivo in Azure.                                                                     |
| Dopo la migrazione, tutti i dati sono archiviati in locale nell'appliance. Non si verificano problemi di latenza durante l'accesso ai dati. | L'utilizzo delle risorse di archiviazione di Azure aumenta finché i dati non vengono eliminati dal dispositivo serie 5000/7000.                                                                                                        |
|                                                                                                                           | Se il dispositivo della serie 7000/5000 contiene una grande quantità di dati, durante la migrazione questi dati devono essere scaricati da Azure, che comporteranno costi e latenze correlati al download dei dati da Azure |
...
Scaricare il documento con le istruzioni dettagliate su come [eseguire la migrazione da un dispositivo StorSimple serie 5000/7000 a un dispositivo serie 8000](https://databoxbuilds.blob.core.windows.net/storsimpledocs/MicrosoftAzureStorSimple_Migration_Guide_20171115.pdf).
