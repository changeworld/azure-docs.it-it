---
title: Usare la libreria client di Azure Batch per JavaScript
description: Informazioni sui concetti di base di Azure Batch e sulla creazione di una soluzione semplice mediante JavaScript.
ms.topic: how-to
ms.date: 01/01/2021
...
ms.locfileid: "106231482"
---
# <a name="get-started-with-batch-sdk-for-javascript"></a>Introduzione a batch SDK per JavaScript

Informazioni di base sulla creazione di un client batch in JavaScript usando [Azure batch JavaScript SDK](https://github.com/Azure/azure-sdk-for-js/). Viene adottato un approccio dettagliato per comprendere uno scenario per un'applicazione batch e quindi configurarlo utilizzando JavaScript.

## <a name="prerequisites"></a>Prerequisiti

In questo articolo si presuppone che l'utente abbia una conoscenza approfondita di JavaScript e della familiarità con Linux. Presuppone anche che abbia un account Azure configurato con i diritti di accesso necessari per creare servizi Batch e Archiviazione.
...
Il diagramma seguente illustra come è possibile ridimensionare lo script Python usando Azure Batch e un client.

![Diagramma che illustra l'architettura dello scenario.](./media/batch-js-get-started/batch-scenario.png)

L'esempio JavaScript distribuisce un processo batch con un'attività di preparazione (illustrata in dettaglio in seguito) e un set di attività a seconda del numero di contenitori nell'account di archiviazione. È possibile scaricare gli script dal repository GitHub.

- [Codice di esempio](https://github.com/Azure-Samples/azure-batch-samples/blob/master/JavaScript/Node.js/sample.js)
- [Script della shell per l'attività di preparazione](https://github.com/Azure-Samples/azure-batch-samples/blob/master/JavaScript/Node.js/startup_prereq.sh)
- [Processore Python per la conversione da CSV a JSON](https://github.com/Azure-Samples/azure-batch-samples/blob/master/JavaScript/Node.js/processcsv.py)
...
> - [Creare un'app per le funzioni](../azure-functions/functions-get-started.md)
> - [Creare una funzione trigger timer](../azure-functions/functions-bindings-timer.md)

## <a name="build-the-application"></a>Compilare l'applicazione

A questo punto, attenersi alla procedura dettagliata per la creazione del client JavaScript:
...
>[!Tip]
> In un'app per le funzioni di Azure, è possibile passare alla "console Kudu" nella scheda Impostazioni di Funzioni di Azure per eseguire i comandi npm install, In questo caso, installare Azure Batch SDK per JavaScript.

### <a name="step-2-create-an-azure-batch-account"></a>Passaggio 2: Creare un account Azure Batch

È possibile crearlo nel [portale di Azure](batch-account-create-portal.md) o dalla riga di comando ([Powershell](batch-powershell-cmdlets-get-started.md) /[interfaccia della riga di comando di Azure](/cli/azure)).
...
Il frammento di codice seguente importa innanzitutto il modulo JavaScript di Azure-batch e quindi crea un client del servizio batch. È necessario prima di tutto creare un oggetto SharedKeyCredentials con la chiave dell'account Batch copiata nel passaggio precedente.
...
![URI di Azure Batch](./media/batch-js-get-started/batch-uri.png)
...
È possibile caricare lo script in un account di archiviazione di Azure e generare un URI di firma di accesso condiviso per accedere allo script. Questo processo può essere automatizzato anche tramite Azure Storage JavaScript SDK.
...
Il portale offre visualizzazioni dettagliate delle attività e degli stati del processo. È anche possibile usare le funzioni list e Get in Azure JavaScript SDK. Per informazioni dettagliate, vedere la [documentazione](https://azure.github.io/azure-sdk-for-node/azure-batch/latest/Job.html).
...
- Vedere le informazioni di [riferimento su batch JavaScript](/javascript/api/overview/azure/batch) per esplorare l'API batch.