...
## <a name="agent-version-12000"></a>Versione dell'agente 12.0.0.0
Le note sulla versione seguenti sono relative alla versione 12.0.0.0 dell'agente di Sincronizzazione file di Azure (rilasciato il 26 marzo 2021).
...
- Nuova esperienza del portale per configurare i criteri di accesso alla rete e le connessioni agli endpoint privati
    - È ora possibile usare il portale per disabilitare l'accesso all'endpoint pubblico del servizio di sincronizzazione archiviazione e per approvare, rifiutare e rimuovere le connessioni agli endpoint privati. Per configurare i criteri di accesso alla rete e le connessioni agli endpoint privati, aprire il portale del servizio di sincronizzazione archiviazione, passare alla sezione Impostazioni e fare clic su rete.
 
- Supporto per la suddivisione in livelli nel cloud per dimensioni del cluster di volumi maggiori di 64KB
    - La suddivisione in livelli cloud supporta ora dimensioni del cluster di volumi fino a 2 MiB nel server 2019. Per altre informazioni, vedere [Qual è la dimensione minima dei file per un file a livello?](https://docs.microsoft.com/azure/storage/files/storage-sync-choose-cloud-tiering-policies#minimum-file-size-for-a-file-to-tier).
 
- Misurare la larghezza di banda e la latenza per Sincronizzazione file di Azure servizio e l'account di archiviazione
    - Il cmdlet Test-StorageSyncNetworkConnectivity ora può essere usato per misurare la latenza e la larghezza di banda per il servizio Sincronizzazione file di Azure e l'account di archiviazione. La latenza per il servizio Sincronizzazione file di Azure e l'account di archiviazione viene misurata per impostazione predefinita durante l'esecuzione del cmdlet.  Il caricamento e il download della larghezza di banda nell'account di archiviazione viene misurato quando si usa il parametro "-MeasureBandwidth".
...
- Messaggi di errore migliorati nel portale quando la creazione dell'endpoint server non riesce
    - Il feedback è stato migliorato e sono stati migliorati i messaggi di errore e le linee guida quando la creazione dell'endpoint server non riesce.
...
- Il riavvio è necessario per i server che dispongono di un'installazione di Sincronizzazione file di Azure Agent esistente.
