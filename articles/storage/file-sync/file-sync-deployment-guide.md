...
- Un endpoint server deve essere un percorso in un server registrato (anziché in una condivisione montata). L'archiviazione NAS (Network Attached Storage) non è supportata.
- Anche se l'endpoint server può essere nel volume di sistema, gli endpoint server nel volume di sistema potrebbero non usare la funzionalità di cloud a livelli.
...
- Un server registrato può supportare più endpoint server, tuttavia un gruppo di sincronizzazione può avere un solo endpoint server per ogni server registrato in un determinato momento. Altri endpoint server all'interno del gruppo di sincronizzazione devono essere in server registrati diversi.
