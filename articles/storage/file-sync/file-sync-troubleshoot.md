...
description: Risolvere i problemi comuni in una distribuzione Sincronizzazione file di Azure, che è possibile usare per trasformare Windows Server in una cache rapida della condivisione file di Azure.
author: jeffpatt24
ms.service: storage
...
- [Forum di archiviazione di Azure](/answers/products/azure?product=storage).
- [UserVoice per File di Azure](https://feedback.azure.com/forums/217298-storage/category/180670-files).
- Supporto tecnico Microsoft. Per creare una nuova richiesta di supporto, nel portale di Azure, nella scheda **Guida**, selezionare **Guida e supporto** e quindi selezionare **Nuova richiesta di supporto**.
...
<a id="agent-installation-gpo"></a>**L'installazione dell'agente ha esito negativo con errore: l'Installazione guidata di sincronizzazione archiviazione è stata terminata in modo prematuro a causa di un errore**
...
Questo problema si verifica perché ServerRegistration.exe solo le sottoscrizioni dai primi 5 Azure AD tenant. 
...
- Se **GetNextJob completato con lo stato: 0** è registrato, il server può comunicare con il servizio Sincronizzazione file di Azure. 
    - Aprire Gestione attività sul server e verificare che il processo di monitoraggio della sincronizzazione dell'archiviazione (AzureStorageSyncMonitor.exe) sia in esecuzione. Se il processo non è in esecuzione, provare a riavviare il server. Se il riavvio del server non risolve il problema, aggiornare all'ultima[versione dell'agente](file-sync-release-notes.md) della Sincronizzazione file di Azure. 

- Se GetNextJob completato con **stato: -2134347756,** il server non è in grado di comunicare con il servizio Sincronizzazione file di Azure a causa di una configurazione dell'ordine del pacchetto di crittografia TLS, proxy o firewall. 
    - Se il server si trova dietro un firewall, verificare che la porta 443 in uscita sia consentita. Se il firewall limita il traffico a domini specifici, verificare che i domini elencati nella [documentazione](file-sync-firewall-and-proxy.md#firewall) del firewall siano accessibili.
    - Se il server si trova dietro un proxy, configurare le impostazioni del proxy a livello di computer o specifiche dell'app seguendo la procedura descritta nella [documentazione](file-sync-firewall-and-proxy.md#proxy) del proxy.
    - Usare il cmdlet Test-StorageSyncNetworkConnectivity per verificare la connettività di rete agli endpoint del servizio. Per altre informazioni, vedere [Testare la connettività di rete agli endpoint di servizio](file-sync-firewall-and-proxy.md#test-network-connectivity-to-service-endpoints).
    - Se l'ordine dei pacchetti di crittografia TLS è configurato nel server, è possibile usare i criteri di gruppo o i cmdlet TLS per aggiungere pacchetti di crittografia:
...
| 0x80c86044 | -2134351804 | ECS_E_AZURE_AUTHORIZATION_FAILED | Il file non può essere sincronizzato perché le impostazioni del firewall e della rete virtuale nell'account di archiviazione sono abilitate e il server non ha accesso all'account di archiviazione. | Aggiungere l'indirizzo IP del server o la rete virtuale seguendo i passaggi descritti nella sezione [Configurare le impostazioni del firewall e della rete virtuale](file-sync-deployment-guide.md?tabs=azure-portal#configure-firewall-and-virtual-network-settings) nella Guida alla distribuzione. |
...
> [!Note]  
> È possibile usare lo [strumento di valutazione](file-sync-planning.md#evaluation-cmdlet) per identificare i caratteri non supportati. Se il set di dati contiene più file con caratteri non validi, usare lo script [ScanUnsupportedChars](https://github.com/Azure-Samples/azure-files-samples/tree/master/ScanUnsupportedChars) per rinominare i file che contengono caratteri non supportati.
...
|---------------|-----------------|
| 0x00000000 - 0x0000001F (caratteri di controllo) | 32 |
| 0x0000FDD0 - 0x0000FDDD (forme di presentazione araba-a) | 14 |
| <ul><li>0x00000022 (virgolette)</li><li>0x0000002A (asterisco)</li><li>0x0000002F (barra)</li><li>0x0000003A (due punti)</li><li>0x0000003C (minore di)</li><li>0x0000003E (maggiore di)</li><li>0x0000003F (punto interrogativo)</li><li>0x0000005C (barra rovesciata)</li><li>0x0000007C (barra o pipe)</li></ul> | 9 |
| <ul><li>0x0004FFFE - 0x0004FFFF = 2 (non carattere)</li><li>0x0008FFFE - 0x0008FFFF = 2 (non carattere)</li><li>0x000CFFFE - 0x000CFFFF = 2 (non carattere)</li><li>0x0010FFFE - 0x0010FFFF = 2 (non carattere)</li></ul> | 8 |
...
| 0x0000FFF0, 0x0000FFFD, 0x0000FFFE, 0x0000FFFF (speciali) | 4 |
| File o directory che terminano con un punto | 1 |
...
4. [Verificare che le impostazioni del firewall e della rete virtuale nell'account di archiviazione siano configurate correttamente (se abilitate)](file-sync-deployment-guide.md?tabs=azure-portal#configure-firewall-and-virtual-network-settings)
...
1. [Verificare l'esistenza dell'account di archiviazione.](#troubleshoot-storage-account)
2. [Garantire l'esistenza della condivisione file di Azure.](#troubleshoot-azure-file-share)
3. [Verificare che le impostazioni del firewall e della rete virtuale nell'account di archiviazione siano configurate correttamente (se abilitate)](file-sync-deployment-guide.md?tabs=azure-portal#configure-firewall-and-virtual-network-settings)
...
1. [Verificare l'esistenza dell'account di archiviazione.](#troubleshoot-storage-account)
2. [Verificare che le impostazioni del firewall e della rete virtuale nell'account di archiviazione siano configurate correttamente (se abilitate)](file-sync-deployment-guide.md?tabs=azure-portal#configure-firewall-and-virtual-network-settings)
...
7. Verificare che l'**Utilizzo** si trovi sotto a **Quota**. Nota: a meno che non sia stata specificata una quota alternativa, la quota corrisponderà alle [dimensioni massime della condivisione file di Azure](../files/storage-files-scale-targets.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json).
...
Le sessioni di sincronizzazione hanno esito negativo con uno di questi errori quando sono presenti molti file che non riescono a eseguire la sincronizzazione con errori per elemento. Eseguire i passaggi documentati nella sezione Ricerca per categorie verificare se sono presenti file o cartelle specifici che non sono [sincronizzati?](?tabs=portal1%252cazure-portal#how-do-i-see-if-there-are-specific-files-or-folders-that-are-not-syncing) per risolvere gli errori per ogni elemento. Per l'errore di ECS_E_SYNC_METADATA_KNOWLEDGE_LIMIT_REACHED, aprire un caso di supporto.
...
1. Rimuovere l'endpoint server nel gruppo di sincronizzazione seguendo la procedura descritta in [Rimuovere un endpoint server](file-sync-server-endpoint.md#remove-a-server-endpoint).
2. Creare un nuovo endpoint server nel gruppo di sincronizzazione seguendo la procedura descritta in [Aggiungere un endpoint server](file-sync-server-endpoint.md#add-a-server-endpoint).
...
| 0x80070005 | -2147024891 | ERROR_ACCESS_DENIED | Impossibile richiamare il file a causa di un errore di accesso negato. Questo problema può verificarsi se le impostazioni del firewall e della rete virtuale nell'account di archiviazione sono abilitate e il server non ha accesso all'account di archiviazione. | Per risolvere il problema, aggiungere l'indirizzo IP del server o la rete virtuale seguendo i passaggi descritti nella sezione [Configurare le impostazioni del firewall e della rete virtuale](file-sync-deployment-guide.md?tabs=azure-portal#configure-firewall-and-virtual-network-settings) nella Guida alla distribuzione. |
...
- Per impedire la creazione di file a livelli orfani in futuro, seguire i passaggi descritti in [Rimuovere un endpoint server](file-sync-server-endpoint.md#remove-a-server-endpoint) quando si elimina un endpoint server.
...
3. Rimuovere l'endpoint server nel gruppo di sincronizzazione (se esiste) attenendosi ai passaggi descritti in [Rimuovere un endpoint server](file-sync-server-endpoint.md#remove-a-server-endpoint).
...
1. [Montare](../files/storage-how-to-use-files-windows.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json) la condivisione file di Azure sul server Windows che contiene file a livelli orfani.
2. Eseguire i comandi di PowerShell seguenti per elencare i file a livelli orfani:
```powershell
Import-Module "C:\Program Files\Azure\StorageSyncAgent\StorageSync.Management.ServerCmdlets.dll"
$orphanFiles = Get-StorageSyncOrphanedTieredFiles -path <server endpoint path>
...
- [Risolvere i problemi di File di Azure in Windows](../files/storage-troubleshoot-windows-file-connection-problems.md)
- [Risolvere i problemi di File di Azure in Linux](../files/storage-troubleshoot-linux-file-connection-problems.md)