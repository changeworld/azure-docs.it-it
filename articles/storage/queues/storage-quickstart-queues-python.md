---
title: 'Avvio rapido: Libreria client di Archiviazione code di Azure v12 - Python'
description: Informazioni su come usare la libreria client di Archiviazione code di Azure v12 per Python per creare una coda e aggiungervi messaggi, oltre che su come leggere ed eliminare i messaggi dalla coda. e come eliminare una coda.
...
- [Python](https://www.python.org/downloads/) per il sistema operativo-2,7 o 3.6 +

## <a name="setting-up"></a>Configurazione

Questa sezione illustra la procedura per preparare un progetto da usare con la libreria client di Archiviazione code di Azure v12 per Python.
...
Installare il pacchetto della libreria client di Archiviazione code di Azure per Python usando il comando `pip install`.
...
1. Salvare il nuovo file come `queues-quickstart-v12.py` nella directory `queues-quickstart-v12`.
...
Creare un'istanza della classe [`QueueClient`](/python/api/azure-storage-queue/azure.storage.queue.queueclient). Chiamare quindi il metodo [`create_queue`](/python/api/azure-storage-queue/azure.storage.queue.queueclient#create-queue---kwargs-) per creare la coda nell'account di archiviazione.
...
Il frammento di codice seguente aggiunge messaggi alla coda chiamando il metodo [`send_message`](/python/api/azure-storage-queue/azure.storage.queue.queueclient#send-message-content----kwargs-). Salva anche il [`QueueMessage`](/python/api/azure-storage-queue/azure.storage.queue.queuemessage) restituito dalla terza chiamata a `send_message`. Il `saved_message` viene usato per aggiornare il contenuto del messaggio in un secondo momento nel programma.
...
Per visualizzare in anteprima i messaggi nella coda, chiamare il metodo [`peek_messages`](/python/api/azure-storage-queue/azure.storage.queue.queueclient#peek-messages-max-messages-none----kwargs-). Questo metodo recupera uno o più messaggi dall'inizio della coda, senza modificare la visibilità del messaggio.
...
Scaricare i messaggi aggiunti precedentemente chiamando il metodo [`receive_messages`](/python/api/azure-storage-queue/azure.storage.queue.queueclient#receive-messages---kwargs-).
...
Il codice seguente pulisce le risorse create dall'app eliminando la coda tramite il metodo [`delete_queue`](/python/api/azure-storage-queue/azure.storage.queue.queueclient#delete-queue---kwargs-).
...
- Per altre app di esempio di Archiviazione code di Azure, vedere gli [esempi della libreria client di Archiviazione code di Azure v12 per Python](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/storage/azure-storage-queue/samples).
