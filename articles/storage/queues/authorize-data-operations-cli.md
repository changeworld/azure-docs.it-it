...
1. Chiamare il [`az storage queue create`](/cli/azure/storage/queue#az-storage-queue-create) comando con il `--auth-mode` parametro impostato su `login` per creare la coda usando le credenziali Azure ad. È necessario ricordare di sostituire i valori segnaposto tra parentesi uncinate con i valori personalizzati:
