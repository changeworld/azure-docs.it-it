...
description: Informazioni su come creare una firma di accesso condiviso (SAS) del servizio per un contenitore o un BLOB usando la libreria client di archiviazione BLOB di Azure.
services: storage
author: tamram
ms.service: storage
ms.topic: how-to
...
Una firma di accesso condiviso del servizio è firmata con la chiave di accesso dell'account. Utilizzare la classe [StorageSharedKeyCredential](/javascript/api/@azure/storage-blob/storagesharedkeycredential) per creare le credenziali utilizzate per firmare la firma di accesso condiviso. Chiamare quindi la funzione [generateBlobSASQueryParameters](/javascript/api/@azure/storage-blob/#generateBlobSASQueryParameters_BlobSASSignatureValues__StorageSharedKeyCredential_) specificando i parametri obbligatori per ottenere la stringa del token SAS.
...
Per creare una firma di accesso condiviso del servizio per un BLOB, chiamare la funzione [generateBlobSASQueryParameters](/javascript/api/@azure/storage-blob/#generateBlobSASQueryParameters_BlobSASSignatureValues__StorageSharedKeyCredential_) fornendo i parametri richiesti.
