...
description: Simulare un errore durante la lettura dei dati dall'area primaria quando l'account di archiviazione è configurato per la ridondanza geografica della zona con accesso in lettura (RA-GZRS).
services: storage
author: tamram
ms.service: storage
ms.subservice: blobs
...
Questa è la seconda di una serie di esercitazioni. Illustra i vantaggi dell'[archiviazione con ridondanza geografica della zona e accesso in lettura](../common/storage-redundancy.md) (RA-GZRS) simulando un errore.

Per simulare un errore, è possibile usare il [routing statico](#simulate-a-failure-with-an-invalid-static-route) o [Fiddler](#simulate-a-failure-with-fiddler). Entrambi i metodi consentono di simulare un errore per le richieste all'endpoint primario di un account di archiviazione con [ridondanza geografica della zona e accesso in lettura](../common/storage-redundancy.md) (RA-GZRS) e fare in modo che l'applicazione esegua invece la lettura dall'endpoint secondario.
...
![Personalizzare le regole di Fiddler](media/simulate-primary-region-failure/figure1.png)
...
![Incollare la regola personalizzata](media/simulate-primary-region-failure/figure2.png)
...
Per altre informazioni sul funzionamento dell'archiviazione con ridondanza geografica della zona e accesso in lettura e sui rischi associati, vedere l'articolo seguente:

> [!div class="nextstepaction"]
> [Progettazione di applicazioni a disponibilità elevata con RA-GZRS](../common/geo-redundant-design.md)
