...
> [!NOTE]
> I criteri password sono disponibili solo per i domini gestiti creati usando il modello di distribuzione Gestione risorse. Per i domini gestiti meno recenti creati con la distribuzione classica, [eseguire la migrazione dal modello di rete virtuale classica a gestione risorse][migrate-from-classic].
...
* Un dominio gestito di Azure Active Directory Domain Services abilitato e configurato nel tenant di Azure AD.
  ...
  * Il dominio gestito deve essere stato creato con il modello di distribuzione Gestione risorse. Se necessario, [eseguire la migrazione dal modello di rete virtuale classica a gestione risorse][migrate-from-classic].
...
I criteri vengono distribuiti tramite l'associazione di gruppo in un dominio gestito e tutte le modifiche apportate vengono applicate al successivo accesso dell'utente. La modifica dei criteri non comporta lo sblocco di un account utente già bloccato.
...
I criteri password personalizzati vengono applicati ai gruppi in un dominio gestito. Questa configurazione sostituisce in modo efficace i criteri predefiniti.
...
> [!NOTE]
> Per creare un criterio personalizzato per le password in un dominio gestito, è necessario avere eseguito l'accesso a un account utente membro del gruppo di *amministratori di AAD DC* .
...
1. Nel riquadro sinistro scegliere il dominio gestito, ad esempio *aaddscontoso.com*.
1. Aprire il contenitore di **sistema** , quindi il **contenitore Impostazioni password**.

    Viene visualizzato un criterio predefinito per le password per il dominio gestito. Non è possibile modificare questo criterio predefinito. In alternativa, creare un criterio password personalizzato per sostituire i criteri predefiniti.
...
1. Modificare le altre impostazioni dei criteri password nel modo desiderato. Tenere presente i seguenti punti chiave:
    ...
    * Impostazioni come la complessità della password, l'età o la scadenza solo per gli utenti creati manualmente in un dominio gestito.
    * Le impostazioni di blocco degli account si applicano a tutti gli utenti, ma hanno effetto solo all'interno del dominio gestito e non in Azure AD stesso.
    ...
    ![Creare criteri specifici per le password personalizzati](./media/password-policy/custom-fgpp.png)
