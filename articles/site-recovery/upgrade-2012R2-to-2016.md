...
- Se si usa System Center 2012 R2 VMM: 
    ...
    - Annotare l'ID di VMM di entrambi i server VMM primario 2012 R2 e di ripristino. È possibile trovare l'ID VMM dal registro di sistema "HKLM: \ SOFTWARE\Microsoft\Microsoft System Center Virtual Machine Manager Server\Setup".
...
> [!IMPORTANT]
> Fare riferimento alla documentazione dettagliata di SCVMM per i [prerequisiti](/system-center/vmm/upgrade-vmm?view=sc-vmm-2016&preserve-view=true#requirements-and-limitations).
...
5. Passare alla portale di Azure e verificare lo stato di integrità replicato all'interno dei servizi di ripristino
...
2. [Mantenere il database SCVMM e aggiornare il sistema operativo](/system-center/vmm/upgrade-vmm?view=sc-vmm-2016&preserve-view=true#back-up-and-upgrade-the-operating-system)
...
4. [Installare VMM 2016](/system-center/vmm/upgrade-vmm?view=sc-vmm-2016&preserve-view=true#install-vmm-2016)
5. Avviare SCVMM e controllare lo stato di ogni host nella scheda **infrastrutture** . Fare clic su **Aggiorna** per ottenere lo stato più recente. Verrà visualizzato lo stato "richiesta attenzione". 
...
4. Passare alla portale di Azure e verificare lo stato di integrità replicato delle macchine virtuali all'interno dell'insieme di credenziali di servizi di ripristino. 
...
1.  Disinstallare il provider di Azure Site Recovery accedendo a Pannello di controllo -> Programmi -> Programmi e funzionalità ->Microsoft Azure Site Recovery e quindi fare clic su Disinstalla.
2. Seguire la procedura descritta [qui](/system-center/vmm/upgrade-vmm?view=sc-vmm-2016&preserve-view=true#upgrade-a-standalone-vmm-server) in base alla modalità di aggiornamento scelta.
3. Avviare la console SCVMM e controllare lo stato di ogni host nella scheda **infrastrutture** . Fare clic su **Aggiorna** per ottenere lo stato più recente. Verrà visualizzato lo stato "richiesta attenzione".
