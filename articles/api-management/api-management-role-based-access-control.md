...
| API Management lettore del servizio | ✓ | | || Dispone di accesso in sola lettura a servizi ed entità Gestione API. |
| API Management operatore di servizio | ✓ | | ✓ | | Può gestire i servizi di Gestione API, ma non le entità.|
| API Management Service Editor<sup>*</sup> | ✓ | ✓ | |  | Può gestire le entità di Gestione API, ma non i servizi.|
| API Management Content Manager<sup>*</sup> | ✓ | | | ✓ | Può gestire il portale per sviluppatori. Accesso in sola lettura a servizi ed entità.|
...
<sup>\* Il ruolo Editor dei servizi sarà disponibile dopo la migrazione di tutta l'interfaccia utente di amministrazione dal portale di pubblicazione esistente alla portale di Azure. Il ruolo Gestione contenuto sarà disponibile dopo il refactoring del portale editore per contenere solo le funzionalità correlate alla gestione del portale per sviluppatori.</sup>  
...
> [!NOTE]
> Per poter visualizzare un'istanza API Management nel portale di Azure, un ruolo personalizzato deve includere ```Microsoft.ApiManagement/service/read``` l'azione .

Quando si crea un ruolo personalizzato, è più semplice iniziare con uno dei ruoli predefiniti. Modificare gli attributi per aggiungere **Actions**, **NotActions** o **AssignableScopes** e quindi salvare le modifiche come nuovo ruolo. L'esempio seguente inizia con il ruolo "API Management Service Reader" e crea un ruolo personalizzato denominato "Calculator API Editor". È possibile assegnare il ruolo personalizzato a un'API specifica. Di conseguenza, questo ruolo può accedere solo a tale API. 
