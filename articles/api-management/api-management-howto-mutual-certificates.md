...
È anche possibile gestire i certificati API Management usando l API Management API [REST](/rest/api/apimanagement/2020-06-01-preview/certificate).
...
* È [possibile applicare criteri di](../key-vault/general/security-features.md#privileged-access) accesso granulari ai certificati archiviati negli insiemi di credenziali delle chiavi
...
1. Abilitare un'identità gestita assegnata [](api-management-howto-use-managed-service-identity.md) dal sistema o dall'utente nell'API Management locale.
...
3. In **Credenziali gateway** selezionare Certificato client **e** selezionare il certificato dall'elenco a discesa.
1. Selezionare **Salva**.
...
> [!CAUTION]
> Questa modifica ha effetto immediato e le chiamate alle operazioni di tale API useranno il certificato per l'autenticazione nel server back-end.
...
Se si usano certificati autofirmati, è necessario disabilitare la convalida della catena di certificati per API Management comunicare con il sistema back-end. In caso contrario, verrà restituito un codice di Errore 500. Per configurare questa funzionalità, è possibile usare i cmdlet di PowerShell (per il nuovo [`New-AzApiManagementBackend`](/powershell/module/az.apimanagement/new-azapimanagementbackend) back-end) o (per il [`Set-AzApiManagementBackend`](/powershell/module/az.apimanagement/set-azapimanagementbackend) back-end esistente) e impostare il `-SkipCertificateChainValidation` parametro su `True` .
...
> [!IMPORTANT]
> Se i criteri fanno riferimento al certificato, viene visualizzata una schermata di avviso. Per eliminare il certificato, è innanzitutto necessario rimuoverlo da tutti i criteri configurati per usarlo.
