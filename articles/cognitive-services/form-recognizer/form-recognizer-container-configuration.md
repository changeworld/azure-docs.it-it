...
Usare montaggi di associazione per leggere e scrivere dati da e verso il contenitore. È possibile specificare un montaggio di input o un montaggio di output specificando l' `--mount` opzione nel [ `docker run` comando](https://docs.docker.com/engine/reference/commandline/run/).
...
|Necessario| `Output` | string | Destinazione del montaggio di output. Il valore predefinito è `/output`.  <br><br>Esempio:<br>`--mount type=bind,src=c:\output,target=/output`|
