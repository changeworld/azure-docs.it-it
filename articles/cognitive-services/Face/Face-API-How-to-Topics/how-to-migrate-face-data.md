---
title: Eseguire la migrazione dei dati viso tra sottoscrizioni-Face
...
- Due chiavi di sottoscrizione del viso, una con i dati esistenti e l'altra per la migrazione. Per sottoscrivere il servizio Face e ottenere la chiave, seguire le istruzioni in [creare un account di servizi cognitivi](../../cognitive-services-apis-create-account.md).
- Stringa dell'ID di sottoscrizione della superficie che corrisponde alla sottoscrizione di destinazione. Per individuarlo, selezionare **Panoramica** nella portale di Azure. 
- Qualsiasi edizione di [Visual Studio 2015 o 2017](https://www.visualstudio.com/downloads/).

## <a name="create-the-visual-studio-project"></a>Creare il progetto di Visual Studio

Questa guida usa una semplice app console per eseguire la migrazione dei dati per i volti. Per un'implementazione completa, vedere l' [esempio di snapshot viso](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/app-samples/FaceApiSnapshotSample/FaceApiSnapshotSample) su GitHub.
...
È necessario l'ID del PersonGroup nella sottoscrizione di origine per eseguirne la migrazione nella sottoscrizione di destinazione. Usare il metodo [PersonGroupOperationsExtensions. ListAsync](/dotnet/api/microsoft.azure.cognitiveservices.vision.face.persongroupoperationsextensions.listasync) per recuperare un elenco di oggetti gruppo. Ottenere quindi la proprietà [gruppo. PersonGroupId](/dotnet/api/microsoft.azure.cognitiveservices.vision.face.models.persongroup.persongroupid#Microsoft_Azure_CognitiveServices_Vision_Face_Models_PersonGroup_PersonGroupId) . Questo processo ha un aspetto diverso a seconda degli oggetti gruppo. In questa guida, l'ID gruppo di origine viene archiviato in `personGroupId` .
...
Usare l'istanza di FaceClient della sottoscrizione di origine per creare uno snapshot di gruppo. Usare [TakeAsync](/dotnet/api/microsoft.azure.cognitiveservices.vision.face.snapshotoperationsextensions.takeasync) con l'ID gruppo e l'ID della sottoscrizione di destinazione. Se si dispone di più sottoscrizioni di destinazione, aggiungerle come voci di matrice nel terzo parametro.
...
> [!NOTE]
> Il processo di acquisizione e applicazione degli snapshot non interrompe alcuna chiamata normale all'origine o alla destinazione persone o visi. Non eseguire chiamate simultanee che modificano l'oggetto di origine, ad esempio le [chiamate di gestione facet](/dotnet/api/microsoft.azure.cognitiveservices.vision.face.facelistoperations) o la chiamata di [Training gruppo](/dotnet/api/microsoft.azure.cognitiveservices.vision.face.persongroupoperations) . L'operazione di snapshot può essere eseguita prima o dopo queste operazioni o potrebbe verificarsi errori.
...
- [Documentazione di riferimento sugli snapshot (.NET SDK)](/dotnet/api/microsoft.azure.cognitiveservices.vision.face.snapshotoperations)
- [Esempio di snapshot viso](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/app-samples/FaceApiSnapshotSample/FaceApiSnapshotSample)
