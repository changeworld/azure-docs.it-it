---
title: 'Avvio rapido: Libreria client del servizio Viso per Go'
description: Usare la libreria client di Viso per Go per rilevare visi, trovare volti simili (ricerca di volti per immagine), identificare i visi (ricerca basata su riconoscimento facciale) ed eseguire la migrazione dei dati sui visi.
...
* Dopo aver ottenuto una chiave e un endpoint, [creare le variabili di ambiente](../../../cognitive-services-apis-create-account.md#configure-an-environment-variable-for-authentication) per la chiave e l'endpoint, denominate rispettivamente `FACE_SUBSCRIPTION_KEY` e `FACE_ENDPOINT`.
...
Le classi e le interfacce seguenti gestiscono alcune delle principali funzionalità della libreria client del servizio Viso per Go.
...
> [!NOTE] 
> Questo argomento di avvio rapido presuppone che siano state [create variabili di ambiente](../../../cognitive-services-apis-create-account.md#configure-an-environment-variable-for-authentication) per la chiave e l'endpoint del servizio Viso, denominate rispettivamente `FACE_SUBSCRIPTION_KEY` e `FACE_ENDPOINT`.
...
Il codice seguente cerca nella radice del progetto l'immagine _test-image-person-group.jpg_ e la carica nella memoria del programma. È possibile trovare questa immagine nello stesso repository delle immagini usate in [creare ed eseguire il training di un gruppo](#create-and-train-a-persongroup): https://github.com/Azure-Samples/cognitive-services-sample-data-files/tree/master/Face/images .
...
Eseguire l'app di riconoscimento volto dalla directory dell'applicazione con il comando `go run <app-name>`.
...
Se è stato creato un oggetto **PersonGroup** in questo argomento di avvio rapido e si vuole eliminarlo, chiamare il metodo **[Delete](https://godoc.org/github.com/Azure/azure-sdk-for-go/services/cognitiveservices/v1.0/face#PersonGroupClient.Delete)** .

## <a name="next-steps"></a>Passaggi successivi

In questo argomento di avvio rapido si è appreso come usare la libreria client di Viso per Go per eseguire attività di riconoscimento facciale di base. Successivamente, esplorare la documentazione di riferimento per altre informazioni sulla libreria.
