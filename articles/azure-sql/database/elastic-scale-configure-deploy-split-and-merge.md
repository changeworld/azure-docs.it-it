...
Lo strumento di divisione e unione sposta i dati tra database partizionati. Vedere [Spostamento di dati tra database cloud con numero maggiore di istanze](elastic-scale-overview-split-and-merge.md)
...
Per istruzioni dettagliate sulla configurazione della sicurezza del servizio, vedere l'articolo [Configurazione di sicurezza per suddivisione-unione](elastic-scale-split-merge-security-configuration.md)
...
Si noti che per distribuzioni destinate alla produzione è necessario usare certificati separati per CA, crittografia, server e client. Per istruzioni dettagliate, vedere l'articolo relativo alla [configurazione della sicurezza](elastic-scale-split-merge-security-configuration.md).
...
Se la messa in linea del proprio ruolo Web non riesce, è probabile che si tratti di un problema relativo alla configurazione della sicurezza. Verificare che TLS/SSL sia configurato come descritto in precedenza.
...
2. Creare un server (o scegliere un server esistente) in cui verranno creati il gestore delle mappe partizioni e le partizioni.
   ...
   Verificare che il server sia configurato per consentire l'accesso dall'indirizzo IP del computer che esegue questi script. È possibile trovare questa impostazione in SQL Server/firewall e reti virtuali/indirizzi IP del client.

3. Eseguire lo script di *SetupSampleSplitMergeEnvironment.ps1* per creare l'ambiente di esempio.
...
5. Eseguire lo script di *ExecuteSampleSplitMerge.ps1* per eseguire un'operazione Split (spostando la metà dei dati sulla prima partizione nella seconda partizione) e quindi un'operazione di Unione (spostando i dati di nuovo nella prima partizione). Se TLS è stato configurato e l'endpoint HTTP è stato disabilitato, assicurarsi di usare invece l'endpoint https://.
...
Quando si eseguono gli script di PowerShell di esempio, è possibile che venga visualizzato il messaggio seguente:
