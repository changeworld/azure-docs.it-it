---
title: Ripristinare un database in un'app SaaS multi-tenant
description: Informazioni su come ripristinare un database SQL di Azure a tenant singolo dopo l'eliminazione accidentale di dati
...
Se il database usa la [replica geografica attiva](active-geo-replication-overview.md) e si esegue il ripristino in parallelo, è consigliabile copiare i dati necessari dalla copia ripristinata nel database originale. Se il database originale viene sostituito con il database ripristinato, è necessario riconfigurare e risincronizzare la replica geografica.
...
* [Panoramica della continuità aziendale del database SQL di Azure](business-continuity-high-availability-disaster-recover-hadr-overview.md)
* [Informazioni sul backup del database SQL](automated-backups-overview.md)