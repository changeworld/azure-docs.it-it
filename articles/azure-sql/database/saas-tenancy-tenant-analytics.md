...
Questa esercitazione presenta uno scenario di analisi completo per l'applicazione SaaS Wingtip Tickets. Prima viene usato *Processi elastici* per estrarre i dati da ogni database tenant e caricarli nelle tabelle di staging in un archivio di analitica, che può essere un database SQL o un pool SQL dedicato. Per l'estrazione di dati su larga scala, è consigliabile usare [Azure Data Factory](../../data-factory/introduction.md).
...
Infine, viene eseguita una query sull'archivio di analisi usando **Power BI** per evidenziare le informazioni dettagliate sul comportamento dei tenant e sull'uso dell'applicazione Wingtip Tickets. Eseguire query che:
