...
Il database SQL di Azure raggiunge questi obiettivi usando più meccanismi di governance delle risorse, inclusi [gli oggetti processo](/windows/win32/procthread/job-objects) di Windows per la governance delle risorse a livello di processo, il [file Server di Windows Gestione risorse (FSRM)](/windows-server/storage/fsrm/fsrm-overview) per la gestione delle quote di archiviazione e una versione modificata ed estesa di SQL Server [Resource Governor](/sql/relational-databases/resource-governor/resource-governor) per implementare la governance delle risorse nel database SQL.
...
- Per un'introduzione ai pool elastici, vedere [pool elastici per gestire e ridimensionare più database nel database SQL di Azure](./elastic-pool-overview.md).
- Per informazioni sull'ottimizzazione dei carichi di lavoro di query per ridurre l'utilizzo delle risorse, vedere [monitoraggio e ottimizzazione]( https://docs.microsoft.com/azure/sql-database/sql-database-monitoring-tuning-index), [monitoraggio e ottimizzazione delle prestazioni](./monitor-tune-overview.md).