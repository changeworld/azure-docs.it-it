---
title: Configurare DNN per l'istanza del cluster di failover
...
Nelle macchine virtuali di Azure il nome di rete distribuita (DNN) instrada il traffico alla risorsa cluster appropriata. Fornisce un modo più semplice per connettersi all'istanza del cluster di failover di SQL Server (FCI) rispetto al nome della rete virtuale (VNN), senza la necessità di un Azure Load Balancer. 
...
La funzionalità DNN è attualmente disponibile solo in SQL Server 2019 CU2 e versioni successive in Windows Server 2016 e versioni successive. 

Per un'opzione di connettività alternativa, considerare invece un [nome di rete virtuale e Azure Load Balancer](failover-cluster-instance-vnn-azure-load-balancer-configure.md) . 

## <a name="overview"></a>Panoramica

Il nome di rete distribuita (DNN) sostituisce il nome della rete virtuale (VNN) come punto di connessione quando viene usato con un' [istanza del cluster di failover di always on in SQL Server macchine virtuali](failover-cluster-instance-overview.md). Questa operazione nega la necessità di un Azure Load Balancer il routing del traffico al VNN, semplificando la distribuzione, la manutenzione e il miglioramento del failover. 

Con una distribuzione FCI, il VNN esiste ancora, ma il client si connette al nome DNS DNN anziché al nome del VNN. 

## <a name="prerequisites"></a>Prerequisiti 

Per poter completare la procedura descritta in questo articolo, è necessario disporre di:

- SQL Server 2019 in CU2 o versioni successive, in Windows Server 2016 e versioni successive
...
- Attualmente, un DNN con FCI è supportato solo per SQL Server 2019 CU2 e versioni successive in Windows Server 2016 e versioni successive. 
