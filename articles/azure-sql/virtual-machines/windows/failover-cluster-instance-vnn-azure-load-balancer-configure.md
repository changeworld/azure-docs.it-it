---
title: Configurare Azure Load Balancer un'istanza del cluster di failover VNN
description: Informazioni su come configurare un Azure Load Balancer per instradare il traffico al nome di rete virtuale (VNN) per l'istanza del cluster di failover (FCI) con SQL Server in macchine virtuali di Azure per la disponibilità elevata e il ripristino di emergenza (HADR).
...
Questo articolo illustra come configurare un servizio di bilanciamento del carico usando il servizio Azure Load Balancer. Il servizio di bilanciamento del carico instraderà il traffico verso l' [istanza del cluster di failover (FCI)](failover-cluster-instance-overview.md) con SQL Server nelle VM di Azure per la disponibilità elevata e il ripristino di emergenza (HADR). 

Per un'opzione di connettività alternativa per SQL Server 2019 CU2 e versioni successive, considerare un [nome di rete distribuita](failover-cluster-instance-distributed-network-name-dnn-configure.md) per la configurazione semplificata e un failover migliorato. 
