...
> [!CAUTION]
> Il comportamento di routing quando si usa un DNN è diverso quando si usa VNN. Non usare la porta 1433. Per altre informazioni, vedere la sezione relativa alla [considerazione della porta](#port-considerations) più avanti in questo articolo.
...
- Identificato la porta univoca che si utilizzerà per il listener DNN. La porta utilizzata per un listener DNN deve essere univoca in tutte le repliche del gruppo di disponibilità o dell'istanza del cluster di failover.  Nessun'altra connessione può condividere la stessa porta.
...
- I listener DNN **devono** essere configurati con una porta univoca.  La porta non può essere condivisa con altre connessioni su una replica.
...
## <a name="port-considerations"></a>Considerazioni sulle porte

I listener DNN sono progettati per restare in ascolto su tutti gli indirizzi IP, ma su una porta univoca specifica. La voce DNS per il nome del listener deve risolversi negli indirizzi di tutte le repliche del gruppo di disponibilità. Questa operazione viene eseguita automaticamente con lo script di PowerShell fornito nella sezione [create script](#create-script) . Poiché i listener DNN accettano connessioni su tutti gli indirizzi IP, è fondamentale che la porta del listener sia univoca e non usata da altre repliche nel gruppo di disponibilità. Poiché SQL Server è sempre in ascolto sulla porta 1433, direttamente o tramite il servizio SQL Browser, la porta 1433 non può essere usata per alcun listener DNN.
