...
Informazioni su come creare una topologia basata su Java per Apache Storm. Creare una topologia Storm che implementa un'applicazione di conteggio delle parole. Per compilare il progetto e creare il pacchetto si usa Apache Maven. Si apprenderà quindi come definire la topologia usando il framework Apache Storm Flux.
...
L'ambiente usato per questo articolo è un computer che esegue Windows 10.  I comandi sono stati eseguiti in un prompt dei comandi e i vari file sono stati modificati con blocco note.
...
Eliminare i file di test e dell'applicazione generati `AppTest.java` e `App.java` immettendo i comandi seguenti:
...
Aprire `pom.xml` immettendo il comando seguente:

```cmd
notepad pom.xml
```

Aggiungere quindi il codice XML seguente dopo la `<url>https://maven.apache.org</url>` riga:
...
Maven consente di definire i valori a livello di progetto denominati proprietà. In `pom.xml` aggiungere il testo seguente dopo la `</repositories>` riga:
...
Aggiungere una dipendenza per i componenti Storm. In `pom.xml` aggiungere il testo seguente nella `<dependencies>` sezione:
...
I plug-in di Maven consentono di personalizzare le fasi di compilazione del progetto. Ad esempio, il modo in cui viene compilato il progetto o viene creato il pacchetto come file JAR. In `pom.xml` aggiungere il testo seguente direttamente sopra la `</project>` riga.
...
* **Plug-in del compilatore Apache Maven**
...
La sezione delle risorse consente di includere le risorse non di codice, ad esempio i file di configurazione richiesti dai componenti della topologia. Per questo esempio, aggiungere il testo seguente nella `<resources>` sezione del `pom.xml` file. Salvare quindi il file e chiuderlo.
...
Per ridurre i requisiti relativi all'impostazione di origini dati esterne, lo spout seguente genera semplicemente frasi casuali. Si tratta di una versione modificata di un beccuccio fornito con gli [esempi di Storm-Starter](https://github.com/apache/storm/blob/0.10.x-branch/examples/storm-starter/src/jvm/storm/starter).  Anche se questa topologia usa un beccuccio, altri possono avere diversi dati che feedano dati da origini diverse alla topologia`.`
...
>
> * [TwitterSampleSPout](https://github.com/apache/storm/blob/0.10.x-branch/examples/storm-starter/src/jvm/storm/starter/spout/TwitterSampleSpout.java): beccuccio di esempio che legge da Twitter.
> * [Storm-Kafka](https://github.com/apache/storm/tree/0.10.x-branch/external/storm-kafka): un beccuccio che legge da Kafka.
...
La topologia unisce i beccucci e i bulloni in un grafico. Il grafico definisce la modalità di flusso dei dati tra i componenti. Fornisce inoltre suggerimenti di parallelismo usati da Storm durante la creazione di istanze di componenti all'interno del cluster.
...
Quindi copiare e incollare il testo XML riportato di seguito nel nuovo file.  Chiudere quindi il file.
...
Per ulteriori informazioni sulla configurazione della registrazione per log4j 2, vedere [https://logging.apache.org/log4j/2.x/manual/configuration.html](https://logging.apache.org/log4j/2.x/manual/configuration.html) .

> [!NOTE]  
> Storm versione 0.10.0 e successive usano Log4j 2.x. Le versioni precedenti usano Log4j 1.x, che impiega un formato diverso per la configurazione del log. Per informazioni sulla configurazione precedente, vedere [https://cwiki.apache.org/confluence/display/LOGGINGLOG4J/Log4jXmlFormat](https://cwiki.apache.org/confluence/display/LOGGINGLOG4J/Log4jXmlFormat) .
...
Questo log di esempio indica che la parola "and" è stata generata 113 volte. Il conteggio continua ad aumentare fino a quando la topologia viene eseguita. Questo aumento è dovuto al fatto che il beccuccio emette continuamente le stesse frasi.

Esiste un intervallo di 5 secondi tra l'emissione di parole e i conteggi. Il componente **WordCount** è configurato per generare informazioni solo quando arriva una tupla tick. Richiede che le tuple tick vengano recapitate solo ogni cinque secondi.

## <a name="convert-the-topology-to-flux"></a>Convertire la topologia in Flux

[Flux](https://storm.apache.org/releases/2.0.0/flux.html) è un nuovo Framework disponibile con Storm 0.10.0 e versioni successive. Flux consente di separare la configurazione dall'implementazione di. I componenti sono ancora definiti in Java, ma la topologia viene definita mediante un file YAML. È possibile impacchettare una definizione di topologia predefinita con il progetto o usare un file autonomo per l'invio della topologia. Quando si invia la topologia a Storm, usare le variabili di ambiente o i file di configurazione per popolare i valori di definizione della topologia YAML.

Il file YAML definisce i componenti da usare per la topologia e i dati di flusso tra essi. È possibile includere un file YAML come parte del file jar. In alternativa, è possibile usare un file YAML esterno.

Per altre informazioni su Flux, vedere il [framework Flux (https://storm.apache.org/releases/current/flux.html)](https://storm.apache.org/releases/current/flux.html).
...
1. In precedenza, `WordCountTopology.java` definire la topologia, ma non è necessaria con Flux. Eliminare il file con il comando seguente:

    ```cmd
    DEL src\main\java\com\microsoft\example\WordCountTopology.java
    ```

1. Immettere il comando seguente per creare e aprire un nuovo file `topology.yaml` :

    ```cmd
    notepad resources\topology.yaml
    ```

    Quindi copiare e incollare il testo seguente nel nuovo file.  Chiudere quindi il file.
...
1. Immettere il comando seguente per aprire `pom.xml` per apportare le revisioni descritte di seguito:

    ```cmd
    notepad pom.xml
    ```

   1. Aggiungere la nuova dipendenza seguente nella sezione `<dependencies>` :
   ...
   1. Aggiungere il plug-in seguente per la sezione `<plugins>` . Questo plug-in gestisce la creazione di un pacchetto (file jar) per il progetto e applica alcune trasformazioni specifiche a Flux durante la creazione del pacchetto.
   ...
   1. Per la sezione plug-in exec Maven, passare a `<configuration>`  >  `<mainClass>` e modificare `${storm.topology}` in `org.apache.storm.flux.Flux` . Questa impostazione consente a Flux di gestire l'esecuzione in locale della topologia nell'ambiente di sviluppo.

   1. Nella `<resources>` sezione aggiungere quanto segue a `<includes>` . Questo XML include il file YAML che definisce la topologia come parte del progetto.
...
1. Immettere il comando seguente per compilare ed eseguire la topologia Flux usando Maven:
    ...
    > [!WARNING]  
    > Questo comando non va a buon fine se la topologia usa bit di Storm 1.0.1. Questo errore è causato da [https://issues.apache.org/jira/browse/STORM-2055](https://issues.apache.org/jira/browse/STORM-2055) . [Installare Storm nell'ambiente di sviluppo](https://storm.apache.org/releases/current/Setting-up-development-environment.html) e usare la procedura seguente:
    >
    ...
    Si verifica un ritardo di 10 secondi tra i batch delle informazioni registrate.

2. Creare una nuova topologia YAML dal progetto.

    1. Immettere il comando seguente per aprire `topology.xml` :

    ```cmd
    notepad resources\topology.yaml
    ```

    1. Trovare la sezione seguente e modificare il valore di `10` in `5` . Questa modifica consente di cambiare l'intervallo tra i batch di emissione del conteggio di parole da 10 secondi a 5.  
    ...
    1. Salva file con nome `newtopology.yaml` .
...
3. Per eseguire la topologia, immettere il comando seguente:
    ...
    Questo comando USA `newtopology.yaml` come definizione di topologia. Siccome il parametro `compile` non è stato incluso, Maven usa la versione del progetto creato nei passaggi precedenti.

    Una volta avviata la topologia, si noterà che il tempo tra i batch emessi è stato modificato per riflettere il valore in `newtopology.yaml` . Pertanto, è possibile modificare la configurazione tramite un file YAML senza dover ricompilare la topologia.
...
[Trident](https://storm.apache.org/releases/current/Trident-API-Overview.html) è un'astrazione di alto livello fornita da Storm che supporta l'elaborazione con informazioni sullo stato. Il vantaggio principale di Trident è che garantisce che tutti i messaggi che entrano nella topologia vengano elaborati una sola volta. Senza l'uso di Trident, la topologia può garantire solo che i messaggi vengono elaborati almeno una volta. Esistono altre differenze, ad esempio la disponibilità di componenti predefiniti che possono essere usati senza che sia necessario creare bolt. I Bolt sono sostituiti da componenti meno generici, ad esempio filtri, proiezioni e funzioni.

È possibile creare applicazioni Trident mediante progetti Maven L'unica differenza consiste nel codice. Anche Trident non può (attualmente) essere usato con il Framework Flux.
...
Si è appreso come creare una topologia Apache Storm usando Java. è possibile passare agli argomenti seguenti:

* [Distribuzione e gestione di topologie Apache Storm in HDInsight](apache-storm-deploy-monitor-topology-linux.md)

* [Sviluppare topologie usando Python](apache-storm-develop-python-topology.md)
