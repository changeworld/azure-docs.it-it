...
description: Informazioni su PsExec, script di PowerShell e altri strumenti remoti che è possibile usare per risolvere i problemi relativi alle macchine virtuali di Azure remote senza usare RDP.
...
* All'esterno della rete virtuale o della distribuzione
  ...
  * Per una macchina virtuale Azure Resource Manager, aggiungere prima di tutto un nome DNS all'indirizzo IP pubblico. Per i passaggi dettagliati, vedere [Creare un nome di dominio completo nel portale di Azure per una macchina virtuale Windows](../create-fqdn.md). Eseguire poi il comando seguente:
...
2. Selezionare **file**  >  **Connetti registro di rete**.
