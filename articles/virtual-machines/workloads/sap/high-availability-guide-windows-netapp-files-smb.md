...
> [!IMPORTANT]
> Attenzione: tenere presente che l'installazione di un sistema SAP con SWPM nella condivisione SMB, ospitata in [Azure NetApp files][anf-azure-doc] volume SMB, potrebbe non riuscire con errore di installazione per autorizzazioni insufficienti, ad esempio "warningPerm non è definito". Per evitare l'errore, l'utente con il quale viene eseguito il SWPM del contesto richiede privilegi elevati "Domain admin" durante l'installazione del sistema SAP.  
