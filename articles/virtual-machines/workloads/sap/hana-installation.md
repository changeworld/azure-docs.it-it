...
Quando si prevede di installare HANA 2.0, vedere [SAP support note #2235581 - SAP HANA: Supported operating systems](https://launchpad.support.sap.com/#/notes/2235581/E) (Nota di supporto SAP n. 2235581 - SAP HANA: sistemi operativi supportati) per verificare che il sistema operativo sia supportato con la versione di SAP HANA da installare. Il sistema operativo supportato per HANA 2.0 ha più restrizioni di quello supportato per HANA 1.0. È anche necessario controllare se la versione del sistema operativo a cui si è interessati è elencata come supportata per la specifica unità HLI in questo [elenco](https://www.sap.com/dmc/exp/2014-09-02-hana-hardware/enEN/iaas.html#categories=Microsoft%20Azure)pubblicato. Fare clic sull'unità per ottenere i dettagli completi con l'elenco di sistemi operativi supportati di tale unità. 
...
- [Best practices for SAP: Enqueue replication – SAP NetWeaver on SUSE Linux Enterprise 12](https://www.scribd.com/document/351887168/SLES4SAP-NetWeaver-ha-guide-EnqRepl-12-color-en-pdf) (Procedure consigliate per SAP: replica di accodamento e SAP NetWeaver in SUSE Linux Enterprise 12)
...
- [SAP support note #1944799 – SAP HANA guidelines for SLES operating system installation](http://service.sap.com/sap/support/notes/1944799) (Nota di supporto SAP n. 1944799: linee guida di SAP HANA per l'installazione del sistema operativo SLES)
...
[Red Hat Enterprise Linux for SAP HANA](https://www.redhat.com/en/resources/red-hat-enterprise-linux-sap-hana) è un'altra offerta per l'esecuzione di SAP HANA in istanze di grandi dimensioni di HANA. Sono disponibili e supportano le versioni di RHEL 7,2 e 7,3. 
...
- [Nota di supporto SAP #2009879-SAP HANA le linee guida per il sistema operativo Red Hat Enterprise Linux (RHEL)](https://launchpad.support.sap.com/#/notes/2009879/E)
...
- [Nota di supporto SAP #2002167-Red Hat Enterprise Linux 7. x: installazione e aggiornamento](https://launchpad.support.sap.com/#/notes/2002167)

### <a name="time-synchronization"></a>Sincronizzazione dell'ora
...
Durante l'onboarding con SAP HANA in Azure `service management` , il cliente specifica un ID utente (UID) e un ID gruppo (GID) per l'utente sidadm e il gruppo sapsys (ad esempio: 1.000.500). È necessario usare questi stessi valori durante l'installazione del sistema SAP HANA. Poiché si vogliono distribuire più istanze di HANA in un'unità, si ottengono più set di volumi, ovvero un set per ogni istanza. In fase di distribuzione è quindi necessario definire quanto segue:
...
Per SAP HANA versioni 1,0 fino a SPS12, questi parametri possono essere impostati durante l'installazione del database di SAP HANA, come descritto nella [Nota SAP #2267798-Configuration del database SAP Hana](https://launchpad.support.sap.com/#/notes/2267798).
...
> [!IMPORTANT]
> Per evitare che HANA provi a espandere i file di dati oltre il limite di dimensioni di 16 TB per l'archiviazione di istanze large di HANA, è necessario impostare i parametri seguenti nel file di configurazione SAP HANA global.ini
> 
