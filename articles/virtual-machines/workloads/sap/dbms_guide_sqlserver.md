...
Prima di continuare, è consigliabile leggere l'articolo [informazioni su SQL Server in macchine virtuali di Azure (Windows)] [ https://docs.microsoft.com/azure/azure-sql/virtual-machines/windows/sql-server-on-azure-vm-iaas-what-is-overview ].
...
* **Prestazioni SQL**: anche se le prestazioni delle macchine virtuali ospitate in Microsoft Azure sono elevate rispetto ad altre offerte per la virtualizzazione del cloud pubblico, i singoli risultati possono variare. Leggere l'articolo [Performance best practices for SQL Server in Azure Virtual Machines](../../../azure-sql/virtual-machines/windows/performance-guidelines-best-practices.md) (Procedure consigliate sulle prestazioni per SQL Server nelle macchine virtuali di Azure).
...
*  **SQL Server supporto per più istanze all'interno di una singola macchina virtuale di Azure**: questo metodo di distribuzione è supportato. Tuttavia, tenere presenti le limitazioni delle risorse, soprattutto in termini di larghezza di banda di rete e di archiviazione del tipo di macchina virtuale in uso. Informazioni dettagliate sono disponibili in [dimensioni degli articoli per le macchine virtuali in Azure](https://docs.microsoft.com/azure/virtual-machines/sizes). Queste limitazioni di quota potrebbero impedire l'implementazione della stessa architettura a più istanze, che può essere implementata in locale. Alla configurazione e all'interferenza della condivisione delle risorse disponibili all'interno di una singola VM, è necessario tenere in considerazione le stesse considerazioni locali.
*  **Più database SAP in una singola istanza di SQL Server in una singola macchina virtuale**: come indicato in precedenza, sono supportate configurazioni come queste. Le considerazioni su più database SAP che condividono le risorse condivise di una singola istanza di SQL Server sono identiche a quelle per le distribuzioni locali. Altri limiti di mantenimento come il numero di dischi che possono essere collegati a uno specifico tipo di macchina virtuale. O limiti della quota di archiviazione e di rete di tipi di VM specifici come [dimensioni dettagliate per le macchine virtuali in Azure](https://docs.microsoft.com/azure/virtual-machines/sizes). 
...
In conformità con la descrizione generale, il sistema operativo, i SQL Server eseguibili e, nel caso di sistemi SAP a 2 livelli, è necessario che i file eseguibili SAP siano presenti o installati dischi di Azure separati. In genere, la maggior parte dei database di sistema di SQL Server non viene pienamente sfruttata dal carico di lavoro di SAP NetWeaver. Tuttavia, i database di sistema di SQL Server (Master, msdb e Model) dovrebbero essere, insieme alle altre directory di SQL Server in un disco di Azure separato. SQL Server tempdb deve trovarsi in nonperisisted D:\ unità o su un disco separato.
...
* Per le versioni precedenti SQL Server, dove SQL Server installa tempdb con un file di dati per impostazione predefinita, è consigliabile usare più file di dati tempdb. Tenere presente che i volumi delle unità D:\ sono diversi in base al tipo di VM. Per le dimensioni esatte dell'unità D:\ delle diverse VM, vedere l'articolo [Dimensioni per le macchine virtuali Windows in Azure](../../sizes.md).

Queste configurazioni consentono a tempdb di utilizzare più spazio e più importanti più IOPS e larghezza di banda di archiviazione rispetto all'unità di sistema che è in grado di fornire. D:\ non persistente l'unità offre anche una migliore latenza di I/O e velocità effettiva (ad eccezione delle VM serie A). Per determinare le dimensioni corrette del database tempdb, è possibile controllare le dimensioni di tempdb nei sistemi esistenti. 

>[!NOTE]
> Se si memorizzano i file di dati di tempdb e il file di log in una cartella nell'unità D:\ creata personalmente, è necessario assicurarsi che la cartella esista dopo un riavvio della VM. Poiché l'unità D:\ viene inizializzata del tutto dopo il riavvio di una macchina virtuale tutte le strutture di file e di directory vengono cancellate. Una possibilità per ricreare le strutture di directory nell'unità D:\ prima dell'avvio del servizio SQL Server è documentata in [questo articolo](https://cloudblogs.microsoft.com/sqlserver/2014/09/25/using-ssds-in-azure-vms-to-store-sql-server-tempdb-and-buffer-pool-extensions/).
...
Il diagramma soprastante mostra un caso semplice. Come eluso nell'articolo Considerazioni sulla [distribuzione DBMS di macchine virtuali di Azure per il carico di lavoro SAP](dbms_guide_general.md), il tipo di archiviazione di Azure, il numero e le dimensioni dei dischi dipendono da diversi fattori. È tuttavia consigliabile:

- Utilizzando un volume di grandi dimensioni, che contiene i file di dati SQL Server. Il motivo alla base di questa configurazione è che nella vita reale vengono usati molti database SAP con file di database di dimensioni diverse con carichi di lavoro I/O diversi.
- Usare l'unità D:\ per tempdb fintanto che le prestazioni sono sufficientemente buone. Se il carico di lavoro complessivo è limitato a livello di prestazioni, tempdb si trova nella D:\ potrebbe essere necessario prendere in considerazione la possibilità di spostare tempdb per separare archiviazione Premium di Azure o dischi Ultra disk come consigliato in [questo articolo](../../../azure-sql/virtual-machines/windows/performance-guidelines-best-practices.md).
...
Per SQL Server, le dimensioni del blocco NTFS per i dischi contenenti SQL Server file di dati e di log devono essere 64 KB. Non è necessario formattare l'unità D:\ perché viene fornita preformattata.
...
I dettagli di questa funzionalità sono disponibili nell'articolo [File di dati di SQL Server in Microsoft Azure](/sql/relational-databases/databases/sql-server-data-files-in-microsoft-azure)
...
Per esaminare le varie possibilità di backup per SQL Server in Azure, leggere l'articolo [Backup e ripristino per SQL Server in macchine virtuali di Azure](../../../azure-sql/virtual-machines/windows/backup-restore.md). L'articolo descrive molti modi diversi.
...
Il primo metodo è ben noto e si applica anche in molti casi in locale. Lascia tuttavia all'amministratore il compito di risolvere il problema della posizione dei backup a lungo termine. Poiché non è utile mantenere i backup per 30 giorni e oltre nella risorsa di archiviazione di Azure collegata localmente, è necessario usare il servizio Backup di Microsoft Azure o un altro strumento di backup/ripristino di terze parti che includa la gestione degli accessi e della conservazione per i backup. In alternativa si può creare un file server di grandi dimensioni in Azure usando gli spazi di archiviazione di Windows.

La seconda modalità è descritta più in dettaglio nell'articolo [Backup di SQL Server nell'URL](../../../azure-sql/virtual-machines/windows/backup-restore.md). Le diverse versioni di SQL Server presentano alcune varianti di questa funzionalità. È quindi consigliabile verificare la documentazione specifica della versione di SQL Server in uso. Importante tenere presente che questo articolo elenca numerose restrizioni. Esiste anche la possibilità di eseguire il backup in:

- Un singolo BLOB di pagine di Azure, che quindi limita la dimensione del backup a 1000 GB. Questa restrizione limita anche la velocità effettiva che è possibile ottenere.
...
Il servizio Backup automatizzato consente il backup automatico per le edizioni di SQL Server Standard ed Enterprise in esecuzione in una macchina virtuale Windows in Azure. Il servizio viene reso disponibile dall'[Estensione SQL Server IaaS Agent](../../../azure-sql/virtual-machines/windows/sql-server-iaas-agent-extension-automate-management.md), installata automaticamente nelle immagini di macchine virtuali Windows di SQL Server nel portale di Azure. Se si distribuiscono le proprie immagini del sistema operativo con SQL Server installato, è necessario installare le estensioni di macchina virtuale separatamente. I passaggi necessari sono documentati in questo [articolo](../../../azure-sql/virtual-machines/windows/sql-server-iaas-agent-extension-automate-management.md).

Altre informazioni sulle funzionalità di questa modalità sono disponibili negli articoli seguenti:

- SQL Server 2014: [Backup automatico per macchine virtuali SQL Server 2014 (Resource Manager)](../../../azure-sql/virtual-machines/windows/automated-backup-sql-2014.md)
- SQL Server 2016/2017: [Backup automatico v2 per macchine virtuali in Azure (Resource Manager)](../../../azure-sql/virtual-machines/windows/automated-backup.md)

Se si cerca nella documentazione, è possibile vedere che la funzionalità è migliorata con le versioni più recenti di SQL Server. Altre informazioni sui backup automatizzati di SQL Server sono disponibili nell'articolo [Backup gestito di SQL Server in Microsoft Azure](/sql/relational-databases/backup-restore/sql-server-managed-backup-to-microsoft-azure). Il limite teorico di dimensione del backup è 12 TB.  I backup automatizzati possono essere una buona soluzione per i backup di dimensioni fino a 12 TB. Poiché le operazioni di scrittura in più BLOB avvengono in parallelo, ci si può aspettare una velocità effettiva superiore a 100 MB/sec. 
...
Una descrizione più dettagliata di questa modalità di backup, che aggiunge molti vantaggi per le configurazioni, il monitoraggio e l'amministrazione centrali dei backup è disponibile [qui](../../../backup/backup-azure-sql-database.md). 
...
## <a name="using-a-sql-server-image-out-of-the-microsoft-azure-marketplace"></a><a name="1b353e38-21b3-4310-aeb6-a77e7c8e81c8"></a>Uso di un'immagine di SQL Server da Microsoft Azure Marketplace
...
Se si usa SQL Server nelle distribuzioni IaaS di Azure per SAP, sono disponibili molte possibilità per distribuire il livello DBMS a disponibilità elevata. Come illustrato in [considerazioni sulla distribuzione DBMS di macchine virtuali di Azure per il carico di lavoro SAP](dbms_guide_general.md) , Azure offre contratti di tempo di esecuzione diversi per una singola macchina virtuale e una coppia di macchine virtuali distribuite in un set di disponibilità di Azure. Si supponga di preferire un contratto di licenza a scadenza per le proprie distribuzioni nella produzione che richieda la distribuzione in set di disponibilità di Azure. In tal caso, è necessario distribuire almeno due macchine virtuali nel set di disponibilità. Una macchina virtuale viene eseguita nell'istanza di SQL Server attiva. L'altra macchina virtuale viene eseguita nell'istanza passiva.

### <a name="sql-server-clustering-using-windows-scale-out-file-server-or-azure-shared-disk"></a>SQL Server clustering con il file server di scalabilità orizzontale di Windows o un disco condiviso di Azure
Con Windows Server 2016, Microsoft ha introdotto la funzionalità [Spazi di archiviazione diretta](/windows-server/storage/storage-spaces/storage-spaces-direct-overview). In base alla distribuzione di Spazi di archiviazione diretta, SQL Server clustering FCI è supportato in generale. Azure offre anche [dischi condivisi di Azure](../../disks-shared-enable.md?tabs=azure-cli) che possono essere usati per il clustering di Windows. Per il carico di lavoro SAP, non sono supportate le opzioni di disponibilità elevata. 

### <a name="sql-server-log-shipping"></a>Log shipping SQL Server
Uno dei metodi di disponibilità elevata utilizzabili è il log shipping SQL Server. Se le macchine virtuali che partecipano alla configurazione a disponibilità elevata hanno una risoluzione dei nomi funzionante, non ci sono problemi. La configurazione in Azure non è diversa da quella eseguita in locale per la configurazione del log shipping e dei principi relativi al log shipping. sono disponibili informazioni dettagliate nell'articolo [Informazioni sul log shipping (SQL Server)](/sql/database-engine/log-shipping/about-log-shipping-sql-server).
...
- Utilizzo del listener del gruppo di disponibilità. Con il listener del gruppo di disponibilità, è necessario distribuire un servizio di bilanciamento del carico di Azure. In questo modo si tratta del metodo di distribuzione predefinito. Le applicazioni SAP potrebbero essere configurate per la connessione rispetto ai listener del gruppo di disponibilità e non un singolo nodo
...
- Con SQL Server 2016, è stata introdotta una nuova funzionalità che consente anche la compressione efficiente dei database crittografati. Per informazioni dettagliate, vedere [questo Blog](/archive/blogs/sqlcat/sqlsweet16-episode-1-backup-compression-for-tde-enabled-databases) .
 
Considerando l'applicazione della crittografia Transparent Data Encryption solo con un solo carico di lavoro SAP, è consigliabile verificare la configurazione specifica se è preferibile applicare Transparent Data Encryption al database SAP in locale o in Azure. In Azure è senz'altro maggiore la flessibilità in termini di provisioning eccessivo dell'infrastruttura e di riduzione della stessa dopo l'applicazione di TDE.
...
- [Extensible Key Management tramite l'insieme di credenziali delle chiavi di Azure (SQL Server)](/sql/relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server).
- [Extensible Key Management TDE di SQL Server con Azure Key Vault - Passaggi di configurazione](/sql/relational-databases/security/encryption/setup-steps-for-extensible-key-management-using-the-azure-key-vault).
- [Manutenzione e risoluzione dei problemi di Connettore SQL Server](/sql/relational-databases/security/encryption/sql-server-connector-maintenance-troubleshooting).
- [More Questions From Customers About SQL Server Transparent Data Encryption – TDE + Azure Key Vault](/archive/blogs/saponsqlserver/more-questions-from-customers-about-sql-server-transparent-data-encryption-tde-azure-key-vault) (Altre domande dei clienti sulla funzionalità Transparent Data Encryption di SQL Server - TDE + Azure Key Vault).
...
## <a name="general-sql-server-for-sap-on-azure-summary"></a><a name="9053f720-6f3b-4483-904d-15dc54141e30"></a>Riepilogo generale su SQL Server per SAP in Azure
...
2. Pianificare attentamente l'infrastruttura di sistema SAP in Azure per bilanciare il layout dei file di dati e le restrizioni di Azure:
   ...
   * Se non si usa Managed Disks, tenere presente che il numero di operazioni di I/O al secondo consentito varia a seconda dell'account di archiviazione di Azure e che gli account di archiviazione sono limitati nelle singole sottoscrizioni di Azure ([altri dettagli][azure-resource-manager/management/azure-subscription-service-limits]). 
...
11. Installare e configurare la funzionalità di monitoraggio dell'host SAP per Azure come illustrato nella [Guida alla distribuzione][deployment-guide].
