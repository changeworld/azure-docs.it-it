...
Altre informazioni su [iostat](https://linux.die.net/man/1/iostat) e [PerfMon](/windows/win32/perfctrs/performance-counters-portal).
...
| **Limiti relativi al ridimensionamento di VM e dischi** |Il limite di IOPS della dimensione di VM scelta deve essere superiore al totale di IOPS basato sui dischi di archiviazione collegati alla VM. |Il limite di velocità effettiva della dimensione di VM scelta deve essere superiore al totale di velocità effettiva basato sul disco di Archiviazione Premium collegato alla VM. |I limiti di ridimensionamento della dimensione di VM scelta devono essere superiori al totale dei limiti di ridimensionamento dei dischi di Archiviazione Premium collegati. |
...
| **Striping del disco** |Usare più dischi ed eseguirne lo striping per ottenere un limite combinato più elevato per IOPS e velocità effettiva. Il limite combinato per ogni VM deve essere superiore ai limiti combinati dei dischi Premium collegati. | &nbsp; | &nbsp; |
...
* Un'applicazione OLTP genera milioni di richieste I/O piccole e casuali. Per gestire questi tipi di richieste I/O, è necessario progettare l'infrastruttura dell'applicazione in modo da ottenere valori di IOPS più elevati.  
* Un'applicazione di tipo data warehouse genera richieste I/O di grandi dimensioni e sequenziali. Per gestire questi tipi di richieste I/O, è necessario progettare l'infrastruttura dell'applicazione in modo da ottenere valori di larghezza di banda e velocità effettiva più elevati.
...
*Nessuno*  
Attualmente l'impostazione **Nessuno** è supportata solo nei dischi dati. Non è supportata nei dischi del sistema operativo. Se si imposta **Nessuno** per un disco del sistema operativo, viene eseguito internamente l'override e l'impostazione viene sostituita da **ReadOnly**.
...
Altre informazioni sui [Gradi di parallelismo](/previous-versions/sql/sql-server-2008-r2/ms188611(v=sql.105)) in SQL Server. Individuare le impostazioni che influenzano il multithreading nell'applicazione e le relative configurazioni per ottimizzare le prestazioni.
