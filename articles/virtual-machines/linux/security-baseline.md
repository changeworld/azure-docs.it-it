...
Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1.0](../../security/benchmarks/overview-v1.md) a macchine virtuali Linux. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure. Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili a macchine virtuali Linux. 

> [!NOTE]
> I **controlli** non applicabili a macchine virtuali Linux o per i quali la responsabilità è Microsoft, sono stati esclusi. Per informazioni su come macchine virtuali Linux viene eseguito il mapping completo al benchmark di sicurezza di Azure, vedere il **[file di mapping di base macchine virtuali Linux sicurezza completo](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/linux-virtual-machines-security-baseline-v1.1.xlsx)**.
...
- [Per acquisire i dati syslog per il monitoraggio, è necessario abilitare l'estensione Log Analytics](https://docs.microsoft.com/azure/azure-monitor/vm/quick-collect-azurevm#enable-the-log-analytics-vm-extension)
...
- [Panoramica dell'agente di log Analytics](../../azure-monitor/agents/log-analytics-agent.md)
...
- [Panoramica dell'agente di log Analytics](/azure/azure-monitor/platform/log-analytics-agent)
...
- [Il collegamento seguente fornisce le linee guida di sicurezza consigliate da Microsoft, che possono fungere da elenco di criteri per il software di vulnerabilità selezionato](/azure/virtual-machines/linux/security-recommendations)
...
**Linee guida**: macchine virtuali Linux e Azure Active Directory (Azure ad) non hanno il concetto di password predefinite. Cliente responsabile di applicazioni di terze parti e servizi del Marketplace che possono usare password predefinite.
...
**Indicazioni**: configurare le impostazioni di diagnostica per Azure Active Directory (Azure ad) per inviare i log di controllo e i log di accesso a un'area di lavoro log Analytics. Usare inoltre monitoraggio di Azure per esaminare i log ed eseguire query sui dati syslog di autenticazione dalle macchine virtuali di Azure.
...
**Linee guida**: nei casi in cui una terza parte debba accedere ai dati dei clienti (ad esempio durante una richiesta di supporto), usare Customer Lockbox per le macchine virtuali di Azure per rivedere e approvare o rifiutare le richieste di accesso ai dati del cliente.
...
- [Linee guida sulla sicurezza consigliate da Microsoft](/azure/virtual-machines/linux/security-recommendations)
...
- [Soluzione Gestione aggiornamenti in Azure](/azure/automation/overview)

- [Gestire gli aggiornamenti e le patch per le macchine virtuali di Azure](/azure/automation/manage-updates-for-vm)
...
Oltre a usare Rilevamento modifiche per il monitoraggio di applicazioni software, i controlli delle applicazioni adattivi nel centro sicurezza di Azure usano Machine Learning per analizzare le applicazioni in esecuzione nei computer e creare un oggetto di consenso da questa intelligence. Questa funzionalità semplifica notevolmente il processo di configurazione e gestione dei criteri per l'utilizzo di applicazioni, consentendo di evitare l'utilizzo di software indesiderato nell'ambiente in uso. È possibile configurare la modalità di controllo o l'imposizione. La modalità di controllo controlla solo l'attività nelle macchine virtuali protette. La modalità di imposizione applica le regole e garantisce che le applicazioni non consentite siano bloccate.

- [Estensione di diagnostica per macchine virtuali Linux](https://docs.microsoft.com/azure/virtual-machines/extensions/diagnostics-linux?toc=/azure/azure-monitor/toc.json)
...
- [Informazioni Rilevamento modifiche](../../automation/change-tracking/overview.md)
...
- [Come caricare un disco rigido virtuale di una macchina virtuale personalizzata in Azure](../windows/upload-generalized-managed.md)
...
**Indicazioni**: se si usano immagini personalizzate, ad esempio un disco rigido virtuale, usare il controllo degli accessi in base al ruolo di Azure per garantire che solo gli utenti autorizzati possano accedere alle immagini
...
- [Come configurare Microsoft antimalware per servizi cloud e macchine virtuali](/azure/virtual-machines/linux/security-recommendations)
...
**Indicazioni**: il Centro sicurezza assegna un livello di gravità a ogni avviso per facilitare la classificazione in ordine di priorità degli avvisi da analizzare. Il livello di gravità è basato sul livello di attendibilità del Centro sicurezza nell'individuazione o sull'analisi utilizzata per emettere l'avviso, oltre che sul livello di confidenza che ha causato l'intento dannoso dietro l'attività che ha portato all'avviso. Contrassegnare anche chiaramente le sottoscrizioni, ad esempio di produzione o non di produzione, tramite i tag e creare un sistema di denominazione per identificare e classificare distintamente le risorse di Azure, in particolare quelle che elaborano i dati sensibili. È responsabilità dell'utente classificare in ordine di priorità la correzione degli avvisi in base alla criticità delle risorse e dell'ambiente di Azure in cui si è verificato l'evento imprevisto. 
