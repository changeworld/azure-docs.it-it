...
- Se si intende caricare un disco rigido virtuale da locale: un disco rigido virtuale di dimensioni fisse preparato per [Azure](../windows/prepare-for-upload-vhd-image.md), archiviato in locale.
...
> [!NOTE]
> Mentre in uno di questi stati, il disco gestito verrà fatturato ai prezzi [hdd standard,](https://azure.microsoft.com/pricing/details/managed-disks/)indipendentemente dal tipo effettivo di disco. Ad esempio, un P10 verrà fatturato come S10. Questo sarà vero fino a quando non viene chiamato sul disco gestito, che è necessario per collegare `revoke-access` il disco a una macchina virtuale.
...
Creare un'unità disco rigido standard vuota per il caricamento specificando sia il parametro **--for-upload** che il **parametro --upload-size-bytes** in un cmdlet [disk create:](/cli/azure/disk#az_disk_create)
...
Ora che è stato creato un disco gestito vuoto configurato per il processo di caricamento, è possibile caricarvi un disco rigido virtuale. Per caricare un disco rigido virtuale sul disco, è necessaria una firma di accesso condiviso scrivibile, in modo da potervi fare riferimento come destinazione per il caricamento.
...
Ora che è disponibile una firma di accesso condiviso per il disco gestito vuoto, è possibile usarla per impostare il disco gestito come destinazione per il comando di caricamento.
...
Questo caricamento ha la stessa velocità effettiva dell'hdd [standard equivalente.](../disks-types.md#standard-hdd) Ad esempio, se si dispone di una dimensione che equivale a S4, si avrà una velocità effettiva fino a 60 MiB/s. Tuttavia, se si dispone di una dimensione che equivale a S70, si avrà una velocità effettiva fino a 500 MiB/s.
...
Lo script seguente lo farà per l'utente, il processo è simile ai passaggi descritti in precedenza, con alcune differenze dal momento che si sta lavorando con un disco esistente.

> [!IMPORTANT]
> È necessario aggiungere un offset di 512 quando si specificano le dimensioni del disco in byte di un disco gestito da Azure. Ciò è dovuto al fatto che Azure omette il piè di pagina quando restituisce le dimensioni del disco. Se non si fa questa operazione, la copia avrà esito negativo. Lo script seguente esegue già questa operazione per l'utente.

Sostituire , , , e (un esempio di valore location sarebbe `<sourceResourceGroupHere>` `<sourceDiskNameHere>` `<targetDiskNameHere>` `<targetResourceGroupHere>` `<yourTargetLocationHere>` uswest2) con i valori, quindi eseguire lo script seguente per copiare un disco gestito.
