...
Creare quindi una macchina virtuale con il comando [az vm create](/cli/azure/vm#az_vm_create). L'esempio seguente crea una macchina virtuale denominata *myVM* e genera chiavi SSH *~/.ssh/* , se non esistono ancora:
...
L'area di lavoro di [Log Analytics](../../azure-monitor/logs/log-query-overview.md) consente di raccogliere i dati generati da funzionalità e servizi, ad esempio Gestione aggiornamenti.
...
> [Monitorare le macchine virtuali](/previous-versions/azure/virtual-machines/linux/tutorial-monitor)