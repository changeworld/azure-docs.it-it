...
Usare l'opzione **EncryptFormatAll** . Per altre informazioni su questa opzione, vedere [usare la funzionalità EncryptFormatAll per i dischi dati nelle macchine virtuali Linux](./disk-encryption-linux.md#use-encryptformatall-feature-for-data-disks-on-linux-vms).
...
Nelle procedure si presuppone che siano già stati esaminati i prerequisiti negli [scenari di crittografia dischi di Azure nelle VM Linux](./disk-encryption-linux.md) e in [Guida introduttiva: creare e crittografare una VM Linux con l'interfaccia della](./disk-encryption-cli-quickstart.md)riga di comando di Azure.
...
df -h
```
![Screenshot mostra una finestra della console con file system montati come DATA0 e Data1.](./media/disk-encryption/lvm-raid-on-crypt/018-lvm-raid-lsblk-after-lvm.png)
...
>[!NOTE] 
>Questo ciclo esegue l'iterazione su un solo dispositivo per questo particolare esempio, viene compilato in questo modo per essere usato per più dispositivi MD, se necessario.
...
df -h
```
![Screenshot mostra una finestra della console con un file system montato come raiddata.](./media/disk-encryption/lvm-raid-on-crypt/021-lvm-raid-lsblk-md-details.png)
...
- [Ridimensionare i dispositivi logici di gestione dei volumi crittografati con crittografia dischi di Azure](how-to-resize-encrypted-lvm.md)
