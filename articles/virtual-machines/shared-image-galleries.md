---
title: Condividere immagini di VM con le raccolte condivise
description: Informazioni su come usare le raccolte di immagini condivise per condividere immagini di VM Linux nell'intera organizzazione.
author: axayjo
...
- Tag: quando si crea la definizione dell'immagine è possibile aggiungere tag. Per altre informazioni sui tag, vedere [Usare i tag per organizzare le risorse](../azure-resource-manager/management/tag-resources.md)
...
Per altre informazioni, vedere [Controllare l'utilizzo delle risorse rispetto ai limiti](../networking/check-usage-against-limits.md) per esempi su come controllare l'utilizzo corrente.
...
- Per le distribuzioni di set di scalabilità di macchine virtuali non virtuali, per ogni 20 VM create simultaneamente, è consigliabile usare una replica. Se, ad esempio, si creano 120 macchine virtuali contemporaneamente usando la stessa immagine in un'area, si consiglia di mantenere almeno 6 repliche dell'immagine. 
- Per le distribuzioni di set di scalabilità di macchine virtuali: per ogni distribuzione di set di scalabilità con un massimo di 600 istanze, si consiglia di contenere almeno una replica. Se, ad esempio, si creano 5 set di scalabilità contemporaneamente, ognuno con 600 istanze di macchine virtuali e usando la stessa immagine in un'area, si consiglia di mantenere almeno 5 repliche dell'immagine. 
...
È inoltre possibile scegliere il tipo di account per ogni area di destinazione. Il tipo di account di archiviazione predefinito è 	Archiviazione con ridondanza locale Standard, tuttavia è anche possibile scegliere Archiviazione con ridondanza della zona Standard per le aree con zone di disponibilità. Controllare [qui](../storage/common/storage-redundancy.md) la disponibilità a livello di area dell'archiviazione con ridondanza della zona.
...
Per altre informazioni, vedere **Gestire le risorse della raccolta** con l'[interfaccia della riga di comando di Azure](update-image-resources-cli.md) o [PowerShell](update-image-resources-powershell.md).
...
- Se il disco rigido virtuale è di una macchina virtuale Windows, vedere [Caricare un disco rigido virtuale](./windows/upload-generalized-managed.md).
- Se il disco rigido virtuale è per una macchina virtuale Linux, vedere [Caricare un disco rigido virtuale](./linux/upload-vhd.md#option-1-upload-a-vhd)
...
Sì, è possibile creare una macchina virtuale da un'immagine specializzata usando l'[interfaccia della riga di comando](vm-specialized-image-version-cli.md), [PowerShell](vm-specialized-image-version-powershell.md) o l'API. 
