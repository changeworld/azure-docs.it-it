...
Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1,0](../security/benchmarks/overview-v1.md) per Microsoft Azure DNS. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure.
Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili al servizio DNS di Azure. I **controlli** non applicabili al servizio DNS di Azure sono stati esclusi.

 
Per informazioni sul mapping completo di DNS di Azure al benchmark di sicurezza di Azure, vedere il [file di mapping di base di sicurezza DNS di Azure completo](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Linee guida**: usare il Centro sicurezza di Azure con aree di lavoro log Analytics per il monitoraggio e l'invio di avvisi su attività anomale rilevate in registri di sicurezza ed eventi.
...
**Linee guida**: è possibile accedere alle origini dei log eventi di attività, controllo e rischio di Azure Active Directory (Azure ad), che consentono di eseguire l'integrazione con qualsiasi soluzione di sistema di gestione degli eventi e informazioni di sistema o uno strumento di monitoraggio.
...
In DNS di Azure esiste il ruolo Collaboratore zona DNS, nonché il livello di zona e il livello di set di record di Azure. 

È anche possibile creare ruoli personalizzati di Azure per fornire un controllo più granulare. 
...
Per altre informazioni sull'assegnazione di tag agli asset, vedere la documentazione relativa alla denominazione delle risorse e alla guida decisionale per l'assegnazione di tag. 

- [Guida alle decisioni per la denominazione delle risorse e l'assegnazione di tag](/azure/cloud-adoption-framework/decision-guides/resource-tagging)
...
Inoltre, Azure Resource Manager supporta un altro tipo di controllo di sicurezza, la possibilità di bloccare le risorse. I blocchi delle risorse vengono applicati alla risorsa e sono efficaci in tutti gli utenti e i ruoli. Esistono due tipi di blocchi di risorse: CanNotDelete e ReadOnly.
