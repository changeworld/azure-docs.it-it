...
I client DNS nei PC o nei dispositivi mobili in genere chiamano un server DNS ricorsivo per eseguire qualsiasi query DNS necessaria per le applicazioni client.

Quando un server DNS ricorsivo riceve una query per un record DNS, ad esempio "www.contoso.com", deve prima trovare il server dei nomi che ospita la zona per il dominio "contoso.com". Per trovare il server di nomi, inizia dal server dei nomi radice e da lì rileva il server dei nomi che ospita la zona "com". Esegue quindi la query sui server dei nomi "com" per trovare i server dei nomi che ospitano la zona "contoso.com".  Infine, è in grado di eseguire una query su questi server dei nomi per "www.contoso.com".

Questa procedura è definita risoluzione del nome DNS. In senso stretto, la risoluzione DNS include altri passaggi, ad esempio i CNAME seguenti, ma questo non è importante per comprendere il funzionamento della delega DNS.
...
2. Il server DNS locale non ha il record, quindi effettua una richiesta al server dei nomi radice.
3. Il server dei nomi radice non ha il record, ma conosce l'indirizzo del server dei nomi, fornisce `.net` tale indirizzo al server DNS
...
7. La zona non ha il record, ma conosce il server dei `contoso.net` nomi per e risponde con `partners.contoso.net` l'indirizzo. In questo caso, si tratta di una zona DNS ospitata in DNS di Azure.
...
Informazioni su come [delegare il dominio a DNS di Azure](dns-delegate-domain-azure-dns.md)
