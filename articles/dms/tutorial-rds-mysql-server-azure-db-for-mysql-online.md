---
title: 'Esercitazione: Eseguire la migrazione online di RDS MySQL a Database di Azure per MySQL'
...
* Assicurarsi che il server MySQL di origine esegua una versione di MySQL Community Edition supportata. Per determinare la versione dell'istanza di MySQL, nell'utilità mysql o in MySQL Workbench eseguire il comando:
    ...
    Per altre informazioni, vedere l'articolo [Versioni supportate di Database di Azure per MySQL](../mysql/concepts-supported-versions.md).
...
* Creare un'istanza di [Database di Azure per MySQL](../mysql/quickstart-create-mysql-server-database-using-azure-portal.md).
...
* Configurare [Windows Firewall](https://docs.microsoft.com/azure/mysql/concepts-firewall-rules) (o il firewall Linux) per consentire l'accesso al motore di database. Per il server MySQL consentire la connettività della porta 3306.
...
5. Quando il gruppo di parametri è pronto, connettersi all'istanza di MySQL e [impostare il periodo di conservazione dei log binari](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/mysql_rds_set_configuration.html#mysql_rds_set_configuration-usage-notes.binlog-retention-hours) almeno su 5 giorni.
```
