...
description: Informazioni su come risolvere i problemi di connessione da punto a sito comuni e altri errori e problemi della rete privata virtuale.
...
Assicurarsi che il server RADIUS sia configurato correttamente. Per ulteriori informazioni, vedere [integrazione dell'autenticazione RADIUS con Azure AD server multi-factor authentication](../active-directory/authentication/howto-mfaserver-dir-radius.md).
...
Il client VPN di Azure non ha l'autorizzazione dell'app "app in background" abilitata nelle impostazioni dell'app per Windows.

### <a name="solution"></a>Soluzione

1. In Windows passare a impostazioni-> privacy-> app in background
2. Consente di impostare l'opzione "lascia app eseguite in background" su on
