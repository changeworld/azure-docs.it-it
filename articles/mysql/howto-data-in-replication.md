---
title: Configurare Replica dei dati in ingresso-database di Azure per MySQL
...
## <a name="create-an-azure-database-for-mysql-single-server-instance-to-use-as-a-replica"></a>Creare un'istanza del server singolo di database di Azure per MySQL da usare come replica

1. Creare una nuova istanza di database di Azure per il server singolo MySQL (ad esempio, "replica.mysql.database.azure.com". Per istruzioni su come creare il server, vedere [Creare un server di Database di Azure per MySQL usando il portale di Azure](quickstart-create-mysql-server-database-using-azure-portal.md). Questo server è il server "replica" per Replica dei dati in ingresso.
...
## <a name="configure-the-source-mysql-server"></a>Configurare il server MySQL di origine

I passaggi seguenti consentono di preparare e configurare il server MySQL ospitato in locale, in una macchina virtuale o un servizio di database ospitato da altri provider di cloud per la replica dei dati in ingresso. Questo server è l'"origine" per la replica dei dati.

1. Prima di procedere, esaminare i [requisiti del server di origine](concepts-data-in-replication.md#requirements) .

2. Verificare che il server di origine consenta il traffico in ingresso e in uscita sulla porta 3306 e che disponga di un **indirizzo IP pubblico**, che il DNS sia accessibile pubblicamente o che disponga di un nome di dominio completo (FQDN).
   ...
   1. Accedere al database di Azure per il server MySQL usando uno strumento come la riga di comando di MySQL.
   ...
   4. Per ottenere l'indirizzo IP, eseguire il comando seguente nell'utilità ping:
...
3. Attivare la registrazione binaria.
   ...
   5. Dopo il riavvio del server, verificare che la registrazione binaria sia abilitata eseguendo la stessa query precedente:
...
4. Configurare le impostazioni del server di origine.
   ...
   Replica dei dati in ingresso richiede che il parametro `lower_case_table_names` sia coerente tra i server di origine e di replica. Per impostazione predefinita, in Database di Azure per MySQL questo parametro è impostato su 1.

   ```sql
   SET GLOBAL lower_case_table_names = 1;
   ```

   **Facoltativo** : se si vuole usare la [replica basata su GTID](https://dev.mysql.com/doc/mysql-replication-excerpt/5.7/en/replication-gtids-concepts.html), è necessario verificare se GTID è abilitato nel server di origine. È possibile eseguire il comando seguente sul server MySQL di origine per verificare se gtid_mode è ON.
   ...
   >[!IMPORTANT]
   > Tutti i server hanno gtid_mode impostato sul valore predefinito OFF. Non è necessario abilitare GTID nel server MySQL di origine in modo specifico per configurare Replica dei dati in ingresso. Se GTID è già abilitato nel server di origine, è possibile usare facoltativamente la replica basata su GTID per configurare Replica dei dati in ingresso anche con il server singolo database di Azure per MySQL. È possibile utilizzare la replica basata su file per configurare la replica dei dati per tutti i server indipendentemente dalla configurazione di gitd_mode nel server di origine.

5. Creare un nuovo ruolo di replica e impostare l'autorizzazione.

   Creare un account utente nel server di origine configurato con privilegi di replica. Questa operazione può essere eseguita tramite i comandi SQL o uno strumento come MySQL Workbench. Valutare se si prevede di eseguire la replica con SSL, in quanto sarà necessario specificare durante la creazione dell'utente. Per informazioni su come [aggiungere gli account utente](https://dev.mysql.com/doc/refman/5.7/en/user-names.html) nel server di origine, vedere la documentazione di MySQL.
...
7. Ottenere l'offset e il nome del file di log binario.
   ...
   ```sql
    show master status;
   ```
   I risultati verranno visualizzati in modo simile al seguente. Assicurarsi di annotare il nome del file binario da usare nei passaggi successivi.
...
1. Determinare quali database e tabelle si vuole replicare nel database di Azure per MySQL ed eseguire il dump dal server di origine.
    ...
    È possibile usare mysqldump per eseguire il dump dei database dal server primario. Per informazioni dettagliate, vedere [Dump e ripristino](concepts-migrate-dump-restore.md). Non è necessario eseguire il dump della libreria MySQL e della libreria di test.

2. **Facoltativo** : se si vuole usare la [replica basata su gtid](https://dev.mysql.com/doc/mysql-replication-excerpt/5.7/en/replication-gtids-concepts.html), è necessario identificare il gtid dell'ultima transazione eseguita nel database primario. È possibile utilizzare il comando seguente per prendere nota del GTID dell'ultima transazione eseguita nel server master.
...
4. Ripristinare il file dump nel nuovo server.
...
5. **Facoltativo** : si noti la GTID del server ripristinato nel database di Azure per MySQL per assicurarsi che sia uguale al server primario. È possibile usare il comando seguente per prendere nota della GTID del valore GTID eliminato nel database di Azure per il server di replica MySQL. Il valore di gtid_purged deve essere uguale a quello gtid_executed nel Master indicato nel passaggio 2 per il funzionamento della replica basata su GTID.
...
## <a name="other-useful-stored-procedures-for-data-in-replication-operations"></a>Altre stored procedure utili per operazioni di Replica dei dati in ingresso
...
Per ignorare la transazione successiva dopo la posizione di replica corrente, usare il comando seguente per identificare il GTID della transazione successiva, come illustrato di seguito.
