...
Dopo aver creato un database di Azure per il server MySQL, è possibile usare il primo account amministratore del server per creare un maggior numero di utenti e concedere loro l'accesso amministrativo. È anche possibile usare l'account amministratore del server per creare utenti con privilegi meno elevati che hanno accesso ai singoli schemi di database.
...
## <a name="to-create-more-admin-users-in-azure-database-for-mysql"></a>Per creare altri utenti amministratore in database di Azure per MySQL
