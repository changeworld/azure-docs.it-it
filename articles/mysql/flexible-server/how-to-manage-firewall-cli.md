...
description: Creare e gestire le regole del firewall per database di Azure per MySQL-server flessibile usando la riga di comando di Azure CLI.
author: savjani
ms.author: pariks
ms.service: mysql
ms.devlang: azurecli
...
Usare il `az mysql flexible-server firewall-rule list` comando per elencare le regole del firewall del server esistenti nel server. Si noti che l'attributo nome server è specificato nell'opzione **--Name** .
```azurecli-interactive
...
Usare il `az mysql flexible-server firewall-rule update` comando per aggiornare una regola del firewall esistente nel server. Specificare il nome della regola del firewall esistente come input, nonché gli attributi indirizzo IP iniziale e indirizzo IP finale da aggiornare.
```azurecli-interactive
...
Usare il `az mysql flexible-server firewall-rule show` comando per visualizzare i dettagli della regola del firewall esistente dal server. Specificare il nome della regola del firewall esistente come input.
```azurecli-interactive
