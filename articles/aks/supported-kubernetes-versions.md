...
* **Le versioni principali cambiano** quando gli aggiornamenti dell'API incompatibili o la compatibilità con le versioni precedenti possono essere interrotte.
...
Eseguire la versione più recente della patch della versione secondaria in esecuzione. Ad esempio, il cluster di produzione si trova in **`1.17.7`** . **`1.17.8`** è l'ultima versione di patch disponibile per la *serie 1.17.* È consigliabile eseguire **`1.17.8`** l'aggiornamento a appena possibile per assicurarsi che il cluster sia completamente patchato e supportato.

## <a name="kubernetes-version-support-policy"></a>Criteri di supporto della versione di Kubernetes

Il servizio AKS definisce una versione disponibile a livello generale come versione abilitata in tutte le misurazioni SLO o SLA e disponibile in tutte le aree. Il servizio Kubernetes supporta tre versioni secondarie ga:

* La versione secondaria ga più recente rilasciata nel servizio AKS (che verrà fatto riferimento a come N).
* Due versioni secondarie precedenti.
    * Ogni versione secondaria supportata supporta anche un massimo di due (2) patch stabili.

Il servizio AKS può anche supportare le versioni di anteprima, etichettate in modo esplicito e soggette a termini [e condizioni di anteprima.][preview-terms]

> [!NOTE]
> Il servizio AKS usa procedure di distribuzione sicure che comportano una distribuzione graduale dell'area. Ciò significa che potrebbero essere necessario fino a 10 giorni lavorativi per la disponibilità di una nuova versione o di una nuova versione in tutte le aree.
...
Ad esempio, se *kube-apiserver* è alla *versione 1.17,* è possibile usare le versioni *da 1.16* a *1.18* di con `kubectl` *kube-apiserver*.
...
Per le **nuove versioni** secondarie di Kubernetes:
  * Il servizio Web Diaks pubblica un pre-annuncio con la data pianificata di una nuova versione e la deprecazione della rispettiva versione precedente nelle note sulla versione del servizio Disattesa almeno 30 giorni prima della rimozione. [](https://aka.ms/aks/releasenotes)
...
Quando si distribuisce un cluster del servizio AzureKs nel portale o con l'interfaccia della riga di comando di Azure, per impostazione predefinita il cluster è la versione secondaria N-1 e la patch più recente. Ad esempio, se il servizio Web Dias supporta *1.17.a*, *1.17.b*, *1.16.c*, *1.16.d*, *1.15.e* e *1.15.f*, la versione predefinita selezionata è *1.16.c*.
...
## <a name="aks-kubernetes-release-calendar"></a>Calendario delle versioni di Kubernetes del servizio Kubernetes

Per la cronologia delle versioni precedenti, vedere [Kubernetes.](https://en.wikipedia.org/wiki/Kubernetes#History)

|  Versione K8s | Versione upstream  | Anteprima del server Delktaks  | AKS GA  | Fine vita |
|--------------|-------------------|--------------|---------|-------------|
| 1.18  | 23-20 marzo  | Maggio 2020   | Agosto 2020  | 1.21 Ga | 
| 1,19  | Aug-04-20  | Settembre 2020   | Novembre 2020  | 1.22 Ga | 
| 1,20  | 20/08/2020  | Gennaio 2021   | Mar 2021  | 1.23 Ga |
...
Il team del servizio Azure Kubernetes pubblica pre-annunci con date pianificate delle nuove versioni di Kubernetes nella documentazione, [gitHub](https://github.com/Azure/AKS/releases) e messaggi di posta elettronica agli amministratori della sottoscrizione proprietari dei cluster che non saranno più supportati.  Oltre agli annunci, il servizio [](https://docs.microsoft.com/azure/advisor/advisor-overview) Azure Azure Advisor usa anche Azure Advisor per inviare una notifica al cliente all'interno del portale di Azure per avvisare gli utenti se non sono supportati, nonché per avvisarli delle API deprecate che influiranno sull'applicazione o sul processo di sviluppo. 

**Con quale frequenza è necessario prevedere l'aggiornamento delle versioni di Kubernetes per mantenere il supporto?**

A partire da Kubernetes 1.19, la community open source ha esteso il supporto [a 1 anno.](https://kubernetes.io/blog/2020/08/31/kubernetes-1-19-feature-one-year-support/) Il servizio AKS esegue il commit per abilitare le patch e supporta la corrispondenza degli impegni upstream. Per i cluster del servizio AKS nella versione 1.19 e successive, sarà possibile eseguire l'aggiornamento almeno una volta all'anno per mantenere una versione supportata. 

Per le versioni 1.18 o successive, la finestra di supporto rimane a 9 mesi, richiedendo un aggiornamento una volta ogni 9 mesi per mantenere una versione supportata. Testare regolarmente nuove versioni ed essere pronti per l'aggiornamento a versioni più recenti per acquisire i miglioramenti stabili più recenti all'interno di Kubernetes.

**Cosa accade quando un utente aggiorna un cluster Kubernetes con una versione secondaria non supportata?**

Se si ha la versione *n-3* o una versione precedente, significa che non si è al di fuori del supporto e verrà richiesto di eseguire l'aggiornamento. Quando l'aggiornamento dalla versione n-3 a n-2 ha esito positivo, si torna entro i criteri di supporto. Ad esempio:
...
- Quando si esegue correttamente l'aggiornamento dalla *versione 1.14.b* alla *versione 1.15.a* o successiva, si rientra nei criteri di supporto.

I downgrade non sono supportati.

**Significato di "Al di fuori del supporto"**

"Al di fuori del supporto" significa che:
* La versione in esecuzione non rientra nell'elenco delle versioni supportate.
* Verrà richiesto di aggiornare il cluster a una versione supportata quando si richiede supporto, a meno che non si sia entro il periodo di tolleranza di 30 giorni dopo la deprecazione della versione. 

Inoltre, il servizio Diaks non garantisce il runtime o altre garanzie per i cluster al di fuori dell'elenco delle versioni supportate.

**Cosa accade quando un utente ridimensiona un cluster Kubernetes con una versione secondaria non supportata?**

Per le versioni secondarie non supportate dal server del supporto di AKS, l'aumento o la riduzione delle dimensioni dovrebbe continuare a funzionare. Poiché non esistono garanzie di qualità del servizio, è consigliabile eseguire l'aggiornamento per riportare il cluster in supporto.

**Un utente può rimanere in una versione di Kubernetes per sempre?**

Se un cluster non è più in supporto per più di tre (3) versioni secondarie ed è stato rilevato che comporta rischi per la sicurezza, Azure contatta in modo proattivo l'utente per aggiornare il cluster. Se non si eserviranno altre azioni, Azure si riserva il diritto di aggiornare automaticamente il cluster per conto dell'utente.

**Quale versione è supportata dal piano di controllo se il pool di nodi non è in una delle versioni supportate del servizio Web Disacks?**
...
**È possibile ignorare più versioni del servizio Web Disassoce durante l'aggiornamento del cluster?**
...
È possibile ignorare più versioni solo quando si esegue l'aggiornamento da una versione non supportata a una versione supportata. Ad esempio, è possibile eseguire l'aggiornamento da una *versione 1.10.x* non supportata a una *versione 1.15.x supportata.*

**È possibile creare un nuovo cluster 1.xx.x durante la finestra di supporto di 30 giorni?**

No. Dopo che una versione è stata deprecata o rimossa, non è possibile creare un cluster con tale versione. Con l'implementazione della modifica, si inizierà a vedere la versione precedente rimossa dall'elenco delle versioni. Questo processo può richiedere fino a due settimane dall'annuncio, progressivamente in base all'area.

**Se si ha una versione appena deprecata, è comunque possibile aggiungere nuovi pool di nodi? O è necessario eseguire l'aggiornamento?**

No. Non sarà possibile aggiungere pool di nodi della versione deprecata al cluster. È possibile aggiungere pool di nodi di una nuova versione. Tuttavia, potrebbe essere necessario aggiornare prima il piano di controllo. 
