...
| Registra | Unità di dati inviata dal dispositivo per la creazione di report. Questa operazione viene spesso definita "log", "Event" o "Alert", ma non necessariamente uno di questi. |
|
...
| **ID utente** | string | Sono attualmente supportati i due ID utente seguenti:<ul><li>SID utente</li><li>ID di Azure Active Directory</li></ul> |
| **Dispositivo** | string | Sono supportate le 3 colonne del dispositivo/host seguenti:<ul><li>ID</li><li>Nome</li><li>Nome di dominio completo (FQDN)</li></ul> |
| **Paese** | string | Stringa che usa ISO 3166-1 in base a questa priorità:<ul><li>Codici alfa-2 (ad esempio, US per la Stati Uniti)</li><li>Codici alfa-3 (ad esempio USA per la Stati Uniti)</li><li>Nome breve</li></ul> |
| **Area** | string | Nome della sottodivisione paese con ISO 3166-2 |
| **Città** | string | |
...
| **Algoritmo hash** | string | Sono supportate le 4 colonne hash seguenti:<ul><li>MD5</li><li>SHA1</li><li>SHA256</li><li>SHA512</li></ul> |
| **Tipo di file** | string | Tipo di file:<ul><li>Estensione</li><li>Classe</li><li>NamedType</li></ul> |
| 
...
| DvcMacAddr | string | 06:10:9F: EB: 8F: 14 | Indirizzo MAC dell'interfaccia di rete del dispositivo di report da cui è stato inviato l'evento. | Dispositivo<br>Mac |
...
| EventOriginalUid | string | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | ID record del dispositivo di report. | Evento |
...
| DstDvcFqdn | string | victim_pc. contoso. local | Nome di dominio completo dell'host in cui è stato creato il log | Destinazione<br>Dispositivo |
...
| DstMacAddr | string | 06:10:9F: EB: 8F: 14 | Indirizzo MAC dell'interfaccia di rete a cui è stata interrotta la connessione o la sessione, che in genere si fa riferimento al MAC di destinazione nel pacchetto di rete | Destinazione<br>MAC |
| DstDvcMacAddr | string | 06:10:9F: EB: 8F: 14 | Indirizzo MAC di destinazione di un dispositivo non associato direttamente al pacchetto di rete. | Destinazione<br>Dispositivo<br>MAC |
| DstDvcDomain | string | CONTOSO | Dominio del dispositivo di destinazione. | Destinazione<br>Dispositivo |
...
| NetworkRuleName | string | AnyAnyDrop | Nome o ID della regola con cui è stato deciso DeviceAction | Rete |
...
| SrcGeoCity | string | Burlington | Città associata all'indirizzo IP di origine | Origine<br>Area geografica |
...
| SrcDvcOs | string | iOS | Sistema operativo del dispositivo di origine | Origine<br>Dispositivo |
| SrcDvcModelName | string | Nota Samsung Galaxy | Nome del modello del dispositivo di origine | Origine<br>Dispositivo |
| SrcDvcModelNumber | string | 10,0 | Il numero di modello del dispositivo di origine | Origine<br>Dispositivo |
| SrcDvcType | string | Dispositivi mobili | Tipo del dispositivo di origine | Origine<br> Dispositivo |
| SrcIntefaceName | string | eth01 | Interfaccia di rete usata per la connessione o la sessione dal dispositivo di origine. | Source (Sorgente) |
| SrcInterfaceGuid | string | 46ad544b-eaf0-47ef-827c-266030f545a6 | GUID dell'interfaccia di rete utilizzata | Source (Sorgente) |
...
| SrcMacAddr | string | 06:10:9F: EB: 8F: 14 | Indirizzo MAC dell'interfaccia di rete da cui ha avuto origine la sessione di connessione od. | Origine<br>Mac |
| SrcDvcMacAddr | string | 06:10:9F: EB: 8F: 14 | Indirizzo MAC di origine di un dispositivo che non è direttamente associato al pacchetto di rete. | Origine<br>Dispositivo<br>Mac |
...
| CloudAppId | string | 124 | ID dell'applicazione di destinazione per un'applicazione HTTP come identificato da un proxy. Questo valore è in genere specifico del proxy utilizzato. | Cloud |
| CloudAppOperation | string | DeleteFile | Operazione eseguita dall'utente nel contesto dell'applicazione di destinazione per un'applicazione HTTP identificata da un proxy. Questo valore è in genere specifico del proxy utilizzato. | Cloud |
| CloudAppRiskLevel | string | 3 | Livello di rischio associato a un'applicazione HTTP identificato da un proxy. Questo valore è in genere specifico del proxy utilizzato. | Cloud |
...
| FilePath | string | C:\Malicious\ImNotMalicious.exe | Percorso completo, incluso il nome file, del file | File |
| FileHashMd5 | string | 51BC68715FC7C109DCEA406B42D9D78F | Valore hash MD5 del file trasmesso sulle connessioni di rete per i protocolli. | File |
| FileHashSha1 | string | 491AE3... C299821476F4 | Valore hash SHA1 del file trasmesso sulle connessioni di rete per i protocolli. | File |
| FileHashSha256 | string | 9B8F8EDB... C129976F03 | Valore hash SHA256 del file trasmesso sulle connessioni di rete per i protocolli. | File |
| FileHashSha512 | string | 5E127D... F69F73F01F361 | Valore hash SHA512 del file trasmesso sulle connessioni di rete per i protocolli. | File |
| FileExtension |  string | exe | Tipo di file trasmesso sulle connessioni di rete per protocolli quali FTP e HTTP. | File
| FileMimeType | string | applicazione/MSWord | Tipo MIME del file trasmesso sulle connessioni di rete per protocolli quali FTP e HTTP | File |
...
| HttpContentType | string | multipart/form-data; limite = qualcosa | Intestazione del tipo di contenuto della risposta HTTP per le sessioni di rete HTTP/HTTPS. | Http |
...
| HttpUserAgentOriginal | string | Mozilla/5.0 (Windows NT 10,0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.97 Safari/537.36 | Intestazione agente utente HTTP per le sessioni di rete HTTP/HTTPS. | Http |
| HttpRequestXff | string | 120.12.41.1 | Intestazione HTTP X-Inoltred-for per le sessioni di rete HTTP/HTTPS. | Http |
| UrlCategory | string | Motori di ricerca | Il raggruppamento definito di un URL (o potrebbe essere basato solo sul dominio nell'URL) correlato a ciò che è (ad esempio, per adulti, notizie, pubblicità, domini parcheggiati e così via) | url |
...
| ThreatCategory | string | Trojan | La categoria di una minaccia identificata da un sistema di sicurezza, ad esempio il gateway di sicurezza Web di un IP, è associata a questa sessione di rete. | Minaccia |
| ThreatId | string | TR. 124 | ID di una minaccia identificata da un sistema di sicurezza, ad esempio un gateway di sicurezza Web di un IP, associato a questa sessione di rete. | Minaccia |
| Threatname | string | File di test di EICAR | Nome della minaccia o del malware identificato | Minaccia |
