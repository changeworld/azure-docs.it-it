...
description: In questa Guida introduttiva si apprenderà come eseguire l'onboarding di Azure Sentinel, abilitando prima di tutto la connessione e quindi le origini dati.
services: sentinel
author: yelevin
ms.author: yelevin
...
- Azure Sentinel può essere eseguito in aree di lavoro nella maggior parte delle [aree di disponibilità generale di Log Analytics](https://azure.microsoft.com/global-infrastructure/services/?products=monitor), ad eccezione delle aree Cina e Germania (sovrane). A volte le nuove aree di Log Analytics potrebbero richiedere del tempo per l'onboarding del servizio Sentinel di Azure. 
...
- I dati generati da Azure Sentinel, ad esempio eventi imprevisti, segnalibri e regole di analisi, possono contenere dati del cliente originati dalle aree di lavoro Log Analytics del cliente. I dati generati da Azure Sentinel vengono salvati nell'area geografica elencata nella tabella seguente, in base all'area geografica in cui si trova l'area di lavoro:
    ...
    | --- | --- |
    | Stati Uniti<br>India<br>Brasile<br>Africa<br>Corea del Sud<br>Emirati Arabi Uniti | Stati Uniti |
