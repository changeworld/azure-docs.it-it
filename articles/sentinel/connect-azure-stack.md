...
description: Questo articolo illustra come eseguire il provisioning dell'estensione macchina virtuale di monitoraggio, aggiornamento e gestione della configurazione di Azure nelle macchine virtuali di Azure Stack Hub e iniziare a monitorarli con Azure Sentinel.
services: sentinel
...
1. Fare clic sulla scheda **Add** (Aggiungi). Viene visualizzato il pannello del menu **Nuova risorsa** che contiene un elenco delle estensioni macchina virtuale disponibili. 

1. Selezionare l'estensione **monitoraggio di Azure, aggiornamento e gestione della configurazione** e fare clic su **Crea**. Viene visualizzata la finestra di configurazione dell' **estensione di installazione** .
...
1. Nel menu Azure Sentinel selezionare impostazioni dell' **area di lavoro** , quindi **Avanzate** e copiare l' **ID** dell'area di lavoro e la chiave dell' **area di lavoro (chiave primaria)**. 
...
Per ulteriori informazioni sull'installazione e la configurazione dell'agente per Windows, vedere [connettere computer Windows](../azure-monitor/agents/agent-windows.md#install-agent-using-setup-wizard).

Per risolvere i problemi relativi agli agenti Linux, vedere [Risoluzione dei problemi relativi all'agente Linux di Azure Log Analytics](../azure-monitor/agents/agent-linux-troubleshoot.md).
...
1. Passare alla pagina **Macchine virtuali** e selezionare la macchina virtuale da cui si vuole rimuovere l'estensione.

1. Selezionare **Estensioni** e quindi l'estensione **Microsoft.EnterpriseCloud.Monitoring**.

1. Fare clic su **Disinstalla** e confermare la selezione.
