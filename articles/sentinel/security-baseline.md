...
ms.locfileid: "104604237"
---
# <a name="azure-security-baseline-for-azure-sentinel"></a>Baseline della sicurezza di Azure per Sentinel di Azure

Questa linea di base di sicurezza applica le linee guida del [benchmark di sicurezza di Azure versione 1,0](../security/benchmarks/overview-v1.md) ad Azure Sentinel. Azure Security Benchmark offre consigli sulla protezione delle soluzioni cloud in Azure.
Il contenuto viene raggruppato in base ai **controlli di sicurezza** definiti dal benchmark di sicurezza di Azure e alle linee guida correlate applicabili a Sentinel di Azure. I **controlli** non applicabili ad Azure Sentinel sono stati esclusi.

 
Per informazioni sul mapping completo di Azure Sentinel al benchmark di sicurezza di Azure, vedere il [file di mapping della linea di base di sicurezza di Azure Sentinel completo](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Linee guida**: usare Azure Active Directory (Azure ad) come sistema di autenticazione e autorizzazione centrale per le aree di lavoro di Sentinel di Azure. Azure AD protegge i dati usando la crittografia avanzata per i dati inattivi e in transito. Azure AD effettua anche il salting, aggiunge hash e archivia in modo sicuro le credenziali utente.

È possibile accedere alle origini del registro eventi di attività, controllo e rischio Azure AD di accesso, che consentono di eseguire l'integrazione con Azure Sentinel o una soluzione di gestione degli eventi e delle informazioni di sistema di terze parti.
...
**Linee guida**: implementare sottoscrizioni separate per includere facoltativamente i gruppi di gestione per le aree di lavoro di sviluppo, test e produzione Sentinel.
...
- [Panoramica della chiave gestita dal cliente di monitoraggio di Azure](https://docs.microsoft.com/azure/azure-monitor/logs/customer-managed-keys#customer-managed-key-overview) 
