...
Questo articolo descrive come configurare il metodo di routing del traffico della subnet. Il metodo di routing del traffico della **subnet** consente di eseguire il mapping di un set di intervalli di indirizzi IP a endpoint specifici. Quando una richiesta viene ricevuta da Gestione traffico, esamina l'indirizzo IP di origine della richiesta e restituisce l'endpoint associato.

In questa esercitazione, usando il routing delle subnet, a seconda dell'indirizzo IP della query dell'utente, il traffico viene indirizzato a un sito Web interno o a un sito Web di produzione.
...
Per visualizzare Gestione traffico in azione, per questa esercitazione è necessario distribuire quanto segue:
...
In questa sezione si installa il server IIS nelle due macchine virtuali, *myIISVMEastUS* e *myIISVMWestEurope*, e quindi si aggiorna la pagina predefinita del sito Web. La pagina sito Web personalizzato Mostra il nome della macchina virtuale a cui ci si connette quando si visita il sito Web da un Web browser.
...
2. Nella pagina **Panoramica** selezionare **Connetti** e quindi in **Connetti a macchina virtuale** selezionare **Scarica file RDP**.
...
4. Selezionare **OK**.
5. Durante il processo di accesso potrebbe essere visualizzato un avviso relativo al certificato. Se viene visualizzato un avviso, selezionare **Sì** o **continua** per continuare con la connessione.
...
2. Nel **profilo crea gestione traffico** immettere o selezionare le informazioni seguenti. Accettare i valori predefiniti per le impostazioni rimanenti e quindi selezionare **Crea**.
...
3. Immettere o selezionare le informazioni seguenti. Accettare i valori predefiniti per le impostazioni rimanenti e quindi selezionare **OK**:
...
2. Selezionare la scheda di **informazioni generali**.
