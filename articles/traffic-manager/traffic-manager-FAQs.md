...
### <a name="does-traffic-manager-support-sticky-sessions"></a>Gestione traffico supporta le sessioni "sticky"?
...
Sì. Per informazioni su come creare un record alias per il dominio apex per fare riferimento a un profilo Gestione traffico di Azure, vedere Configurare un record alias per supportare nomi di dominio [apex con Gestione traffico.](../dns/tutorial-alias-tm.md)
...
No, Gestione traffico non può garantire che l'area geografica dedotto dall'indirizzo IP di origine di una query DNS corrisponda sempre alla posizione dell'utente per i motivi seguenti:

- In primo luogo, come indicato nella domanda precedente, l'indirizzo IP di origine visualizzato è quello di un resolver DNS che esegue la ricerca per conto dell'utente. La posizione geografica del resolver DNS è un proxy valido per la posizione geografica dell'utente ma può anche essere diversa, a seconda della superficie del servizio resolver DNS e dello specifico servizio resolver DNS che un cliente sceglie di usare. Ad esempio, un cliente che si trova in Singapore potrebbe specificare nelle impostazioni del dispositivo di usare un servizio resolver DNS il cui server DNS a Singapore potrebbe essere scelto per gestire le risoluzioni delle query per tale utente/dispositivo. In tal caso, Gestione traffico può visualizzare solo l'indirizzo IP del resolver corrispondente alla località Singapore. Vedere anche le domande frequenti precedenti, disponibili in questa pagina, relative al supporto dell'indirizzo della subnet client.
...
-    Nel caso della notazione CIDR, l'indirizzo IP prima di '/' deve essere l'indirizzo iniziale di tale intervallo (ad esempio, 1.2.3.0/24 è valido ma 1.2.3.4.4/24 non è valido)
...
In un profilo con routing Subnet, se un endpoint è presente ma disabilitato, Gestione traffico si comporta come se tale endpoint e i relativi mapping della subnet non esistessero. Se viene ricevuta una query corrispondente al relativo mapping degli indirizzi IP e l'endpoint è disabilitato, Gestione traffico restituirà un endpoint di fallback (uno senza mapping) oppure, se tale endpoint non è presente, restituirà una risposta NXDOMAIN.
...
Un altro uso del metodo di routing MultiValue è se un endpoint è "dual-homed" per gli indirizzi IPv4 e IPv6 e si vuole fornire al chiamante entrambe le opzioni tra cui scegliere quando avvia una connessione all'endpoint.
...
No, non è necessario usare Gestione traffico. Il lato di routing di Gestione traffico funziona separatamente dalla parte Misurazione utente reale e, sebbene sia una buona idea avere entrambi nella stessa proprietà Web, non è necessario che lo sia.
...
No, non è necessario ospitare componenti sul lato server in Azure per misurazioni utente reale funzionamento. L'immagine a pixel singolo scaricata dal codice JavaScript di misurazione e il servizio che lo esegue in aree di Azure diverse sono ospitati e gestiti da Azure. 
...
Monitoraggio di Azure aiuta a comprendere, a livello di aggregazione, il traffico ricevuto dal proprio profilo e dai relativi endpoint. Il servizio consente di tenere traccia dello stato di integrità degli endpoint tramite l'esposizione dei risultati del controllo di integrità. Quando è necessario superare questi limiti e comprendere l'esperienza dell'utente finale che si connette ad Azure a livello di Visualizzazione traffico a tale scopo.
...
### <a name="how-do-i-move-my-traffic-manager-profiles-azure-endpoints-to-a-different-resource-group-or-subscription"></a>Ricerca per categorie gli endpoint di Azure del profilo di Gestione traffico in un gruppo di risorse o una sottoscrizione diversa?

Per tenere traccia degli endpoint di Azure associati a un profilo di Gestione traffico vengono usati i relativi ID di risorsa. Quando una risorsa di Azure usata come endpoint (ad esempio, IP pubblico, servizio cloud classico, WebApp o un altro profilo di Gestione traffico usato in modo annidato) viene spostata in un gruppo di risorse o una sottoscrizione diversa, il relativo ID risorsa cambia. In questo scenario, è attualmente necessario aggiornare il profilo di Gestione traffico eliminando e riaggiungendo gli endpoint al profilo.
...
### <a name="i-stopped-an-web-application-endpoint-in-my-traffic-manager-profile-but-i-am-not-receiving-any-traffic-even-after-i-restarted-it-how-can-i-fix-this"></a>L'endpoint di un'applicazione Web è stato arrestato nel profilo di Gestione traffico, ma non si riceve traffico anche dopo il riavvio. Come si risolve questo problema?
...
- Se viene ricevuta una risposta SYN-ACK entro il periodo di timeout specificato nelle impostazioni di monitoraggio, tale endpoint viene considerato integro. Una FIN o FIN-ACK è la risposta prevista da Gestione traffico quando termina regolarmente un socket.
...
Un caso d'uso comune è specificare intestazioni host in modo che le richieste di Gestione traffico possano essere instradate correttamente a un endpoint ospitato in un ambiente multi-tenant. Un altro caso d'uso di questo tipo è identificare le richieste di Gestione traffico dai log delle richieste HTTP(S) di un endpoint
...
Fare [clic qui](../virtual-network/service-tags-overview.md#use-the-service-tag-discovery-api-public-preview) per informazioni su come recuperare gli elenchi di indirizzi IP da cui possono avere origine i controlli di integrità di Gestione traffico. È possibile usare l'API REST, l'interfaccia della riga di comando di Azure o Azure PowerShell per recuperare l'elenco più recente. Esaminare gli indirizzi IP elencati per assicurarsi che le connessioni in ingresso da questi indirizzi IP siano consentite agli endpoint per controllarne lo stato di integrità.

Esempio di uso Azure PowerShell:
...
> [!NOTE]
> Gli indirizzi IP pubblici possono cambiare senza preavviso. Assicurarsi di recuperare le informazioni più recenti usando l'API di individuazione dei tag di servizio o un file JSON scaricabile.
