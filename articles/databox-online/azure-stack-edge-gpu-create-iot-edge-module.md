...
description: Informazioni su come sviluppare un modulo IoT Edge C# che può essere distribuito nel Azure Stack Edge Pro GPU.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
Questo articolo illustra come creare un modulo di IoT Edge per la distribuzione con il Azure Stack Edge Pro dispositivo. Azure Stack Edge Pro è una soluzione di archiviazione che consente di elaborare i dati e inviarli in rete ad Azure.

È possibile usare Azure IoT Edge moduli con il Azure Stack Edge Pro per trasformare i dati durante lo spostamento in Azure. Il modulo usato in questo articolo implementa la logica per copiare un file da una condivisione locale a una condivisione cloud nel Azure Stack Edge Pro dispositivo.
...
![Funzionamento Azure IoT Edge modulo in Azure Stack Edge Pro](./media/azure-stack-edge-gpu-create-iot-edge-module/how-module-works-1.png)
...
- Un Azure Stack Edge Pro in esecuzione.

    - Il dispositivo ha anche una risorsa dell'hub IoT associata.
    - Il dispositivo ha il ruolo di calcolo Edge configurato.
    Per altre informazioni, vedere [Configurare le risorse di](azure-stack-edge-j-series-deploy-configure-compute.md#configure-compute) calcolo per Azure Stack Edge Pro.<!--Update link?-->
...
3. Specificare:
   ...
   4. Selezionare un **percorso**. È consigliabile che questo percorso sia uguale a quello associato alla Azure Stack Edge risorsa.
...
5. Trovare e puntare alla cartella **EdgeSolution** creata in precedenza. La finestra di VS Code carica l'area di lavoro della soluzione IoT Edge con i cinque componenti di livello superiore. Non si modificherà la cartella, il file con estensione `.vscode` **gitignore,** il file con estensione **env** e deployment.template.jssu** in questo articolo.
...
8. È anche possibile [scaricare un esempio di codice esistente](https://azure.microsoft.com/resources/samples/data-box-edge-csharp-modules/?cdn=disable) per questo progetto. È quindi possibile convalidare il file salvato nel file **program.cs** in questo esempio.
...
3. Dopo aver specificato le credenziali, è quindi possibile eseguire il push dell'immagine del modulo in Registro Azure Container. Nello strumento di esplorazione di VS Code fare clic con il pulsante destro del mouse sul file **module.json** e scegliere **Build and Push IoT Edge solution** (Compila ed esegui il push della soluzione IoT Edge).
    ...
    ![Compilare ed eseguire il push IoT Edge soluzione 2](./media/azure-stack-edge-gpu-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    *Program.cs(77,44): avviso CS1998: questo metodo asincrono non dispone di operatori 'await' e verrà eseguito in modo sincrono. È consigliabile usare l'operatore 'await' per attendere chiamate API non bloccante o 'await Task.Run(...)' per eseguire operazioni associate alla CPU in un thread in background.*
