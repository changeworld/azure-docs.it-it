---
title: Distribuire il carico di lavoro Kubernetes usando la condivisione GPU nel dispositivo GPU Pro Azure Stack Edge
description: Viene descritto come distribuire un carico di lavoro condiviso GPU tramite Kubernetes nel dispositivo GPU Pro Azure Stack Edge.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "103565140"
---
# <a name="deploy-a-kubernetes-workload-using-gpu-sharing-on-your-azure-stack-edge-pro"></a>Distribuire un carico di lavoro Kubernetes usando la condivisione GPU in Azure Stack Edge Pro

Questo articolo descrive il modo in cui i carichi di lavoro in contenitori possono condividere le GPU sul dispositivo GPU Azure Stack Edge Pro. In questo articolo vengono eseguiti due processi, uno senza la condivisione del contesto GPU e uno con la condivisione del contesto abilitata tramite il servizio multiprocesso (MP) nel dispositivo. Per ulteriori informazioni, vedere il [servizio](https://docs.nvidia.com/deploy/pdf/CUDA_Multi_Process_Service_Overview.pdf)a più processi.
...
1. Sono stati creati uno spazio dei nomi e un utente. È stato anche concesso all'utente l'accesso a questo spazio dei nomi. Il file kubeconfig di questo spazio dei nomi è installato nel sistema client che verrà usato per accedere al dispositivo. Per istruzioni dettagliate, vedere [connettersi a un cluster Kubernetes e gestirlo tramite kubectl sul dispositivo GPU Azure stack Edge Pro](azure-stack-edge-gpu-create-kubernetes-cluster.md#configure-cluster-access-via-kubernetes-rbac). 

1. Salvare la distribuzione seguente `yaml` nel sistema locale. Questo file verrà usato per eseguire la distribuzione di Kubernetes. Questa distribuzione è basata su [semplici contenitori CUDA](https://docs.nvidia.com/cuda/wsl-user-guide/index.html#running-simple-containers) disponibili pubblicamente da NVIDIA. 
...
## <a name="job-without-context-sharing"></a>Processo senza condivisione del contesto

Il primo processo verrà eseguito per distribuire un'applicazione nel dispositivo nello spazio dei nomi `mynamesp1` . Questa distribuzione dell'applicazione indicherà anche che la condivisione del contesto della GPU non è abilitata per impostazione predefinita. 

1. Elencare tutti i pod in esecuzione nello spazio dei nomi. Eseguire il comando seguente: 
    ...
    No resources found.
    ```
1. Avviare un processo di distribuzione nel dispositivo usando la distribuzione. YAML fornita in precedenza. Eseguire il comando seguente: 

    ```powershell
    kubectl apply -f <Path to the deployment .yaml> -n <Name of the namespace> 
    ```  

    Questo processo crea due contenitori ed esegue una simulazione n-body in entrambi i contenitori. Il numero di iterazioni della simulazione viene specificato in `.yaml` . Per ulteriori informazioni, vedere [simulazione del corpo N](https://physics.princeton.edu//~fpretori/Nbody/intro.htm).
...
1. Per elencare i pod avviati nella distribuzione, eseguire il comando seguente:
    ...
    Sono presenti due pod `cuda-sample1-cf979886d-xcwsq` e in `cuda-sample2-68b4899948-vcv68` esecuzione nel dispositivo.

1. Recuperare i dettagli dei pod. Eseguire il comando seguente:
    ...
    PS C:\WINDOWS\system32>
    ```
    L'output indica che entrambi i pod sono stati creati correttamente dal processo. 
...
1. Mentre entrambi i contenitori eseguono la simulazione n-body, visualizzare l'utilizzo della GPU dall'output di NVIDIA SMI. Passare all'interfaccia di PowerShell del dispositivo ed eseguire `Get-HcsGpuNvidiaSmi` .
    ...
    [10.100.10.10]: PS>    
    ```
    Come si può notare, sono disponibili due contenitori (Type = C) in esecuzione con simulazione a n corpo sulla GPU 0. 

1. Monitorare la simulazione del corpo n. Eseguire i `get pod` comandi. Di seguito è riportato un esempio di output durante l'esecuzione della simulazione. 
    ...
    Al termine della simulazione, l'output indicherà che. Di seguito è riportato un esempio di output:
...
1. Al termine della simulazione, è possibile visualizzare i log e il tempo totale per il completamento della simulazione. Eseguire il comando seguente:
    ...
    PS C:\WINDOWS\system32>    
    ```
1. Al momento non devono essere presenti processi in esecuzione nella GPU. Per verificarlo, è possibile visualizzare l'utilizzo della GPU usando l'output di NVIDIA SMI.
...
## <a name="job-with-context-sharing"></a>Processo con condivisione del contesto

Si eseguirà il secondo processo per distribuire la simulazione n-body in due contenitori CUDA quando è abilitata la condivisione del contesto GPU, sebbene i MP. In primo luogo, verranno abilitati i MPS sul dispositivo.
...
1. Per abilitare i Management Pack nel dispositivo, eseguire il `Start-HcsGpuMPS` comando.
    ...
    [10.100.10.10]: PS>    
    ```
1. Eseguire il processo usando la stessa distribuzione `yaml` usata in precedenza. Potrebbe essere necessario eliminare la distribuzione esistente. Vedere [eliminare la distribuzione](#delete-deployment).
...
1. Mentre è in esecuzione la simulazione, è possibile visualizzare l'output di NVIDIA SMI. L'output Mostra i processi corrispondenti ai contenitori CUDA (tipo M + C) con la simulazione n-body e il servizio MPS (tipo C) come in esecuzione. Tutti questi processi condividono la GPU 0.
...
1. Al termine della simulazione, è possibile visualizzare i log e il tempo totale per il completamento della simulazione. Eseguire il comando seguente:
    ...
    ```powershell
        ...
        PS C:\WINDOWS\system32>
    ```
1. Al termine della simulazione, è possibile visualizzare di nuovo l'output di NVIDIA SMI. Solo il processo NVIDIA-CUDA-MPS-server per il servizio MP viene visualizzato come in esecuzione. 
...
Potrebbe essere necessario eliminare le distribuzioni quando si esegue con MP abilitato e con MP disabilitare nel dispositivo.
...
- [Distribuire un carico di lavoro di IOT Edge con la condivisione GPU in Azure stack Edge Pro](azure-stack-edge-gpu-deploy-iot-edge-gpu-sharing.md).
