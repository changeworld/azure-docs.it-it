...
![Diagramma per Azure Resource Manager](media/azure-stack-edge-gpu-connect-resource-manager/edge-device-flow.svg)
...
3. Per indicazioni, vedere l'immagine seguente. Salvare il file **hosts**.
    ...
    ![File hosts nel Blocco note](media/azure-stack-edge-gpu-connect-resource-manager/hosts-file-notepad.png)
...
1. È possibile utilizzare l'utilità da riga di comando ping.exe per verificare che il nome dell'endpoint sia stato risolto. Dato un indirizzo IP, il comando ping restituirà il nome host TCP/IP del computer che si sta \' ritracciando.
    ...
    ![Ping nel prompt dei comandi](media/azure-stack-edge-gpu-connect-resource-manager/ping-command-prompt.png)
...
1. Usare il cmdlet `Add-AzureRmEnvironment` per assicurarsi che la comunicazione tramite Azure Resource Manager funzioni correttamente e che le chiamate API attraversino la porta dedicata per Azure Resource Manager - 443.

    Il cmdlet `Add-AzureRmEnvironment` aggiunge endpoint e metadati per consentire ai cmdlet di Azure Resource Manager di connettersi a una nuova istanza di Azure Resource Manager. 
...
2. Impostare l'ambiente come Azure Stack Edge Pro e la porta da usare per le chiamate di Azure Resource Manager deve essere la porta 443. L'ambiente viene definito in due modi:
...
2. Chiamare le API del dispositivo locale per autenticare le connessioni ad Azure Resource Manager. 
    ...
    2. È possibile connettersi tramite il `login-AzureRMAccount` comando o via `Connect-AzureRMAccount` . 
        ...
        1. Per accedere, digitare il comando seguente. L'ID tenant in questa istanza è hardcoded-c0257de7-538f-415c-993a-1b87a031879d. Usare il nome utente e la password seguenti.
            ...
            - **Password**  -  di [Impostare la password per Azure Resource Manager](azure-stack-edge-gpu-set-azure-resource-manager-password.md) e usare questa password per accedere. 
...
[Distribuire macchine virtuali nel dispositivo Azure Stack Edge Pro](azure-stack-edge-gpu-deploy-virtual-machine-powershell.md).
