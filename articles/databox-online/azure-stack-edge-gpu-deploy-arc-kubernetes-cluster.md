...
1. Per creare un'entità servizio, usare il comando seguente tramite `az cli` .
    ...
    Per informazioni su come accedere a `az cli` , [avviare Cloud Shell in portale di Azure](../cloud-shell/quickstart-powershell.md#start-cloud-shell)
...
1. Dopo aver creato la nuova entità servizio, assegnare il `Kubernetes Cluster - Azure Arc Onboarding` ruolo all'entità appena creata. Si tratta di un ruolo predefinito di Azure (usare l'ID del ruolo nel comando) con autorizzazioni limitate. Usare il comando seguente:
    ...
    PS /home/user>
    ```
    Per altre informazioni su come creare un'entità servizio ed eseguire l'assegnazione di ruolo, vedere la procedura descritta in [creare un'entità servizio di onboarding abilitata per Azure Arc](../azure-arc/kubernetes/create-onboarding-service-principal.md).
...
Per informazioni su come eseguire una distribuzione di Azure Arc, vedere [distribuire un' `Guestbook` applicazione php senza stato con Redis tramite GitOps in un dispositivo Azure stack Edge Pro](azure-stack-edge-gpu-deploy-stateless-application-git-ops-guestbook.md)