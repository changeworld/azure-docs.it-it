...
* > [!IMPORTANT]
  > Durante la fase di anteprima del modulo **Az.CostManagement** di PowerShell, è necessario installarlo separatamente usando il cmdlet `Install-Module`. Quando il modulo di PowerShell diventerà disponibile a livello generale, entrerà a far parte delle future versioni del modulo Az di PowerShell e sarà disponibile per impostazione predefinita all'interno di Azure Cloud Shell.

  ```azurepowershell-interactive
  Install-Module -Name Az.CostManagement
  ```

1. Dopo aver eseguito l'accesso, usare il cmdlet [Get-AzCostManagementExport](/powershell/module/Az.CostManagement/get-azcostmanagementexport) per visualizzare le esportazioni correnti:
...
1. Creare un gruppo di risorse o usarne uno esistente. Per creare un gruppo di risorse, usare il cmdlet [New-AzResourceGroup](/powershell/module/az.resources/new-azresourcegroup):

   ```azurepowershell-interactive
   New-AzResourceGroup -Name TreyNetwork -Location eastus
   ```

1. Creare un account di archiviazione in cui ricevere i dati esportati oppure usarne uno esistente. Per creare un account di archiviazione, usare il cmdlet [New-AzStorageAccount](/powershell/module/az.storage/new-azstorageaccount):

   ```azurepowershell-interactive
   New-AzStorageAccount -ResourceGroupName TreyNetwork -AccountName cmdemo -SkuName Standard_RAGRS -Location eastus
   ```

1. Per creare l'esportazione, eseguire il cmdlet [New-AzCostManagementExport](/powershell/module/Az.CostManagement/new-azcostmanagementexport):
   ...
   Per il parametro **DefinitionType** è possibile scegliere `ActualCost`, `AmortizedCost` o `Usage`.

   In questo esempio viene utilizzato `MonthToDate`. L'esportazione crea un file ogni giorno per i costi da inizio mese. I dati più recenti vengono aggregati dalle esportazioni giornaliere del mese corrente.

1. Per visualizzare i dettagli dell'operazione di esportazione, usare il cmdlet `Get-AzCostManagementExport`:

   ```azurepowershell-interactive
   Get-AzCostManagementExport -Scope 'subscriptions/00000000-0000-0000-0000-000000000000'
   ```

1. Per aggiornare un'esportazione, usare il cmdlet [Update-AzCostManagementExport](/powershell/module/Az.CostManagement/update-azcostmanagementexport):
...
È possibile eliminare un'esportazione usando il cmdlet [Remove-AzCostManagementExport](/powershell/module/Az.CostManagement/remove-azcostmanagementexport):
