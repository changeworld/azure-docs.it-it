...
description: Informazioni su come pagare le sottoscrizioni di Azure tramite fattura. Per altre informazioni, vedere le domande frequenti.
author: bandersmsft
...
ms.locfileid: "99052482"
---
# <a name="pay-for-your-azure-subscription-by-invoice"></a>Pagare la sottoscrizione di Azure tramite fattura

Questo articolo si applica ai clienti con un Contratto del cliente Microsoft che hanno effettuato l'iscrizione ad Azure tramite il sito Web di Azure. [Verificare l'accesso a un contratto cliente Microsoft](#check-access-to-a-microsoft-customer-agreement). Se l'iscrizione ad Azure è stata effettuata tramite un rappresentante Microsoft, il metodo di pagamento predefinito sarà già impostato su *assegno o bonifico bancario*.
...
1. Accedere al portale di Azure e inviare una richiesta di supporto. Cercare e selezionare **Guida e supporto**.  
...
1. Selezionare **Fatturazione** per **Tipo di problema**. *Tipo di problema* è la categoria della richiesta di supporto. Selezionare la sottoscrizione che si vuole pagare tramite fattura, selezionare un piano di supporto e quindi scegliere **Avanti**.
1. Selezionare **Pagamento** per **Tipo di problema**. *Tipo di problema* è la categoria della richiesta di supporto.
1. Selezionare **Passare al modello di pagamento con fattura** per **Sottotipo del problema**.
1. Immettere le informazioni seguenti nella casella **Dettagli** e quindi selezionare **Avanti**.
    - Cliente nuovo o esistente:
    - Se esistente, metodo di pagamento corrente:
    - ID dell'ordine (opzione di richiesta fattura):
    - ID Live (o ID organizzazione) degli amministratori account (deve essere il dominio aziendale):
    - ID account commerciale:
    - Nome della società (registrato assieme alla partita IVA o sui siti Web di enti pubblici):
    - Indirizzo della società (registrato assieme alla partita IVA o sui siti Web di enti pubblici):
    - Sito Web società:
    - Paese:
    - ID imposta/partita IVA:
    - Anno di fondazione della società:
    - Attività svolte con Microsoft in precedenza:
    - Nome del contatto:
    - Numero di telefono del contatto:
    - Indirizzo di posta elettronica del contatto:
    - Giustificazione del motivo per cui si vuole usare l'opzione di fatturazione anziché una carta di credito:
    - Per l'aumento di core, fornire le informazioni aggiuntive seguenti:
        - (Quota precedente) Core esistenti:
        - (Nuova quota) Core richieste:
        - Area e serie specifiche della sottoscrizione:
...
1. Verificare le informazioni di contatto e il metodo di contatto preferito e quindi selezionare **Crea**.
...
* Se necessario, aggiornare le informazioni di contatto per la fatturazione nel [portale di Azure](https://portal.azure.com).