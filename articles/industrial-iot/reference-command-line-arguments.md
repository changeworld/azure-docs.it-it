---
title: Argomenti della riga di comando di Microsoft OPC Publisher
description: Questo articolo fornisce una panoramica degli argomenti della riga di comando del server di pubblicazione OPC
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787561"
---
# <a name="command-line-arguments"></a>Argomenti della riga di comando

Nell'esempio seguente sono descritti diversi argomenti della riga di comando che possono essere usati per impostare le impostazioni globali per il server di pubblicazione OPC.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Argomenti della riga di comando del server di pubblicazione OPC per la versione 2,5 e precedenti

* Utilizzo: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: il nome dell'applicazione OPC UA da usare, obbligatorio il nome dell'applicazione viene usato anche per registrare il server di pubblicazione con questo nome nel registro del dispositivo dell'hub Internet.
    
* iothubconnectionstring: il proprietario dell'hub Internet delle cose è ConnectionString, facoltativo. In genere si specifica il proprietario di IoTHub ConnectionString solo al primo avvio dell'applicazione. La stringa di connessione viene crittografata e archiviata nell'archivio certificati piattaforme.
Nelle chiamate successive viene letto da qui e riutilizzato. Se si specifica ConnectionString a ogni avvio, il dispositivo creato per l'applicazione nel registro dei dispositivi dell'hub Internet viene rimosso e ricreato ogni volta.
    
Esistono un paio di variabili di ambiente che possono essere usate per controllare l'applicazione:
```
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Argomenti della riga di comando del server di pubblicazione OPC per la versione 2,6 e successive
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Altre risorse sono disponibili nei repository GitHub:
