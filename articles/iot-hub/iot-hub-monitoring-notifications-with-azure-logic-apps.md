...
1. Selezionare **Crea**. Al termine della creazione dell'endpoint, procedere con il passaggio successivo.
...
1. Creare una connessione del servizio SMTP.
   ...
   1. Immettere le informazioni SMTP per la cassetta postale, quindi selezionare **Crea**.
      ...
      > [!NOTE]
      > Potrebbe essere necessario disabilitare TLS/SSL per stabilire la connessione. In tal caso e si desidera abilitare nuovamente TLS dopo aver stabilito la connessione, vedere il passaggio facoltativo alla fine di questa sezione.
...
1. Opzionale Se è stato necessario disabilitare TLS per stabilire una connessione con il provider di posta elettronica e si desidera riabilitarlo, attenersi alla procedura seguente:
