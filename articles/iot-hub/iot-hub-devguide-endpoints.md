...
* **Endpoint del dispositivo**. Per ogni dispositivo nel registro delle identità, l'hub IoT espone un set di endpoint. Tranne dove specificato, questi endpoint vengono esposti usando i protocolli [MQTT v3.1.1,](https://mqtt.org/)HTTPS 1.1 e [AMQP 1.0.](https://www.amqp.org/) AMQP e MQTT sono disponibili anche su [WebSockets](https://tools.ietf.org/html/rfc6455) sulla porta 443.
  ...
  * *Ricevere messaggi da cloud a dispositivo*. Un dispositivo usa questo endpoint per ricevere messaggi [da cloud a dispositivo di destinazione.](iot-hub-devguide-messages-c2d.md)

  * *Avvio di caricamenti di file*. Un dispositivo usa questo endpoint per ricevere un URI Archiviazione di Azure firma di accesso condiviso dall'hub IoT per [caricare un file](iot-hub-devguide-file-upload.md).
...
* **Endpoint di servizio**. Ogni hub IoT espone un set di endpoint per il back-end della soluzione per comunicare con i dispositivi. Con un'unica eccezione, questi endpoint vengono esposti solo usando i protocolli [AMQP](https://www.amqp.org/) e AMQP su WebSockets. L'endpoint di chiamata diretta al metodo viene esposto tramite il protocollo HTTPS.
...
È possibile collegare i servizi di Azure esistenti nelle sottoscrizioni di Azure all'hub IoT per fungere da endpoint per il routing dei messaggi. Questi endpoint agiscono come endpoint di servizio e vengono usati come sink per il ruoting dei messaggi. I dispositivi non possono scrivere direttamente sugli endpoint aggiuntivi. Informazioni su [routing dei messaggi](../iot-hub/iot-hub-devguide-messages-d2c.md).
...
## <a name="endpoint-health"></a>Integrità degli endpoint
...
* [Comprendere l'indirizzo IP dell'hub IoT](iot-hub-understand-ip-address.md)
