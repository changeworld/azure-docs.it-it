...
Azure IoT device SDK per C è scritto in ANSI C (C99) per ottimizzare la portabilità. Questa funzionalità rende le librerie più adatte per operare su più piattaforme e dispositivi, soprattutto quando la riduzione del footprint di memoria e del disco è una priorità.

Esiste una vasta gamma di piattaforme in cui è stato testato l'SDK. Per altri dettagli, vedere l'[elenco dei dispositivi Azure Certified per IoT](https://devicecatalog.azure.com/). Anche se questo articolo include procedure dettagliate di codice di esempio eseguito su piattaforma Windows, il codice descritto è identico per tutta la gamma di piattaforme supportate.
...
Sono disponibili diversi strumenti open source che consentono di gestire l'hub Internet delle cose.

* Un'applicazione Windows denominata [Azure Internet Explorer](https://github.com/Azure/azure-iot-explorer).
...
Questa esercitazione usa lo strumento grafico *Device Explorer*. È possibile usare *Azure IoT Tools per Visual Studio Code* se si sviluppa in Visual Studio Code. Se si preferisce usare uno strumento dell'interfaccia della riga di comando, è anche possibile usare l'estensione Internet per l'interfaccia della riga *di comando di Azure 2,0* .
...
1. Dopo la configurazione della stringa di connessione all'hub IoT, fare clic sulla scheda **Management** (Gestione):
   ...
   ![Screenshot di Device Explorer Twin / Management](./media/iot-hub-device-sdk-c-intro/DeviceExplorerTwinManagementTab.png)
...
Dopo aver completato i passaggi precedenti, si è pronti per iniziare a eseguire il codice. La maggior parte degli esempi ha una costante all'inizio del file di origine principale che consente di immettere una stringa di connessione. Ad esempio, la riga corrispondente dal **iothub_client \_ esempi \_ iothub_convenience_sample** applicazione viene visualizzata nel modo seguente.
...
La versione di Windows dell' **iothub_client \_ esempi \_ iothub_convenience_sample** applicazione include la soluzione di Visual Studio seguente:
...
È possibile trovare l'implementazione per l'applicazione di esempio nell' **iothub_client \_ esempi \_ iothub_convenience_sample** file di origine.
...
Si noti la chiamata alla funzione **IoTHubMessage \_ Destroy** al termine del messaggio. Questa funzione libera le risorse allocate quando si crea il messaggio.
...
* ID del messaggio e ID correlazione dal messaggio.
...
È possibile trovare l'implementazione dell'applicazione di esempio nel file **iothub_client \_ samples \_ iothub_convenience_sample** .
...
Un modello nella libreria **serializer** definisce i messaggi che il dispositivo può inviare all'hub IoT e i messaggi, chiamati *azioni* nel linguaggio di modellazione, che può ricevere. Per definire un modello, usare un set di macro C come nell' **esempio \_ iothub_client \_ iothub_convenience_sample** applicazione di esempio:
...
Questo articolo ha trattato le nozioni di base sull'uso delle librerie nell' **SDK per dispositivi Azure per dispositivi per C**. Sono state fornite informazioni sufficienti per comprendere il contenuto dell'SDK, la relativa architettura e come iniziare a usare gli esempi di Windows. L'articolo successivo continua la descrizione dell'SDK, fornendo [altre informazioni sulla libreria IoTHubClient](iot-hub-device-sdk-c-iothubclient.md).
