...
Se è necessario eseguire calcoli più complessi, è possibile concatenare più istanze di **Apply Math Operation**. È possibile ad esempio aggiungere due colonne tramite un'istanza di **Apply Math Operation**, quindi usare un'altra istanza di **Apply Math Operation** per dividere la somma per una costante in modo da ottenere la media.  
