...
description: Informazioni su come Azure Machine Learning le pipeline inseriscono i dati e su come gestire e spostare i dati tra i passaggi della pipeline.
...
- Creare nuovi `Dataset` oggetti da `OutputFileDatasetConfig` wisƒh per renderli permanente
...
- Alcuni dati preesistenti. Questo articolo illustra brevemente l'uso di un [contenitore BLOB di Azure](../storage/blobs/storage-blobs-overview.md).

- Facoltativo: una pipeline di Machine Learning esistente, ad esempio quella descritta in [creare ed eseguire pipeline di Machine Learning con Azure Machine Learning SDK](./how-to-create-machine-learning-pipelines.md).

## <a name="use-dataset-objects-for-pre-existing-data"></a>Usare `Dataset` gli oggetti per i dati preesistenti 

Il modo migliore per inserire i dati in una pipeline consiste nell'usare un oggetto [DataSet](/python/api/azureml-core/azureml.core.dataset%28class%29?preserve-view=true&view=azure-ml-py) . `Dataset` gli oggetti rappresentano dati permanenti disponibili in un'area di lavoro.
...
> [!NOTE]
> È necessario sostituire i valori per tutti questi argomenti, ovvero,,, `"train_data"` e, `"train.py"` `cluster` `iris_dataset` con i propri dati. Il frammento di codice precedente mostra semplicemente il form della chiamata e non fa parte di un esempio Microsoft. 
...
> [!NOTE]
> I frammenti di codice precedenti mostrano il formato delle chiamate e non fanno parte di un esempio Microsoft. È necessario sostituire i vari argomenti con i valori del proprio progetto.

## <a name="use-outputfiledatasetconfig-for-intermediate-data"></a>`OutputFileDatasetConfig`Da usare per i dati intermedi

Sebbene `Dataset` gli oggetti rappresentino solo dati persistenti, [`OutputFileDatasetConfig`](/python/api/azureml-core/azureml.data.outputfiledatasetconfig?preserve-view=true&view=azure-ml-py) è possibile utilizzare gli oggetti per l'output di dati temporanei da passaggi della pipeline **e** dati di output permanenti. `OutputFileDatasetConfig` supporta la scrittura di dati nell'archiviazione BLOB, FileShare, adlsgen1 o adlsgen2. Supporta sia la modalità di montaggio che la modalità di caricamento. In modalità di montaggio i file scritti nella directory montata vengono archiviati in modo permanente quando il file viene chiuso. In modalità di caricamento, i file scritti nella directory di output vengono caricati alla fine del processo. Se il processo ha esito negativo o viene annullato, la directory di output non verrà caricata.
...
> [!NOTE]
> Le scritture simultanee in un `OutputFileDatasetConfig` non riusciranno. Non tentare di usare un singolo oggetto `OutputFileDatasetConfig` simultaneamente. Non condividere un singolo `OutputFileDatasetConfig` in una situazione di multielaborazione, ad esempio quando si usa il training distribuito. 
...
## <a name="delete-outputfiledatasetconfig-contents-when-no-longer-needed"></a>Elimina `OutputFileDatasetConfig` contenuto quando non è più necessario
...
* Usare l'archiviazione BLOB con criteri di archiviazione a breve termine per i dati intermedi. vedere [ottimizzare i costi automatizzando i livelli di accesso all'archivio BLOB di Azure](../storage/blobs/storage/blobs/storage-lifecycle-management-concepts.md). 
