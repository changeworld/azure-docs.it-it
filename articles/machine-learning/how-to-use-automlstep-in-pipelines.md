...
Il modo migliore per spostare inizialmente i dati _in_ una pipeline di ml è con `Dataset` gli oggetti. Per spostare i dati _tra_ le fasi e possibile salvare l'output dei dati dalle esecuzioni, la modalità preferita è con [`OutputFileDatasetConfig`](/python/api/azureml-core/azureml.data.outputfiledatasetconfig?preserve-view=true&view=azure-ml-py) [`OutputTabularDatasetConfig`](/python/api/azureml-core/azureml.data.output_dataset_config.outputtabulardatasetconfig?preserve-view=true&view=azure-ml-py) gli oggetti e. Per poter essere utilizzato con `AutoMLStep` , l' `PipelineData` oggetto deve essere trasformato in un `PipelineOutputTabularDataset` oggetto. Per altre informazioni, vedere [dati di input e di output dalle pipeline di ml](how-to-move-data-in-out-of-pipelines.md).

`AutoMLStep`Viene configurato tramite un `AutoMLConfig` oggetto. `AutoMLConfig` è una classe flessibile, come descritto in [configurare esperimenti di Machine Learning automatici in Python](./how-to-configure-auto-train.md#configure-your-experiment-settings). 
...
Il AutoMLStep configura automaticamente le dipendenze durante l'invio del processo. Il contesto di runtime viene impostato tramite la creazione e la configurazione di un `RunConfiguration` oggetto. Qui viene impostata la destinazione di calcolo.
...
- Scrivere i dati trasformati nei `OutputFileDatasetConfig` percorsi di output
...
Quando il codice definisce le funzioni di preparazione dei dati, il codice analizza l'argomento di input, ovvero il percorso in cui si desidera scrivere i dati.  Questi valori verranno determinati da `OutputFileDatasetConfig` oggetti che verranno discussi nel passaggio successivo. Il codice recupera l'oggetto registrato `'titanic_cs'` `Dataset` , lo converte in un Pandas `DataFrame` e chiama le varie funzioni di preparazione dei dati. 

Poiché `output_path` è una directory, la chiamata a `to_csv()` specifica il nome file `prepped_data.csv` .
...
Il codice di preparazione dei dati descritto in precedenza deve essere associato a un `PythonScripStep` oggetto da usare con una pipeline. Il percorso in cui viene scritto l'output CSV viene generato da un `OutputFileDatasetConfig` oggetto. `ComputeTarget` `RunConfig` `'titanic_ds' Dataset` Per completare la specifica vengono usate le risorse preparate in precedenza, ad esempio, e.
...
L' `prepped_data_path` oggetto è di tipo `OutputFileDatasetConfig` che punta a una directory.  Si noti che è specificato nel `arguments` parametro. Se si esamina il passaggio precedente, si noterà che all'interno del codice di preparazione dei dati, il valore dell'argomento `'--output_path'` è il percorso di directory in cui è stato scritto il file CSV. 

## <a name="train-with-automlstep"></a>Eseguire il training con AutoMLStep

La configurazione di un passaggio automatico della pipeline di Machine Learning viene eseguita con la `AutoMLConfig` classe. Questa classe flessibile è descritta in [configurare esperimenti di Machine Learning automatici in Python](./how-to-configure-auto-train.md). L'input e l'output dei dati sono gli unici aspetti della configurazione che richiedono particolare attenzione in una pipeline ML. L'input e l'output per `AutoMLConfig` nelle pipeline sono descritti in dettaglio di seguito. Oltre ai dati, un vantaggio delle pipeline di ML è la possibilità di usare destinazioni di calcolo diverse per diversi passaggi. È possibile scegliere di usare un più potente `ComputeTarget` solo per il processo di Machine Learning automatizzato. Questa operazione è semplice come assegnare un più potente `RunConfiguration` al `AutoMLConfig` parametro dell'oggetto `run_configuration` .

### <a name="send-data-to-automlstep"></a>Invia dati a `AutoMLStep`

In una pipeline ML i dati di input devono essere un `Dataset` oggetto. Il modo più elevato consiste nel fornire i dati di input sotto forma di `OutputTabularDatasetConfig` oggetti. Si crea un oggetto di quel tipo con  `read_delimited_files()` su `OutputFileDatasetConfig` , ad esempio `prepped_data_path` , ad esempio l' `prepped_data_path` oggetto.
...
|| [Notebook che mostra la `OutputTabularDatasetConfig` tecnica](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/machine-learning-pipelines/nyc-taxi-data-regression-model-building/nyc-taxi-data-regression-model-building.ipynb) |
...
Gli output di `AutoMLStep` rappresentano i punteggi della metrica finale del modello con prestazioni più elevate e il modello stesso. Per usare questi output in altri passaggi della pipeline, preparare `OutputFileDatasetConfig` gli oggetti per riceverli.
...
Una volta definiti gli input e gli output, è possibile creare `AutoMLConfig` e `AutoMLStep` . I dettagli della configurazione dipenderanno dall'attività, come descritto in configurare gli esperimenti di Machine Learning [automatici in Python](./how-to-configure-auto-train.md). Per l'attività di classificazione di sopravvivenza Titanic, il frammento di codice seguente illustra una semplice configurazione.
...
- `featurization` è impostato su `auto`. Per altri dettagli, vedere la sezione [Data conteggi](./how-to-configure-auto-train.md#data-featurization) del documento di configurazione di Machine Learning automatico 
- `label_column_name` indica la colonna da stimare 
- `training_data` viene impostato `OutputTabularDatasetConfig` sugli oggetti creati dagli output del passaggio di preparazione dei dati 
...
- Informazioni sull'integrazione della pipeline in per [MLOps end-to-end](./concept-model-management-and-deployment.md#automate-the-ml-lifecycle) o sull'analisi del [repository GitHub MLOps](https://github.com/Microsoft/MLOpspython)
