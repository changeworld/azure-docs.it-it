...
- Estensione dell'interfaccia della riga [di comando di Azure per il servizio Machine Learning](reference-azure-machine-learning-cli.md), l' [SDK Azure Machine Learning per Python](/python/api/overview/azure/ml/intro)o l' [estensione di Visual Studio code Azure Machine Learning](tutorial-setup-vscode-extension.md).
...
1. Per determinare se il cluster AKS è abilitato per RBAC Kubernetes, usare il comando seguente:

    ```azurecli-interactive
    az aks show --name <AKS cluster name> --resource-group <resource group name> --subscription <subscription id> --query enableRbac
    ```

    Questo comando restituisce un valore `true` se KUBERNETES RBAC è abilitato. Questo valore determina il comando da usare nel passaggio successivo.
