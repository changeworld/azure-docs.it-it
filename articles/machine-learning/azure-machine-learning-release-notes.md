...
### <a name="azure-machine-learning-sdk-for-python-v1260"></a>SDK di Azure Machine Learning per Python v 1.26.0
+ **Correzioni di bug e miglioramenti**
  + **azureml-automl-core**
    + È stato risolto un problema per cui i modelli Naive venivano consigliati nelle esecuzioni di AutoMLStep e hanno esito negativo con funzionalità di intervallo o di ritardo. Questi modelli non saranno consigliati quando vengono impostati i ritardi di destinazione o le dimensioni della finestra mobile di destinazione.
    +  L'output della console è stato modificato quando si invia un'esecuzione AutoML per visualizzare un collegamento del portale all'esecuzione.
  + **azureml-core**
    + Aggiunta della modalità HDFS nella documentazione.
    + Aggiunta del supporto per comprendere le partizioni del set di dati del file in base alla struttura glob.
    + Aggiunta del supporto per l'aggiornamento del registro contenitori associato all'area di lavoro AzureML.
    + Gli attributi di ambiente deprecati in DockerSection-"Enabled", "shared_volume" e "Arguments" fanno ora parte di DockerConfiguration in RunConfiguration.
    + Documentazione del clone dell'interfaccia della riga di comando aggiornata
    + URI del portale aggiornati per includere il tenant per l'autenticazione
    + Il nome dell'esperimento è stato rimosso dagli URI di esecuzione per evitare i reindirizzamenti 
    + Aggiornamento dell'esperimento URO per l'uso dell'ID esperimento.
    + Correzioni di bug per il fissaggio del calcolo remoto con l'interfaccia della riga di comando AzureML.
    + URI del portale aggiornati per includere il tenant per l'autenticazione.
    + URI dell'esperimento aggiornato per usare l'ID esperimento.
  + **azureml-interpret**
    + azureml-interpreta aggiornata per l'uso dell'0.17.0 di interpretazione della community
  + **azureml-opendatasets**
    + Data di inizio e di fine dell'input, convalida del tipo e indicazione di errore se non è di tipo DateTime.
  + **azureml-esecuzione parallela**
    + [Funzionalità sperimentale] Aggiungere `partition_keys` il parametro a ParallelRunConfig, se specificato, i set di dati di input verranno partizionati in mini-batch in base alle chiavi specificate. Richiede il partizionamento di tutti i set di dati di input.
  + **azureml-pipeline-steps**
    + Bugfix: supporto path_on_compute durante il passaggio della configurazione del set di dati come download.
    + Deprecare RScriptStep a favore dell'uso di CommandStep per l'esecuzione di script R nelle pipeline. 
    + Deprecare EstimatorStep a favore dell'uso di CommandStep per l'esecuzione del training di ML (incluso il training distribuito) nelle pipeline.
  + **azureml-sdk**
    + Aggiornare python_requires < 3,9 per azureml-SDK
  + **azureml-train-automl-client**
    +  L'output della console è stato modificato quando si invia un'esecuzione AutoML per visualizzare un collegamento del portale all'esecuzione.
  + **azureml-train-core**
    + Gli attributi ' Enabled ',' shared_volume ' è arguments ' di DockerSection sono deprecati a favore dell'uso di DockerConfiguration con ScriptRunConfig.
    +  Usare i set di dati aperti di Azure per il set di dati MNIST
    + I messaggi di errore iperguidati sono stati aggiornati.


## <a name="2021-03-22"></a>2021-03-22

### <a name="azure-machine-learning-sdk-for-python-v1250"></a>SDK di Azure Machine Learning per Python v 1.25.0
+ **Correzioni di bug e miglioramenti**
  + **azureml-automl-core**
    +  L'output della console è stato modificato quando si invia un'esecuzione AutoML per visualizzare un collegamento del portale all'esecuzione.
  + **azureml-core**
    + Inizia a supportare l'aggiornamento del registro contenitori per l'area di lavoro in SDK e CLI
    + Gli attributi ' Enabled ',' shared_volume ' è arguments ' di DockerSection sono deprecati a favore dell'uso di DockerConfiguration con ScriptRunConfig.
    + Documentazione del clone dell'interfaccia della riga di comando aggiornata
    + URI del portale aggiornati per includere il tenant per l'autenticazione
    + Il nome dell'esperimento è stato rimosso dagli URI di esecuzione per evitare i reindirizzamenti
    + Aggiornamento dell'esperimento URO per l'uso dell'ID esperimento.
    + Correzioni di bug per il fissaggio di risorse di calcolo remote con AZ CLI
    + URI del portale aggiornati per includere il tenant per l'autenticazione.
    + Aggiunta del supporto per comprendere le partizioni del set di dati del file in base alla struttura glob.
  + **azureml-interpret**
    + azureml-interpreta aggiornata per l'uso dell'0.17.0 di interpretazione della community
...
### <a name="azure-machine-learning-studio-notebooks-experience-march-update"></a>Esperienza di Azure Machine Learning Studio notebook (aggiornamento di marzo)
+ **Nuove funzionalità**
  + Rendering CSV/TSV. Gli utenti saranno in grado di eseguire il rendering del file TSV/CSV in un formato griglia per semplificare l'analisi dei dati. 
  + Autenticazione SSO per l'istanza di calcolo. Gli utenti possono ora autenticare facilmente le nuove istanze di calcolo direttamente nell'interfaccia utente del notebook, semplificando l'autenticazione e l'uso di Azure SDK direttamente in AzureML. 
  + Metriche dell'istanza di calcolo. Gli utenti potranno visualizzare le metriche di calcolo, ad esempio l'utilizzo della CPU e la memoria tramite terminale.
  + Dettagli del file. Gli utenti possono ora visualizzare i dettagli dei file, tra cui l'ora dell'Ultima modifica e le dimensioni del file, facendo clic sui 3 puntini accanto a un file.

+ **Correzioni di bug e miglioramenti**
  + Tempi di caricamento delle pagine migliorati.
  + Prestazioni migliorate.
  + Maggiore affidabilità del kernel e della velocità.
  + Ottenere una proprietà reale verticale spostando in modo permanente il riquadro dei file del notebook
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-train-core**
    ...
    +  Tutte le classi Estimator sono deprecate a favore dell'utilizzo di ScriptRunConfig per configurare le esecuzioni degli esperimenti. Le classi deprecate includono: + MMLBaseEstimator + Estimator + PyTorch + TensorFlow + Chainer + SKLearn
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-pipeline-core**
    + Aggiungere il supporto per creare, elencare e ottenere una pianificazione della pipeline basata su un endpoint della pipeline.
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-train-core**
    ...
    + Ripristino della versione predefinita di PyTorch su 1,4.
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-train-core**
    ...
    + Risolvere il problema per cui non è possibile recuperare l'immagine del Framework nell'area di Azure per enti pubblici quando si usano gli estimatori del Framework di training
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-core**
    + Il bug fisso in era `datastore.upload_files` il percorso relativo che non iniziava con `./` non è stato possibile utilizzarlo.
    ...
    + Aggiunta di un'opzione per [Environment.build_local ()](/python/api/azureml-core/azureml.core.environment.environment) per eseguire il push di un'immagine nel registro contenitori dell'area di lavoro AzureML
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-core**
    ...
    + Aggiunta di informazioni CreatedBy a oggetti modello e servizio. È possibile accedere through.created_by
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-core**
    ...
    + È stato aggiunto un nuovo amlcompute.get_active_runs API che restituisce un generatore per l'esecuzione e l'esecuzione in coda in un determinato amlcompute.
    + Aggiunto Load Balancer tipo a MLC per i tipi AKS.
...
Azure Machine Learning è ora un provider di risorse per griglia di eventi, è possibile configurare gli eventi di Machine Learning tramite l'portale di Azure o l'interfaccia della riga di comando di Azure. Gli utenti possono creare eventi per il completamento dell'esecuzione, la registrazione del modello, la distribuzione del modello e la deriva dei dati. Questi eventi possono essere indirizzati ai gestori eventi supportati da griglia di eventi per l'utilizzo. Per informazioni dettagliate, vedere gli articoli relativi [allo schema](../event-grid/event-schema-machine-learning.md) di eventi e alle [esercitazioni](how-to-use-event-grid.md) di Machine Learning
...
+ **Nuove funzionalità**
  ...
  + **Funzionalità in anteprima**
    + Si sta rilasciando il supporto in anteprima per la crittografia del disco dell'unità SSD locale in Azure Machine Learning calcolo. Genera un ticket di supporto tecnico per consentire alla tua sottoscrizione di usare questa funzionalità.
...
+ **Correzioni di bug e miglioramenti**
  ...
  + [**azureml-core**](/python/api/azureml-core)
    ...
    + I set di dati recuperati durante un'esecuzione verranno rilevati e possono essere visualizzati nella pagina Dettagli esecuzione o chiamando al [`run.get_details()`](/python/api/azureml-core/azureml.core.run%28class%29#get-details--) termine dell'esecuzione.
  ...
  + [**azureml-datadrift**](/python/api/azureml-datadrift)
    ...
    + Nuovi metodi `create_from_model()` e `create_from_dataset()` alla [`DataDriftDetector`](/python/api/azureml-datadrift/azureml.datadrift.datadriftdetector%28class%29) classe. Il `create()` metodo verrà deprecato.
  ...
  + **azureml-train-core**
    + Aggiunto il supporto di PyTorch 1,3 in [`PyTorch`](/python/api/azureml-train-core/azureml.train.dnn.pytorch) Estimator
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **[azureml-core](/python/api/azureml-core)**
    + Aggiunta dell'API per annullare la registrazione di DataSet. DataSet. [unregister_all_versions ()](/python/api/azureml-core/azureml.data.abstract_datastore.abstractdatastore#unregister--).
    ...
    + La possibilità di utilizzare [filedataset](/python/api/azureml-core/azureml.data.filedataset) e [TabularDataset](/python/api/azureml-core/azureml.data.tabulardataset) come input per [PythonScriptStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.python_script_step.pythonscriptstep), [EstimatorStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.estimatorstep)e [HyperDriveStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.hyperdrivestep) nella pipeline Azure Machine Learning.
    + Prestazioni del set di dati. [Mount ()](/python/api/azureml-core/azureml.data.filedataset#mount-mount-point-none----kwargs-) è stato migliorato per le cartelle con un numero elevato di file
    + Aggiunta dell'URL alle raccomandazioni degli errori noti nei dettagli dell'esecuzione.
    + Correzione di un bug in run.get_metrics in cui le richieste avrebbero esito negativo se un'esecuzione aveva troppi elementi figlio
    + Correzione di un bug in [Run.get_metrics](/python/api/azureml-core/azureml.core.run.run#get-metrics-name-none--recursive-false--run-type-none--populate-false-) in cui le richieste avrebbero esito negativo se un'esecuzione aveva troppi elementi figlio
  ...
  + **azureml-Train-automl**
    ...
    + Supporto per l'uso dell'ambiente per la distribuzione dei modelli e per l'aggiornamento del servizio.
  + **[azureml-datadrift](/python/api/azureml-datadrift)**
    + L'attributo show della classe [DataDriftDetector](/python/api/azureml-datadrift/azureml.datadrift.datadriftdetector.datadriftdetector) non supporterà più l'argomento facoltativo ' with_details '. L'attributo show presenterà solo il coefficiente di drifting dei dati e il contributo alla deviazione dei dati delle colonne di funzionalità.
    + Funzione DataDriftDetector [get_output] Python/API/azureml-datadrift/azureml. datadrift. DataDriftDetector. DataDriftDetector # Get-output-Start-Time-None--End-Time-None--Run-ID-None-) cambia il comportamento:
      + Parametro di input start_time, end_time sono facoltativi anziché obbligatoria;
    ...
    + Supporto del recupero degli output di spostamento dei dati basati su DataSet.
  + **azureml-explain-model**
    + Aggiungere il supporto per [ScoringExplainer](/python/api/azureml-interpret/azureml.interpret.scoring.scoring_explainer.scoringexplainer) da creare direttamente usando MimicWrapper
  + **[azureml-pipeline-core](/python/api/azureml-pipeline-core)**
    + Miglioramento delle prestazioni per la creazione di pipeline di grandi dimensioni.
  + **[azureml-train-core](/python/api/azureml-train-core)**
    + Aggiunto il supporto di TensorFlow 2,0 in [TensorFlow](/python/api/azureml-train-core/azureml.train.dnn.tensorflow) Estimator.
  + **[azureml-Train-automl](/python/api/azureml-train-automl-runtime/)**
...
### <a name="azure-machine-learning-sdk-for-python-v1065"></a>SDK di Azure Machine Learning per Python v 1.0.65
  ...
  + **Nuove funzionalità**
    ...
    + Sono stati aggiunti ambienti curati. Questi ambienti sono stati preconfigurati con le librerie per le attività di Machine Learning comuni e sono stati pre-compilazione e memorizzati nella cache come immagini Docker per un'esecuzione più rapida. Vengono visualizzati per impostazione predefinita nell'elenco dell'ambiente dell' [area di lavoro](/python/api/azureml-core/azureml.core.workspace%28class%29), con prefisso "AzureML".
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-core**
    + Aggiunta della possibilità di recuperare l'URL di firma di accesso condiviso nel modello nell'archiviazione tramite l'oggetto modello. Ad esempio: model.get_sas_url ()
    + Introduci `run.get_details()['datasets']` per ottenere i set di impostazioni associati all'esecuzione inviata
    + Aggiungere `Dataset.Tabular.from_json_lines_files` un'API per creare un TabularDataset da file di righe JSON. Per informazioni su questi dati tabulari nei file di righe JSON in TabularDataset, vedere [questo articolo](how-to-create-register-datasets.md) per la documentazione.
  ...
  + **[azureml-core](/python/api/azureml-core/azureml.core)**
    + Aggiunta della possibilità di recuperare l'URL di firma di accesso condiviso nel modello nell'archiviazione tramite l'oggetto modello. Esempio: modello. [get_sas_url ()](/python/api/azureml-core/azureml.core.model.model#get-sas-urls--)
    + Introdurre l'esecuzione. [get_Details](/python/api/azureml-core/azureml.core.run%28class%29#get-details--)[' DataSets '] per ottenere i set di impostazioni associati all'esecuzione inviata
    + Aggiungere un'API `Dataset.Tabular` .[ from_json_lines_files ()](/python/api/azureml-core/azureml.data.dataset_factory.tabulardatasetfactory#from-json-lines-files-path--validate-true--include-path-false--set-column-types-none--partition-format-none-) per creare un TabularDataset dai file di righe JSON. Per informazioni su questi dati tabulari nei file di righe JSON in TabularDataset, vedere https://aka.ms/azureml-data la documentazione relativa a.
    + Aggiunta di campi di dimensioni di VM aggiuntivi (disco del sistema operativo, numero di GPU) alla funzione [supported_vmsizes ()](/python/api/azureml-core/azureml.core.compute.amlcompute.amlcompute#supported-vmsizes-workspace--location-none-)
    + Aggiunta di campi aggiuntivi alla funzione [list_nodes ()](/python/api/azureml-core/azureml.core.compute.amlcompute.amlcompute#list-nodes--) per visualizzare l'esecuzione, il privato e l'IP pubblico, la porta e così via.
  ...
  + **azureml-Train-automl**
    ...
    + Aggiunta del messaggio di deprecazione per explain_model () e retrieve_model_explanations ()
  + **[azureml-pipeline-core](/python/api/azureml-pipeline-core)**
    + È stato aggiunto un [notebook](https://aka.ms/pl-modulestep) per descrivere il [modulo](/python/api/azureml-pipeline-core/azureml.pipeline.core.module%28class%29), [ModuleVersion e [ModuleStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.modulestep).
  + **[azureml-pipeline-steps](/python/api/azureml-pipeline-steps)**
    + Aggiunta di [RScriptStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.rscriptstep) per supportare l'esecuzione di script R tramite la pipeline AML.
    + Correzione dell'analisi dei parametri di metadati in [AzureBatchStep che causava il messaggio di errore "l'assegnazione per il parametro SubscriptionId non è stata specificata".
  + **[azureml-Train-automl](/python/api/azureml-train-automl-runtime/)**
    + Training_data supportati, validation_data, label_column_name weight_column_name come formato di input dei dati.
...
+ **Nuove funzionalità**
  + Ha introdotto il `timeseries`  tratto in TabularDataset. Questo tratto consente di filtrare facilmente i timestamp sui dati TabularDataset, ad esempio l'acquisizione di tutti i dati tra un intervallo di tempo o i dati più recenti.  https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/work-with-data/datasets-tutorial/timeseries-datasets/tabular-timeseries-dataset-filtering.ipynb per un notebook di esempio.
  + Training abilitato con TabularDataset e filedataset. 
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-core**
    ...
    + Correzione del bug `DataReference` in cui il nome cambia quando viene `DataReference` modificata la modalità, ad esempio quando si chiama `as_upload` , `as_download` o `as_mount` .
...
+ **Nuove funzionalità**
  + È stato introdotto filedataset, che fa riferimento a uno o più file negli archivi dati o negli URL pubblici. I file possono essere di qualsiasi formato. Filedataset offre la possibilità di scaricare o montare i file nel calcolo. 
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-automl-core**
    + Correzione di un problema in AutoML se le righe con etichette mancanti non sono state rimosse correttamente.
  ...
  + **azureml-core**
    ...
    + Aggiunta Environment.add_private_pip_wheel (), che consente di caricare pacchetti Python personalizzati privati nell' `whl` area di lavoro e di usarli in modo sicuro per compilare/materializzare l'ambiente.
    + È ora possibile aggiornare il certificato TLS/SSL per l'endpoint di assegnazione dei punteggi distribuito nel cluster AKS per Microsoft e per il certificato del cliente.
  + **azureml-explain-model**
  ...
  + **azureml-Train-automl**
    ...
    + Consente agli utenti di AutoML di eliminare i granularità dal set di test che non esiste nel set di training durante la previsione.
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-contrib-explain-Model**
    ...
    + Modifiche dei widget: viene installato automaticamente con `contrib` , non più `nbextension` spiegazione di installazione/abilitazione e supporto con importanza della funzionalità globale (ad esempio, Permutative)
...
+ **Nuove funzionalità**
  ...
  + Azure Machine Learning ora offre la possibilità di riprendere il training da un'esecuzione precedente, un checkpoint o un file di modello.
    + Informazioni su come [usare gli estimatori per riprendere il training da un'esecuzione precedente](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/ml-frameworks/tensorflow/train-tensorflow-resume-training/train-tensorflow-resume-training.ipynb)
...
+ **Correzioni di bug e miglioramenti**
  ...
  + **azureml-core**
    ...
    + Miglioramento delle eccezioni di errore per la creazione dell'area di lavoro In modo che gli utenti non visualizzino "Impossibile creare l'area di lavoro. Impossibile trovare... " come messaggio e visualizzano invece l'effettivo errore di creazione.
  ...
  + **azureml-mlflow**
    ...
    + Bug patch dove mlflow.log_artifacts ("my_dir") Salva gli artefatti in "my_dir/<artefatto-percorsi>" anziché "<artefatti-percorsi>"
  + **azureml-opendatasets**
...
+ Azure ML Modelli con accelerazione hardware su [FPGA](how-to-deploy-fpga-web-service.md) è disponibile a livello generale.
  ...
  + Distribuire il contenitore in un dispositivo di [Azure Data Box Edge](../databox-online/azure-stack-edge-overview.md) server
...
+ **Rilevamento dei riferimenti git**<br/> I clienti hanno richiesto le funzionalità di integrazione git di base per un certo periodo di tempo, in quanto consente di mantenere un audit trail completo. È stato implementato il rilevamento tra le entità principali in Azure ML per i metadati correlati a git (repository, commit, stato pulito). Queste informazioni verranno raccolte automaticamente dall'SDK e dall'interfaccia della riga di comando.
...
[`PipelineEndpoint`](/python/api/azureml-pipeline-core/azureml.pipeline.core.pipeline_endpoint.pipelineendpoint)È stato introdotto per aggiungere una nuova versione di una pipeline pubblicata mantenendo lo stesso endpoint.
...
+ **Nuove funzionalità**
  ...
  Per un elenco delle versioni supportate dalla versione più recente dell'SDK, vedere la [documentazione di DNN Estimator](/python/api/azureml-train-core/azureml.train.dnn).
...
+ **Nuove funzionalità**
  + Il *azureml.Core.Run.create_children* metodo consente la creazione a bassa latenza di più esecuzioni figlio con una singola chiamata.
...
+ **Nuove funzionalità**
  + Azure Machine Learning offre ora il supporto di primo livello per il famoso Chainer DNN Framework. L'uso delle [`Chainer`](/python/api/azureml-train-core/azureml.train.dnn.chainer) classi consente di eseguire facilmente il training e la distribuzione di modelli di Chainer.
