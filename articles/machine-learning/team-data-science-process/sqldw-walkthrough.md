...
> [!NOTE]
> A seconda della posizione geografica dell'account di archiviazione BLOB privato, il processo di copia dei dati da un BLOB pubblico all'account di archiviazione privato può richiedere circa 15 minuti o anche più a lungo e il processo di caricamento dei dati dall'account di archiviazione all'analisi delle sinapsi di Azure Azure potrebbe richiedere più di 20 minuti.
>
>
...
## <a name="data-exploration-and-feature-engineering-in-azure-synapse-analytics"></a><a name="dbexplore"></a>Esplorazione dei dati e progettazione di funzionalità in Azure sinapsi Analytics
...
--Segnala il numero di righe nella tabella <nyctaxi_trip> senza analisi della tabella

   ```sql
   SELECT SUM(rows) FROM sys.partitions WHERE object_id = OBJECT_ID('<schemaname>.<nyctaxi_trip>')
   ```

--Segnala il numero di colonne nella tabella <nyctaxi_trip>
...
--Query di esempio per chiamare la funzione per creare funzionalità
...
2. Salvare in modo permanente i dati campionati e progettati che si prevede di usare per la compilazione di modelli in una nuova tabella di analisi delle sinapsi di Azure e usare la nuova tabella nel modulo [Importa dati][Import-Data] in Azure Machine Learning. Lo script di PowerShell nel passaggio precedente ha eseguito questa attività per l'utente. È possibile leggere direttamente in questa tabella nel modulo Import Data.
...
1. Ottenere i dati in Azure Machine Learning Studio (classico) usando il modulo [Import Data][Import-Data] , disponibile nella sezione **data input and output** . Per altre informazioni, vedere la pagina di riferimento sul modulo [Importa dati][import-data].
...
Un esempio di un esperimento di classificazione binaria per la lettura dei dati direttamente dal database di analisi delle sinapsi di Azure è illustrato nella figura seguente (ricordare di sostituire i nomi di tabella nyctaxi_trip e nyctaxi_fare con il nome dello schema e i nomi di tabella usati nella procedura dettagliata). È possibile creare esperimenti dello stesso tipo per i problemi di classificazione multiclasse e di regressione.
