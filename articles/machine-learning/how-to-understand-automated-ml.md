...
Al termine dell'esperimento di Machine Learning automatizzato, è possibile trovare una cronologia delle esecuzioni tramite:
  ...
  - Notebook di Jupyter con il [widget Jupyter di RunDetails](/python/api/azureml-widgets/azureml.widgets.rundetails)
...
r2_score|R<sup>2</sup> (il coefficiente di determinazione) misura la riduzione proporzionale nell'errore quadratico medio (MSE) rispetto alla varianza totale dei dati osservati. <br> <br> **Obiettivo:** Più vicino a 1 migliore <br> **Intervallo:** [-1, 1]<br><br>Nota: R<sup>2</sup> ha spesso l'intervallo (-inf, 1]. Il MSE può essere più grande della varianza osservata, quindi R<sup>2</sup> può avere valori negativi arbitrariamente grandi, a seconda dei dati e delle stime del modello. Le clip automatiche ML hanno segnalato i punteggi R<sup>2</sup> in-1, quindi un valore pari a-1 per R<sup>2</sup> probabilmente significa che il Punteggio di r<sup>2</sup> è minore di-1. Prendere in considerazione gli altri valori di metrica e le proprietà dei dati quando si interpreta un punteggio R<sup>2</sup> negativo.|[Calcolo](https://scikit-learn.org/0.16/modules/generated/sklearn.metrics.r2_score.html)|
