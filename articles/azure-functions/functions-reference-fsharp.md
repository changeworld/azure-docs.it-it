...
Per registrare l'output nei [log in streaming](../app-service/troubleshoot-diagnostic-logs.md) in F#, la funzione deve accettare un argomento di tipo [ILogger](/dotnet/api/microsoft.extensions.logging.ilogger). Per coerenza è consigliabile denominare questo argomento `log`. Ad esempio:
