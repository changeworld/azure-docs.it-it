...
Questa esercitazione illustra come individuare, valutare ed eseguire la migrazione di macchine virtuali Amazon Web Services (AWS) in macchine virtuali di Azure usando gli strumenti di migrazione Azure Migrate: Valutazione server e Azure Migrate: Migrazione del server.
...
Prima di procedere con la migrazione ad Azure, è consigliabile eseguire un'individuazione delle macchine virtuali e una valutazione della migrazione. Questa valutazione consente di definite le dimensioni corrette per le macchine virtuali AWS per la migrazione ad Azure e stimare i potenziali costi di esecuzione di Azure.
...
## <a name="prepare-aws-instances-for-migration"></a>Preparare le istanze di AWS per la migrazione
...
## <a name="enable-replication-for-aws-vms"></a>Abilitare la replica per le VM AWS
...
10. In  **Account di archiviazione cache** mantenere l'opzione predefinita per usare l'account di archiviazione cache creato automaticamente per il progetto. Usare l'elenco a discesa se si desidera specificare un account di archiviazione diverso da usare come account di archiviazione della cache per la replica. <br/> 
    > [!NOTE]
    >
...
- Se la macchina virtuale è una macchina virtuale PV (paravirtualizzata) e non HVM, potrebbe non essere possibile eseguirla così com'è in Azure perché le macchine virtuali paravirtualizzate usano una sequenza di avvio personalizzata in AWS. È possibile superare questo problema disinstallando i driver PV prima di eseguire una migrazione ad Azure.  
...
**Risposta:** non è possibile eseguire la migrazione di macchine virtuali che eseguono Amazon Linux così come sono, perché il sistema operativo Amazon Linux è supportato solo in AWS.
