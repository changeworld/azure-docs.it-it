...
- È possibile selezionare fino a 10 macchine virtuali contemporaneamente per la replica tramite il portale di Azure. Se si desidera eseguire la migrazione di più computer, eseguire la replica in gruppi di 10. Non esiste alcun limite al numero di macchine virtuali che possono essere replicate tramite i cmdlet di PowerShell. Si consiglia di replicare non più di 500 VM alla volta da un singolo vCenter tramite PowerShell per garantire prestazioni ottimali.
- Per la migrazione senza agenti VMware, è possibile eseguire contemporaneamente fino a 500 repliche da ogni server vCenter.
...
**autorizzazioni server vCenter** | La migrazione senza agenti usa l' [appliance di migrazione](migrate-appliance.md). Il dispositivo richiede queste autorizzazioni in server vCenter:<br/><br/> - Archivio **dati. browse** (datastore-> browse datastore): consente l'esplorazione dei file di log della macchina virtuale per la risoluzione dei problemi di creazione ed eliminazione di snapshot.<br/><br/> - **Datastore. filemanagement** (archivio dati-> operazioni su file di basso livello): consente operazioni di lettura/scrittura/eliminazione/ridenominazione nel browser archivio dati per risolvere i problemi di creazione ed eliminazione di snapshot.<br/><br/> - **VirtualMachine.Config. Rilevamento modifiche** (Virtual Machine-> rilevamento modifiche disco): consente di abilitare o disabilitare il rilevamento delle modifiche dei dischi delle VM per eseguire il pull dei blocchi di dati modificati tra gli snapshot.<br/><br/> - **VirtualMachine.Config. DiskLease** (macchina virtuale-lease del disco >): consente le operazioni di lease del disco per una macchina virtuale per la lettura del disco con VMware vSphere Virtual Disk Development Kit (VDDK).<br/><br/> - **Virtualmachine. provisioning. DiskRandomRead** (macchina virtuale-> provisioning-> consentire l'accesso al disco di sola lettura): consentire l'apertura di un disco in una macchina virtuale, per leggere il disco usando VDDK.<br/><br/> - **Virtualmachine. provisioning. DiskRandomAccess** (macchina virtuale-> provisioning-> consentire l'accesso al disco): consentire l'apertura di un disco in una macchina virtuale, per leggere il disco usando VDDK.<br/><br/> - **Virtualmachine. provisioning. GetVmFiles** (macchina virtuale-provisioning di >-> consentire il download della macchina virtuale): consente le operazioni di lettura nei file associati a una VM, per scaricare i log e risolvere i problemi se si verifica un errore.<br/><br/> - **Virtualmachine. state. \* *_ (Virtual Machine-> snapshot Management): consente la creazione e la gestione di snapshot di VM per la replica. <br/> <br/> -_* Virtualmachine. Interact. spento** (macchina virtuale-> interazione-> spegnimento): consente di spegnere la macchina virtuale durante la migrazione ad Azure.
...
**Replica simultanea** | 500 VM per ogni server vCenter. Se si dispone di altro, eseguirne la migrazione in batch di 500.
