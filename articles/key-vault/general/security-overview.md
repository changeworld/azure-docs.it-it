...
> [!NOTE]
> Per un elenco completo delle raccomandazioni Azure Key Vault sicurezza, vedere [Baseline di sicurezza per Azure Key Vault](security-baseline.md).
...
- Nonostante le vulnerabilità note nel protocollo TLS, non esiste alcun attacco noto che consenta a un agente dannoso di estrarre informazioni dall'insieme di credenziali delle chiavi quando l'utente malintenzionato avvia una connessione con una versione di TLS con vulnerabilità. L'utente malintenzionato dovrà comunque autenticarsi e autorizzare se stesso e, purché i client legittimi si connettono sempre con le versioni recenti di TLS, non esiste alcun modo in cui le credenziali potrebbero essere state perse dalle vulnerabilità nelle versioni precedenti di TLS.
...
Per informazioni dettagliate, vedere [Nozioni fondamentali Key Vault'autenticazione di base](authentication-fundamentals.md)

## <a name="key-vault-authentication-options"></a>Key Vault opzioni di autenticazione
