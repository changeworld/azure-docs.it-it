...
> * Caricare un recinto virtuale nel servizio dati di Mappe di Azure (anteprima) usando l'API di caricamento dati.
...
4. [Creare un gruppo di risorse](../azure-resource-manager/management/manage-resource-groups-portal.md#create-resource-groups). In questa esercitazione si assegnerà il nome *ContosoRental* al gruppo di risorse, ma è possibile scegliere qualsiasi nome.
...
4. La funzione registra le coordinate della posizione del dispositivo del veicolo, l'ora dell'evento e l'ID del dispositivo. Usa quindi l'[API Get Geofence del servizio spaziale](/rest/api/maps/spatial/getgeofence) per determinare se l'auto è uscita dal recinto virtuale. Se si trova al di fuori dei confini del recinto virtuale, la funzione archivia i dati sulla posizione ricevuti dall'evento in un contenitore BLOB. Inoltre, la funzione esegue una query di [ricerca di indirizzi inversa](/rest/api/maps/search/getsearchaddressreverse) per convertire le coordinate della posizione in un indirizzo e archiviarlo con il resto dei dati sulla posizione del dispositivo.
...
Nella figura seguente l'area del recinto virtuale è evidenziata in blu. L'itinerario dell'auto a noleggio è indicato da una linea verde.
   ...
   :::image type="content" source="./media/tutorial-iot-hub-maps/geofence-route.png" border="false" alt-text="Figura che mostra il percorso nel recinto virtuale.":::
...
1. Passare all'account di archiviazione appena creato. Nella sezione **Informazioni di base** fare clic sul collegamento **Contenitori**.
    ...
    :::image type="content" source="./media/tutorial-iot-hub-maps/containers.png" alt-text="Screenshot dei contenitori per l'archiviazione BLOB.":::
...
2. Nell'angolo in alto a sinistra selezionare **+ Contenitore**. Sul lato destro del browser viene visualizzato un pannello. Assegnare al contenitore il nome *contoso-rental-logs* e selezionare **Crea**.
     ...
     :::image type="content" source="./media/tutorial-iot-hub-maps/container-new.png" alt-text="Screenshot della creazione di un contenitore BLOB.":::
...
3. Passare al riquadro **Chiavi di accesso** nell'account di archiviazione e copiare i valori per **Nome account di archiviazione** e **Chiave** nella sezione **key1**. Entrambi i valori sono necessari nella sezione "Creare una funzione di Azure e aggiungere una sottoscrizione di Griglia di eventi".
    ...
    :::image type="content" source="./media/tutorial-iot-hub-maps/access-keys.png" alt-text="Screenshot della copia del nome dell'account di archiviazione e della chiave.":::

## <a name="upload-a-geofence"></a>Caricare un recinto virtuale

A questo punto usare l'[app Postman](https://www.getpostman.com) per [caricare il recinto virtuale](./geofence-geojson.md) in Mappe di Azure. Il recinto virtuale definisce l'area geografica autorizzata per il veicolo a noleggio. Per determinare se un'auto è uscita dall'area del recinto virtuale, verrà usato il recinto virtuale nella funzione di Azure.
...
1. Nella pagina di creazione dell'**App per le funzioni** assegnare un nome all'app per le funzioni. In **Gruppo di risorse** selezionare **ContosoRental** nell'elenco a discesa. Selezionare **.NET Core** come **Stack di runtime**. Nella parte inferiore della pagina selezionare **Avanti: Hosting >** .
    ...
    :::image type="content" source="./media/tutorial-iot-hub-maps/rental-app.png" alt-text="Screenshot della creazione di un'app per le funzioni.":::
...
1. Dopo aver creato l'app, aggiungervi una funzione. Passare all'app per le funzioni. Selezionare il riquadro **Funzioni**. Nella parte superiore della pagina selezionare **+ Aggiungi**. Viene visualizzato il pannello del modello di funzione. Scorrere il pannello verso il basso e selezionare **Trigger Griglia di eventi di Azure**.
    ...
    :::image type="content" source="./media/tutorial-iot-hub-maps/function-create.png" alt-text="Screenshot della creazione di una funzione.":::
...
1. Nel menu a sinistra selezionare il riquadro **Codice e test**. Copiare e incollare lo [script C#](https://github.com/Azure-Samples/iothub-to-azure-maps-geofencing/blob/master/src/Azure%20Function/run.csx) nella finestra del codice.
     ...
     :::image type="content" source="./media/tutorial-iot-hub-maps/function-code.png" alt-text="Screenshot del copia/incolla del codice nella finestra della funzione.":::
...
1. Nel menu a sinistra selezionare il riquadro **Integrazione**. Selezionare **Trigger Griglia di eventi** nel diagramma. Digitare un nome per il trigger, ad esempio *eventGridEvent*, quindi selezionare **Crea sottoscrizione di Griglia di eventi**.
     ...
     :::image type="content" source="./media/tutorial-iot-hub-maps/function-integration.png" alt-text="Screenshot dell'aggiunta della sottoscrizione di eventi.":::
...
Quando si aggiunge una sottoscrizione di Griglia di eventi alla funzione di Azure, viene creata automaticamente una route di messaggistica nell'hub IoT specificato. Il routing dei messaggi consente di instradare tipi di dati diversi a vari endpoint. Ad esempio, è possibile eseguire il routing dei messaggi di telemetria del dispositivo, degli eventi del ciclo di vita del dispositivo e degli eventi di modifica del dispositivo gemello. Per altre informazioni, vedere [Usare il routing di messaggi dell'hub IoT](../iot-hub/iot-hub-devguide-messages-d2c.md).

:::image type="content" source="./media/tutorial-iot-hub-maps/hub-route.png" alt-text="Screenshot del routing dei messaggi nell'hub IoT.":::
...
:::image type="content" source="./media/tutorial-iot-hub-maps/hub-filter.png" alt-text="Screenshot del filtro dei messaggi di routing.":::

>[!TIP]
>Esistono vari modi per eseguire query sui messaggi tra il dispositivo IoT e il cloud. Per altre informazioni sulla sintassi del routing dei messaggi, vedere [Routing dei messaggi dell'hub IoT](../iot-hub/iot-hub-devguide-routing-query-syntax.md).
...
:::image type="content" source="./media/tutorial-iot-hub-maps/terminal.png" alt-text="Screenshot dell'output del terminale.":::
...
:::image type="content" source="./media/tutorial-iot-hub-maps/blob.png" alt-text="Screenshot della visualizzazione di BLOB all'interno del contenitore.":::
...
:::image type="content" source="./media/tutorial-iot-hub-maps/violation-map.png" alt-text="Screenshot della mappa delle violazioni.":::
...
* [API REST di Mappe di Azure](/rest/api/maps/spatial/getgeofence)

* [Plug and Play IoT](../iot-pnp/index.yml)

Per ottenere un elenco di dispositivi Microsoft Azure Certified per IoT, visitare:

* [Dispositivi Azure Certified](https://devicecatalog.azure.com/)
...
> [Inviare dati di telemetria da un dispositivo](../iot-hub/quickstart-send-telemetry-dotnet.md)