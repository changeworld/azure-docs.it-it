...
- **SQL Server nella VM di Azure** : se il computer SQL è ospitato in una macchina virtuale di Azure, è possibile [abilitare il provisioning automatico <a name="auto-provision-mma"></a> dell'agente di log Analytics](security-center-enable-data-collection.md#auto-provision-mma). In alternativa, è possibile seguire la procedura manuale per l' [onboarding delle macchine virtuali dell'Hub Azure stack](quickstart-onboard-machines.md?pivots=azure-portal#onboard-your-azure-stack-hub-vms).
...
- **SQL Server** locale: se la SQL Server è ospitata in un computer Windows locale senza Azure Arc, sono disponibili due opzioni per la connessione ad Azure:
    ...
    - **Connettere il computer Windows senza Azure Arc** : se si sceglie di connettere un SQL Server in esecuzione in un computer Windows senza usare Azure Arc, seguire le istruzioni riportate in [connettere computer Windows a monitoraggio di Azure](../azure-monitor/agents/agent-windows.md).
...
1. Impostare l'opzione per **Azure Defender per SQL Server in macchine virtuali** piano **su on**. 
    ...
    :::image type="content" source="./media/security-center-advanced-iaas-data/sql-servers-on-vms-in-pricing-small.png" alt-text="Pagina dei prezzi del Centro sicurezza con piani facoltativi":::
    ...
    >[!TIP] 
    > Per creare una nuova area di lavoro, seguire le istruzioni riportate in [creare un'area di lavoro log Analytics](../azure-monitor/logs/quick-create-workspace.md).
