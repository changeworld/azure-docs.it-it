...
Il linguaggio delle espressioni di U-SQL è C# e offre diversi modi per scalare in orizzontale il codice .NET personalizzato.
...
### <a name="transform-scalar-inline-u-sql-c-expressions"></a>Trasforma espressioni scalari U-SQL in linea scalare

Il linguaggio delle espressioni di U-SQL è C#. Molte delle espressioni U-SQL inline scalari vengono implementate in modo nativo per migliorare le prestazioni, mentre le espressioni più complesse possono essere eseguite tramite la chiamata a .NET Framework.
...
Poiché il sistema di tipi U-SQL è basato sul sistema di tipi .NET e Spark ha un proprio sistema di tipi, che è influenzato dall'associazione del linguaggio host, sarà necessario assicurarsi che i tipi in uso siano vicini e per determinati tipi, gli intervalli di tipo, la precisione e/o la scala potrebbero essere leggermente diversi. Inoltre, U-SQL e Spark considerano `null` i valori in modo diverso.
...
- [Tipi Spark SQL e dataframes](https://spark.apache.org/docs/latest/sql-ref-datatypes.html)
...
Spark fornisce supporto per i concetti relativi all'archivio meta hive, principalmente database e tabelle, in modo che sia possibile eseguire il mapping di database e schemi U-SQL ai database hive e alle tabelle U-SQL per le tabelle Spark (vedere lo stato di [trasferimento dei dati archiviati nelle tabelle u-SQL](understand-spark-data-formats.md#move-data-stored-in-u-sql-tables)), ma non sono supportati viste, funzioni con valori di tabella (funzioni con valori), stored procedure, assembly U-SQL, origini dati esterne e così via.

Gli oggetti di codice U-SQL come viste, funzioni con valori, stored procedure e assembly possono essere modellati tramite funzioni di codice e librerie in Spark e a cui viene fatto riferimento usando la funzione del linguaggio host e i meccanismi di astrazione procedurali (ad esempio, tramite l'importazione di moduli Python o facendo riferimento a funzioni scala).
