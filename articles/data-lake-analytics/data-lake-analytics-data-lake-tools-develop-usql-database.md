...
1. Aggiungere riferimenti al progetto C# che includano UDO/UDAG/UDF per il progetto di database U-SQL.
   ...
   ![Strumenti di Data Lake per Visual Studio-Aggiungi riferimento](./media/data-lake-analytics-data-lake-tools-develop-usql-database/data-lake-tools-add-project-reference.png)

   ![Strumenti di Data Lake per Visual Studio-aggiungere un riferimento al progetto di database U-SQL](./media/data-lake-analytics-data-lake-tools-develop-usql-database/data-lake-tools-add-project-reference-wizard.png)
...
3. Aggiungere le **dipendenze gestite** e **i file aggiuntivi** , se presenti. Quando si aggiungono file, lo strumento usa il percorso relativo per assicurarsi di poter trovare gli assembly sia sul computer locale sia sul computer di compilazione in un momento successivo.
...
1. Per distribuire il database U-SQL selezionare **Invia**. Vengono caricate tutte le risorse, assembly e file aggiuntivi, e viene inviato un progetto U-SQL che include tutti gli strumenti DDL.
...
1. Fare clic con il pulsante destro del mouse su **Database U-SQL**, quindi scegliere **Distribuisci database**.

1. Impostare **Database Source** (Origine database) sul percorso del pacchetto di distribuzione del database U-SQL (file .usqldbpack).

1. Immettere il **Nome database** per creare un database. Se esiste già un database con lo stesso nome nell'account Azure Data Lake Analytics di destinazione, tutti gli oggetti definiti nel progetto di database vengono creati senza ricreare il database.
