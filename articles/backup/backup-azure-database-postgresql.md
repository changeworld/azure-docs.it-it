...
|Supporto  |Dettagli  |
|---------|---------|
|Distribuzioni supportate   |  [Database di Azure per PostgreSQL - Server singolo](https://docs.microsoft.com/azure/postgresql/overview#azure-database-for-postgresql---single-server)     |
...
1. Esistono due modi per avviare il processo:
    ...
    1. In alternativa, è possibile passare direttamente al backup degli insiemi di credenziali di [backup](backup-vault-overview.md)  ->  .
...
1. Passare a **Backup Center**  ->  **backup criteri**  ->  **Aggiungi**. In alternativa, è possibile passare a **backup** dell'insieme di credenziali di backup  ->    ->  **Aggiungi**.
...
1. Esistono due modi per avviare il processo di ripristino:
    ...
    1. In alternativa, è possibile passare direttamente alle istanze di backup dell'insieme di credenziali di **backup**  ->  . Selezionare l' **istanza di backup** corrispondente al database che si desidera ripristinare.
...
1. Selezionare i criteri di modifica dell' **istanza di backup**  ->  .
