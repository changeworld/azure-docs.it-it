...
> [!IMPORTANT]
> Si consiglia vivamente di usare l'interfaccia della riga di comando di Azure 2.0.74 o versioni successive per sfruttare tutti i vantaggi di un ripristino rapido, incluso il ripristino dei dischi gestiti. È consigliabile usare sempre la versione più recente.
...
L'output di questa query fornirà tutti i dettagli, ma ai fini dell'esercitazione sono rilevanti solo le informazioni relative al contenuto dell'account di archiviazione. È possibile usare la [funzionalità di query](/cli/azure/query-azure-cli) dell'interfaccia della riga di comando di Azure per recuperare i dettagli pertinenti
...
Ottenere ora il token di firma di accesso condiviso per il contenitore e il modello come descritto [qui](../azure-resource-manager/templates/secure-template-with-sas-token.md?tabs=azure-cli#provide-sas-token-during-deployment)
...
A questo punto, distribuire il modello per creare la nuova macchina virtuale, come spiegato [qui](../azure-resource-manager/templates/deploy-cli.md).
