...
- [Matrice di supporto](backup-support-matrix-mabs-dpm.md) per Backup di Azure server/System Center Data Protection Manager (DPM)
...
Backup al giorno (tramite l'estensione della macchina virtuale di Azure) | Un backup pianificato al giorno.<br/><br/>Il Backup di Azure supporta fino a tre backup su richiesta al giorno e un backup pianificato aggiuntivo.
...
Eseguire il backup della macchina virtuale la cui ora del computer è cambiata | Non supportato.<br/><br/> Se l'ora del computer viene modificata in una data e ora futura dopo l'abilitazione del backup per la macchina virtuale, tuttavia, anche se la modifica dell'ora viene ripristinata, il backup riuscito non è garantito.
...
Eseguire il backup di macchine virtuali Linux di Azure con punti di montaggio Docker | Attualmente, Backup di Azure non supporta l'esclusione dei punti di montaggio Docker perché vengono montati in percorsi diversi ogni volta.
...
- Backup di Azure non supporta una macchina virtuale Linux configurata tramite proxy se python versione 2.7 non è installato.
- Backup di Azure non supporta il backup di file NFS montati dall'archiviazione o da qualsiasi altro server NFS in computer Linux o Windows. Il backup viene eseguito solo su dischi collegati in locale alla macchina virtuale.
...
Tempo massimo prima della scadenza di un punto di ripristino | Nessun limite (99 anni).
...
**Sostituisci esistente** | È possibile ripristinare un disco e usarlo per sostituire un disco nella macchina virtuale esistente.<br/><br/> Deve essere presente la macchina virtuale corrente. Se è stata eliminata, non è possibile usare questa opzione.<br/><br/> Backup di Azure crea uno snapshot della macchina virtuale esistente prima della sostituzione del disco e l'archivia nella posizione di gestione temporanea specificata dall'utente. I dischi esistenti connessi alla macchina virtuale vengono sostituiti con il punto di ripristino selezionato.<br/><br/> Lo snapshot viene copiato nell'insieme di credenziali e conservato in conformità con i criteri di conservazione. <br/><br/> Dopo l'operazione di sostituzione del disco, il disco originale viene conservato nel gruppo di risorse. È possibile scegliere di eliminare manualmente i dischi originali se non sono necessari. <br/><br/>Sostituire esistente è supportato per le macchine virtuali gestite non crittografate e per le [macchine virtuali create usando immagini personalizzate.](https://azure.microsoft.com/resources/videos/create-a-custom-virtual-machine-image-in-azure-resource-manager-with-powershell/) Non è supportato per i dischi e le macchine virtuali non gestiti, le macchine virtuali classiche e [le macchine virtuali generalizzate.](../virtual-machines/windows/capture-image-resource.md)<br/><br/> Se il punto di ripristino ha un numero maggiore o minore di dischi rispetto alla macchina virtuale corrente, il numero di dischi nel punto di ripristino rifletterà solo la configurazione della macchina virtuale.<br><br> Sostituire esistente è supportato anche per le macchine virtuali con risorse collegate, ad esempio [l'identità](../active-directory/managed-identities-azure-resources/overview.md) gestita assegnata dall'utente [e Key Vault](../key-vault/general/overview.md).
...
Ripristinare i file da disco condiviso, unità temporanea, disco deduplicato, disco Ultra e disco con acceleratore di scrittura abilitato | Ripristino non supportato. <br/><br/>vedere [Supporto per l'archiviazione di macchine virtuali di Azure](#vm-storage-support).
...
Ripristinare una macchina virtuale del controller di dominio  | Supportata. Per informazioni dettagliate, vedere [Ripristinare le macchine virtuali del controller di dominio.](backup-azure-arm-restore-vms.md#restore-domain-controller-vms)
...
Backup della coerenza tra più macchine virtuali | Backup di Azure non fornisce la coerenza dei dati e dell'applicazione tra più macchine virtuali.
Backup con [impostazioni di diagnostica](../azure-monitor/essentials/platform-logs-overview.md)  | Non supportato. <br/><br/> Se il ripristino della macchina virtuale di Azure con impostazioni di diagnostica viene attivato usando l'opzione [Crea nuova](backup-azure-arm-restore-vms.md#create-a-vm), il ripristino ha esito negativo.
Ripristino di macchine virtuali aggiunte alla zona | Supportato (per una macchina virtuale di cui viene eseguito il backup dopo gennaio 2019 e in cui sono [disponibili le](https://azure.microsoft.com/global-infrastructure/availability-zones/) zone di disponibilità).<br/><br/>Attualmente è possibile eseguire il ripristino nella stessa zona bloccata nelle macchine virtuali. Tuttavia, se la zona non è disponibile a causa di un'interruzione del servizio, il ripristino avrà esito negativo.
Macchine virtuali di seconda generazione | Supportato <br> Backup di Azure supporta il backup e il ripristino di [macchine virtuali di seconda generazione](https://azure.microsoft.com/updates/generation-2-virtual-machines-in-azure-public-preview/). Quando queste macchine virtuali vengono ripristinate dal punto di ripristino, vengono ripristinate come [macchine virtuali di generazione 2.](https://azure.microsoft.com/updates/generation-2-virtual-machines-in-azure-public-preview/)
...
[VM Spot](../virtual-machines/spot-vms.md) | Non supportato. Backup di Azure ripristina le macchine virtuali spot come normali macchine virtuali di Azure.
[Host dedicato di Azure](../virtual-machines/dedicated-hosts.md) | Supportato
Configurazione di Spazi di archiviazione Windows di macchine virtuali di Azure autonome | Supportato
[Set di scalabilità di macchine virtuali di Azure](../virtual-machine-scale-sets/virtual-machine-scale-sets-orchestration-modes.md#scale-sets-with-flexible-orchestration) | Supportato per modelli di orchestrazione uniformi e flessibili per il backup e il ripristino di una singola macchina virtuale di Azure.

## <a name="vm-storage-support"></a>Supporto per l'archiviazione delle macchine virtuali

**Componente** | **Supporto**
--- | ---
Dischi di dati delle VM di Azure | Supporto per il backup di macchine virtuali di Azure con un massimo di 32 dischi.<br><br> Il supporto per il backup di macchine virtuali di Azure con dischi non gestiti o di macchine classiche copre un massimo di soli 16 dischi.
...
Dischi con l'acceleratore di scrittura abilitato | A partire dal 23 novembre 2020, è supportato solo nelle aree Corea centrale (KRC) e Sudafrica settentrionale (SAN) per un numero limitato di sottoscrizioni. Per le sottoscrizioni supportate, Backup di Azure eseguire il backup delle macchine virtuali con dischi abilitati per l'accelerazione di scrittura durante il backup.<br><br>Per le aree non supportate, è necessaria la connettività Internet nella macchina virtuale per creare snapshot delle macchine virtuali con WA abilitato.<br><br> **Nota importante:** in queste aree non supportate, le macchine virtuali con dischi WA necessitano di connettività Internet per un backup corretto (anche se tali dischi sono esclusi dal backup).
Backup e ripristino di macchine virtuali o dischi deduplicati | Backup di Azure non supporta la deduplicazione. Per altre informazioni, vedere questo [articolo](./backup-support-matrix.md#disk-deduplication-support) <br/> <br/>  - Backup di Azure non deduplica tra macchine virtuali nell'insieme di credenziali di Servizi di ripristino <br/> <br/>  - Se sono presenti macchine virtuali in stato di deduplicazione durante il ripristino, i file non possono essere ripristinati perché l'insieme di credenziali non comprende il formato. Tuttavia, è possibile eseguire correttamente il ripristino completo della macchina virtuale.
...
Ultra SSD dischi | Non supportata. Per altre informazioni, vedere [queste limitazioni.](selective-disk-backup-restore.md#limitations)
...
NVMe/dischi effimeri | Non supportata.
...
- Solo la chiave di crittografia è disponibile per sbloccare questi dati. Microsoft non può decrittografare i dati di backup in nessun caso.
...
- Per le VM di Azure, l'estensione VM legge i dati direttamente dall'account di archiviazione di Azure attraverso la rete di archiviazione. Non è necessario comprimere questo traffico.
