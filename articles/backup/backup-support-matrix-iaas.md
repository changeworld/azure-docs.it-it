...
Backup al giorno (tramite l'estensione della macchina virtuale di Azure) | Un backup pianificato al giorno.<br/><br/>Il servizio backup di Azure supporta fino a tre backup su richiesta al giorno e un ulteriore backup pianificato.
...
Dischi con l'acceleratore di scrittura abilitato | A partire dal 23 novembre 2020, supportato solo nelle aree Corea centrale (KRC) e Sud Africa settentrionale (SAN) per un numero limitato di sottoscrizioni. Per le sottoscrizioni supportate, backup di Azure eseguirà il backup delle macchine virtuali con dischi con accelerazione di scrittura (WA) abilitati durante il backup.<br><br>Per le aree non supportate, è necessaria la connettività Internet nella VM per creare snapshot delle macchine virtuali con WA abilitato.<br><br> **Nota importante**: in queste aree non supportate, per le macchine virtuali con dischi WA è necessaria la connettività Internet per un backup riuscito (anche se tali dischi sono esclusi dal backup).
