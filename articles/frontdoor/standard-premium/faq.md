...
description: Questa pagina fornisce le risposte alle domande frequenti su Azure front door standard/Premium.
services: frontdoor
author: duongau
...
Questo articolo risponde alle domande più comuni sulle funzionalità e sulle funzionalità di Azure front door.

## <a name="general"></a>Generale

### <a name="what-is-azure-front-door"></a>Che cos'è il servizio Frontdoor di Azure?

Il front-end di Azure è una rete CDN moderna, affidabile e sicura con protezione dalle minacce intelligente. Offre accelerazione del contenuto statico e dinamico, bilanciamento del carico globale e sicurezza avanzata per le applicazioni, le API, i siti Web e i servizi cloud su vasta scala globali con protezione dalle minacce intelligente.

### <a name="what-features-does-azure-front-door-support"></a>Quali funzionalità sono supportate da Azure front door?

Lo sportello anteriore di Azure supporta:

* Contenuto statico e accelerazione dell'applicazione dinamica
* Offload TLS/SSL e TLS end-to-end
* Web application firewall
* Affinità di sessione basata su cookie
* Routing basato su percorso URL
* Certificati gratuiti e più gestioni dominio

Per un elenco completo delle funzionalità supportate, vedere [Panoramica di sportello anteriore di Azure](overview.md).
...
Sebbene sia il gateway principale che quello del gateway applicazione siano servizi di bilanciamento del carico di livello 7 (HTTP/HTTPS), la differenza principale è che la porta anteriore è un servizio globale. Il gateway applicazione è un servizio a livello di area. Mentre la porta anteriore può bilanciare il carico tra le diverse unità di scala/cluster/unità di timbri tra le aree, il gateway applicazione consente di bilanciare il carico tra le VM o i contenitori che si trovano all'interno dell'unità di scala.
...
* La porta anteriore e il gateway applicazione supportano entrambi l'affinità di sessione. Lo sportello anteriore può indirizzare il traffico risultante da una sessione utente allo stesso cluster o back-end in una determinata area. Il gateway applicazione può indirizzare creare affinità tra il traffico allo stesso server all'interno del cluster.  
...
### <a name="what-resources-are-supported-today-as-part-of-origin-group"></a>Quali risorse sono attualmente supportate come parte del gruppo di origine?

Il gruppo di origine può essere costituito da archiviazione, app Web, istanze Kubernetes o qualsiasi altro nome host personalizzato con connettività pubblica. Per lo sportello anteriore di Azure è necessario che le origini siano definite tramite un indirizzo IP pubblico o un nome host DNS risolvibile pubblicamente. I membri del gruppo Origin possono trovarsi in zone, aree o anche all'esterno di Azure, purché dispongano di connettività pubblica.

### <a name="what-regions-is-the-service-available-in"></a>In quale aree è disponibile il servizio?

Il front-end di Azure è un servizio globale e non è associato ad alcuna area di Azure specifica. L'unica posizione che è necessario specificare durante la creazione di una porta anteriore è per il gruppo di risorse. Questo percorso indica in sostanza dove verranno archiviati i metadati per il gruppo di risorse. La risorsa della porta anteriore stessa viene creata come risorsa globale e la configurazione viene distribuita a livello globale in tutti i pop (punto di presenza). 
...
Azure front door è un servizio multi-tenant distribuito a livello globale. L'infrastruttura per la porta anteriore è condivisa tra tutti i clienti. Creando un profilo di porta anteriore, si definisce la configurazione specifica necessaria per l'applicazione. Le modifiche apportate alla porta anteriore non influiscono su altre configurazioni di porte anteriori.

### <a name="is-http-https-redirection-supported"></a>È supportato il reindirizzamento HTTP->HTTPS?

Sì. In realtà, il front-end di Azure supporta l'host, il percorso, il reindirizzamento della stringa di query e parte del reindirizzamento URL. Altre informazioni sul [Reindirizzamento URL](concept-rule-set-url-redirect-and-rewrite.md). 
...
* Configurare ACLing IP per i back-end in modo che accettino il traffico dallo spazio degli indirizzi IP back-end di Azure front door e dai servizi di infrastruttura di Azure. Vedere i dettagli dell'indirizzo IP seguente per ACLing il back-end:
    ...
    * Vedere la sezione *AzureFrontDoor. backend* negli [intervalli IP e nei tag di servizio di Azure](https://www.microsoft.com/download/details.aspx?id=56519) per l'intervallo di indirizzi IP back-end IPv4. È anche possibile usare il tag di servizio *AzureFrontDoor. backend* nei [gruppi di sicurezza di rete](../../virtual-network/network-security-groups-overview.md#security-rules).
...
* Eseguire un'operazione GET sulla porta anteriore con la versione dell'API `2020-01-01` o una versione successiva. Nella chiamata API cercare il `frontdoorID` campo. Filtrare in base all'intestazione in ingresso '**X-Azure-FDID**' inviata dalla porta anteriore al back-end con il valore del campo `frontdoorID` . È anche possibile trovare `Front Door ID` il valore nella sezione Panoramica della pagina del portale di porta anteriore. 
...
L'IP anycast front-end per la porta anteriore in genere non cambia e può rimanere statico per la durata della porta anteriore. Non sono tuttavia previste **garanzie** per lo stesso. Non prendere alcuna dipendenza diretta dall'IP.
...
Tutti gli aggiornamenti alle route o ai pool back-end sono semplici e provocheranno un tempo di inattività pari a zero (se la nuova configurazione è corretta). Gli aggiornamenti dei certificati non causeranno alcuna interruzione, a meno che non si stia passando da' Azure front door Managed ' a' use your own cert ' o viceversa.
...
Il servizio di Azure front door (AFD) richiede un indirizzo IP pubblico o un nome DNS risolvibile pubblicamente per instradare il traffico. Il front-end di Azure non può essere indirizzato direttamente alle risorse in una rete virtuale. Per risolvere il problema, è possibile usare un gateway applicazione o un Azure Load Balancer con un indirizzo IP pubblico.
...
### <a name="can-i-configure-azure-cdn-behind-my-front-door-profile-or-front-door-behind-my-azure-cdn"></a>È possibile configurare la rete CDN di Azure dietro il profilo della porta anteriore o anteriore dietro la rete CDN di Azure?

La rete CDN di Azure e la rete CDN di Azure non possono essere configurate insieme perché entrambi i servizi usano gli stessi siti Azure Edge per rispondere alle richieste. 
...
Il front-end di Azure è una piattaforma multi-tenant distribuita a livello globale con una grande quantità di capacità per soddisfare le esigenze di scalabilità dell'applicazione. Distribuito dal perimetro della rete globale di Microsoft, la porta anteriore fornisce funzionalità di bilanciamento del carico globale che consentono di eseguire il failover dell'intera applicazione o anche di singoli microservizi tra aree o cloud diversi.
...
Per abilitare il protocollo HTTPS in un dominio personalizzato di una porta anteriore, è possibile scegliere un certificato che viene gestito da sportello anteriore di Azure o usare il proprio certificato.
...
Per il certificato TLS/SSL personalizzato, la rotazione automatica non è supportata. Analogamente al modo in cui si configura la prima volta per un determinato dominio personalizzato, è necessario puntare davanti alla versione del certificato corretta nel Key Vault. Assicurarsi che l'entità servizio per la porta anteriore abbia ancora accesso al Key Vault. Questa operazione di implementazione del certificato aggiornata dalla porta anteriore non provoca alcun tempo di inattività di produzione purché il nome del soggetto o la SAN per il certificato non cambiano.
...
L'uso di domini personalizzati con TLS 1.0/1.1 abilitato sono supportati i pacchetti di crittografia seguenti:
...
No, la configurazione della porta anteriore per specifici pacchetti di crittografia non è supportata. È possibile ottenere un certificato TLS/SSL personalizzato dall'autorità di certificazione (ad indicare VeriSign, Entrust o DigiCert). Quindi, quando si generano pacchetti di crittografia specifici contrassegnati per il certificato. 
...
Sì, il front-end di Azure supporta l'offload TLS/SSL e la porta TLS end-to-end, che consente di crittografare nuovamente il traffico nel back-end. Poiché le connessioni al back-end avvengono tramite l'indirizzo IP pubblico, è consigliabile configurare la porta principale per l'uso di HTTPS come protocollo di trasmissione.
...
* **Backend**: non è possibile usare certificati autofirmati quando si esegue l'inoltro del traffico come probe di integrità HTTPS o HTTPS o si compila la cache per dall'origine per le regole di routing con la memorizzazione nella cache abilitata.
* Front- **end**: non è possibile usare certificati autofirmati quando si usa un certificato TLS/SSL personalizzato per abilitare HTTPS nel dominio personalizzato.
...
* **Mancata corrispondenza del nome del soggetto del certificato**: per le connessioni HTTPS, la porta anteriore prevede che il back-end presenti un certificato da una CA valida con i nomi di soggetto corrispondenti al nome host del back-end. Ad esempio, se il nome host del back-end è impostato su `myapp-centralus.contosonews.net` e il certificato presentato dal back-end durante l'handshake TLS non ha `myapp-centralus.contosonews.net` o `*myapp-centralus*.contosonews.net` nel nome del soggetto. Quindi la porta anteriore rifiuterà la connessione e restituirà un errore. 
    * **Soluzione**: non è consigliabile dal punto di vista della conformità, ma è possibile aggirare questo errore disabilitando la verifica del nome del soggetto del certificato per la porta anteriore. È possibile trovare questa opzione in impostazioni in portale di Azure e in BackendPoolsSettings nell'API.
* **Certificato di hosting back-end da CA non valida**: solo i certificati delle [autorità di certificazione valide](troubleshoot-allowed-certificate-authority.md) possono essere usati nel back-end con sportello anteriore. I certificati da autorità di certificazione interne o certificati autofirmati non sono consentiti.
...
Per informazioni sui log e altre funzionalità di diagnostica, vedere monitoraggio di metriche e log per lo sportello anteriore.
...
I log di controllo sono disponibili per il front-end di Azure. Nel portale selezionare **log attività** nella pagina del menu della porta anteriore per accedere al log di controllo. 
